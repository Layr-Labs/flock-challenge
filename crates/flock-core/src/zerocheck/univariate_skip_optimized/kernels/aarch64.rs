...
pub(crate) fn verify_bstatic_guards(
    ...
    let a_ok = |blk: usize| -> bool {
        let w = blk / 16;
        let b_med = blk % 16;
        // Unlike the B-side census guards, these A-side zero-byte predicates
        // are not both structural theorems. Check every live position before
        // hoisting either sniff across the ranked sweep.
...
pub(crate) fn shift_reduce_inner_ab_fused_neon_checked_with_fast_policy<const FAST_POLICY: u8>(
    ...
    shift_reduce_inner_ab_fused_neon_checked_impl::<FAST_POLICY, false>(
        a_packed,
        b_packed,
        inv_table,
        chunk_byte_base,
...
/// Ranked specialization. Its caller has verified every live static-B arm
/// and all five specialized sniff predicates once for this proof.
#[cfg(target_arch = "aarch64")]
#[inline(always)]
pub(crate) fn shift_reduce_inner_ab_fused_neon_ranked_verified<const FAST_POLICY: u8>(
    a_packed: &[u8],
    b_packed: &[u8],
    inv_table: &InvNttTableByteSingleGf8,
    chunk_byte_base: usize,
    ...
    shift_reduce_inner_ab_fused_neon_checked_impl::<FAST_POLICY, true>(
        a_packed,
        b_packed,
        inv_table,
        chunk_byte_base,
...
#[cfg(target_arch = "aarch64")]
#[inline(always)]
fn shift_reduce_inner_ab_fused_neon_checked_impl<
...
>(
    ...
    let specialized_direct = bstatic_w <= 1
        && if RANKED_VERIFIED {
            (0x6000_0007u64 >> (bstatic_w * 16 + b_med)) & 1 == 1
        } else {
            matches!(
                static_b_context,
                Some(StaticBContext::Prepared { specialized, .. })
                    if (specialized >> (bstatic_w * 16 + b_med)) & 1 == 1
            )
        };
    if specialized_direct && let Some(context) = static_b_context {
    ...
    if bstatic_w <= 1
        && let Some(context) = static_b_context
    {
        let enabled = RANKED_VERIFIED
            || !matches!(context, StaticBContext::LegacyPerCall)
            || !bstatic_legacy_enabled();
        ...
        let pure = RANKED_VERIFIED
            || matches!(
                context,
                StaticBContext::Prepared { verified, .. }
                    if (verified >> (bstatic_w * 16 + b_med)) & 1 == 1
            );
        let handled = enabled
            && if fast_shift_reduce_with_policy::<FAST_POLICY>() {
                if pure {
