...
pub(crate) unsafe fn accumulate_c_banks(
    ...
    unsafe {
        ...
        if drain4 {
            ...
            // Lane-major order exposes eight independent table streams to
            // the OoO core before moving to the next four-lane group, rather
            // than completing one bank's dependent RMW chain first.
            for lane in (0..64).step_by(4) {
                for s in 0..8 {
