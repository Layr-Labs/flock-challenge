...
/// [`fold_round2_compact_chunk_neon_unchecked_8`]. Each 1 KiB packed block is
/// laid out as 64 even rows followed by 64 adjacent-row deltas; the odd row is
/// reconstructed in registers for the round message, and only anchors are
/// written because the deltas already reside in A/B.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn fold_round2_in_place_deltas_chunk_neon_unchecked_8(
    ...
    unsafe {
        ...
        for x_lo in 0..lo_size {
            ...
            let a0_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(even_off).cast::<u64>(),
            ));
            let da = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(delta_off).cast::<u64>(),
            ));
            let b0_code = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(even_off).cast::<u64>(),
            ));
            let db = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(delta_off).cast::<u64>(),
            ));
...
/// Anchors-only CPU sibling for an in-place-delta GPU prefix. Delta rows are
/// irrelevant to anchors, so this path reads just the retained even half.
#[cfg(target_arch = "aarch64")]
pub(crate) unsafe fn fold_round2_in_place_deltas_chunk_neon_anchors_only_8(
    ...
    unsafe {
        ...
        for x_lo in 0..lo_size {
            ...
            let (even_off, _) = in_place_pair_offsets(pair_idx_base + x_lo);
            let a0_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(even_off).cast::<u64>(),
            ));
            let b0_code = u64::from_le(core::ptr::read_unaligned(
...
/// First-tail reconstruction from anchors plus deltas retained in the second
/// half of each block-split 1 KiB A/B region.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn fold_in_place_deltas_chunk_neon_unchecked_8(
    ...
    unsafe {
        ...
        for x_lo in 0..lo_size {
            let out = 2 * x_lo;
            let (_, delta0) = in_place_pair_offsets(pair_idx_base + out);
            let (_, delta1) = in_place_pair_offsets(pair_idx_base + out + 1);
            let a0_code = u64::from_le(core::ptr::read_unaligned(
                a_pairs.add(delta0).cast::<u64>(),
            ));
            let b0_code = u64::from_le(core::ptr::read_unaligned(
                b_pairs.add(delta0).cast::<u64>(),
            ));
            let a1_code = u64::from_le(core::ptr::read_unaligned(
                a_pairs.add(delta1).cast::<u64>(),
            ));
            let b1_code = u64::from_le(core::ptr::read_unaligned(
                b_pairs.add(delta1).cast::<u64>(),
            ));
