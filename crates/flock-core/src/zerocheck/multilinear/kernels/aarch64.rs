...
unsafe fn r2_group_fold_and_store_unpadded(
    ...
    unsafe {
        ...
        // Deltas are algebraically bit-exact: when deg is true, b_code == u64::MAX, so b0_code ^ b1_code == 0.
        // Storing deltas immediately overlaps non-temporal write-back with table gathers and drops register pressure.
        let d0 = core::mem::transmute::<[u64; 2], uint64x2_t>([a0_code ^ a1_code, b0_code ^ b1_code]);
        let d1 = core::mem::transmute::<[u64; 2], uint64x2_t>([a2_code ^ a3_code, b2_code ^ b3_code]);
        store_delta_pair_nt(deltas.add(32 * u), d0, d1);

        let deg0 = degen && (b0_code & b1_code) == u64::MAX;
        let deg1 = degen && (b2_code & b3_code) == u64::MAX;

        let (a0, a1, b0, b1) = if deg0 {
            let (a0, a1) = fold_two_row_codes_q(table_data, a0_code, a1_code);
