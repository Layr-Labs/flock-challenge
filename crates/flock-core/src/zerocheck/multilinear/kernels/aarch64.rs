...
/// writes byte-identical `a_out`/`b_out` groups and skips the round-4/5
/// products entirely. Used for the chunk prefix whose products the GPU
/// K-pass arm owns — the CPU must still materialize every composed output
/// for later cascade rounds, but the message/lookahead muls move to the GPU.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
#[allow(clippy::too_many_arguments)]
pub(crate) unsafe fn fold2_compact_and_round45_chunk_neon_reconstruct_only_8(
    table_l1: *const u8,
    table_l3: *const u8,
    rho2: F128,
    anchors: *const F128,
