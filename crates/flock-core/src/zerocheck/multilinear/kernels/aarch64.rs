...
/// Round-two message-only worker used by the no-checkpoint path. It performs
/// the same four folded-row loads as the compact producer, but deliberately
/// emits no anchors or deltas. The packed A/B rows remain live for the
/// challenge-dependent reconstruction after the transcript samples rho.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn fold_round2_message_only_neon_unchecked_8(
    table_data: *const u8,
    a_packed: *const u8,
    b_packed: *const u8,
    eq_lo: *const F128,
...
/// Reconstruct the first post-rho multilinear level directly from the packed
/// A/B inputs. For two output entries at a time, the normal 32 KiB table folds
/// each even input row and the challenge-scaled table folds its XOR delta:
///
/// `fold_z(row0) + rho * fold_z(row0 ^ row1)`.
///
/// This is algebraically the compact anchor/delta reconstruction, but avoids
/// writing and rereading the large anchor and delta checkpoints between the
/// round-two transcript message and the sampled rho.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
pub(crate) unsafe fn fold_round2_reconstruct_direct_chunk_neon_unchecked_8(
    table_data: *const u8,
    ...
    unsafe {
        ...
        for x_lo in 0..lo_size {
            ...
            let a0_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(byte0).cast::<u64>(),
            ));
            let a1_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(byte0 + 8).cast::<u64>(),
            ));
            let a2_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(byte1).cast::<u64>(),
            ));
            let a3_code = u64::from_le(core::ptr::read_unaligned(
                a_packed.add(byte1 + 8).cast::<u64>(),
            ));
            let b0_code = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(byte0).cast::<u64>(),
            ));
            let b1_code = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(byte0 + 8).cast::<u64>(),
            ));
            let b2_code = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(byte1).cast::<u64>(),
            ));
            let b3_code = u64::from_le(core::ptr::read_unaligned(
                b_packed.add(byte1 + 8).cast::<u64>(),
            ));

            let (aa0, aa1, bb0, bb1) =
                fold_four_row_codes_q(table_data, a0_code, a2_code, b0_code, b2_code);
            ...
            let valid1 = ((pair_idx_base + out + 1) & pair_in_block_mask)
                < useful_pairs_inclusive;
            let (a0, b0) = if valid0 {
                (veorq_u64(aa0, ad0), veorq_u64(bb0, bd0))
            } else {
                (zero, zero)
            };
            let (a1, b1) = if valid1 {
                (veorq_u64(aa1, ad1), veorq_u64(bb1, bd1))
            } else {
                (zero, zero)
            };
