...
/// Selector for adopting all six lookahead aggregates from the round-two GPU
/// arm. The exact full path is retained but parked after three official share
/// experiments regressed; scored execution uses the promoted odd-only split.
#[cfg(all(target_os = "macos", target_arch = "aarch64"))]
fn zc_r2_odd_offload_enabled() -> bool {
    // Parked after three verified official regressions; retain the exact code
    // and oracle for future runner diagnostics without selecting it in score.
    false
}
