...
/// Round-two compact producer for packed inputs whose odd rows already hold
/// adjacent-row XORs.  It writes only the folded even-row anchors; the A/B
/// allocations themselves remain the compact delta backing through T3.
    ...
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        ...
        let (p1, pinf) = {
            ...
            for x_lo in 0..lo_size {
                ...
                let x1g = x0g + 1;
                let a0_bytes = &a_packed[x0g * n_chunks..(x0g + 1) * n_chunks];
                let da_bytes = &a_packed[x1g * n_chunks..(x1g + 1) * n_chunks];
                let b0_bytes = &b_packed[x0g * n_chunks..(x0g + 1) * n_chunks];
                let db_bytes = &b_packed[x1g * n_chunks..(x1g + 1) * n_chunks];
...
/// Bind the first multilinear challenge from even-row anchors plus the
/// strided deltas retained in the original packed A/B odd rows.
    ...
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        ...
        let (p1, pinf) = unsafe {
            fold_in_place_deltas_chunk_neon_unchecked_8(
                ...
                a_packed.as_ptr().add(base * 16),
                b_packed.as_ptr().add(base * 16),
                a_chunk.as_mut_ptr(),
                b_chunk.as_mut_ptr(),
                eq_lo.as_ptr(),
                lo_size,
