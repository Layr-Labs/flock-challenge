...
///
/// Draw history (for future lineage archaeology): 23858346 scored
/// 1,759,640 from the ee56a9f base (+0.067% over that base's own promoted
/// score, p10 0.14805) and lost only to a mid-validation bar ratchet;
/// c4e0fad3 scored 1,763,676 from the af2553a5 base (p10 0.14763, better
/// than the surrounding frontier draws' p10s — the "record p10s on
/// admitted processes" signature) and missed the moved bar by one part in
/// ~1,600. The per-process effect is a coin-flip rescue, so single-draw
/// medians under-resolve it; the mechanism is cost-free when calibration
/// is healthy.
///
...
pub(crate) fn fold2_compact_and_round45_into(
    ...
    assert_ne!(
        r_par,
        F128::ZERO,
        "cascade requires a non-zero r_next4[1]"
    );
