...
use kernels::aarch64::{
    ...
    fold_round2_message_only_neon_unchecked_8,
    fold_round2_reconstruct_direct_chunk_neon_unchecked_8,
};
#[cfg(all(
    target_arch = "x86_64",
    target_feature = "avx512f",
...
/// Compute the round-two message while keeping the packed witness rows live.
/// Unlike the compact producer above, this does not materialize folded anchors
/// or packed deltas; the sampled rho is applied later by direct reconstruction.
pub fn round2_message_only_padded(
    a_packed: &[u8],
    b_packed: &[u8],
    m: usize,
    k_skip: usize,
    ...
    assert_eq!(k_skip, 6, "optimized round-two variant is k_skip=6 only");
    assert_eq!(table.n_chunks, 8);
    let n_chunks = table.n_chunks;
    let n_out = 1usize << (m - k_skip);
    let n_pairs = n_out / 2;
    ...
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        ...
        let (p1, pinf) = unsafe {
            fold_round2_message_only_neon_unchecked_8(
                table.data.as_ptr().cast::<u8>(),
                a_packed.as_ptr().add(pair_idx_base * 2 * n_chunks),
                b_packed.as_ptr().add(pair_idx_base * 2 * n_chunks),
                eq_lo.as_ptr(),
                lo_size,
                pair_idx_base,
                pair_in_block_mask,
        ...
        let (p1, pinf) = {
            ...
            for x_lo in 0..lo_size {
                ...
                let x0g = row_base + 2 * x_lo;
                let x1g = x0g + 1;
                let a0 = table.fold_one_row(
                    &a_packed[x0g * n_chunks..(x0g + 1) * n_chunks],
                );
                let a1 = table.fold_one_row(
                    &a_packed[x1g * n_chunks..(x1g + 1) * n_chunks],
                );
                let b0 = table.fold_one_row(
                    &b_packed[x0g * n_chunks..(x0g + 1) * n_chunks],
                );
                let b1 = table.fold_one_row(
                    &b_packed[x1g * n_chunks..(x1g + 1) * n_chunks],
                );
                let eq_l = eq_lo[x_lo];
                p1_acc ^= eq_l.mul_unreduced(a1 * b1);
                pinf_acc ^= eq_l.mul_unreduced((a0 + a1) * (b0 + b1));
        ...
        let eq_h = eq_hi[x_hi];
        // SAFETY: each queue job owns one distinct partials slot.
        unsafe {
            *partials_base.ptr().add(x_hi) = (eq_h * p1, eq_h * pinf);
        }
    });
...
/// Apply the sampled round-two rho directly to the packed witness rows and
/// compute the following multilinear message, without an anchor/delta
/// checkpoint between the two transcript observations.
pub fn fold_round2_reconstruct_direct_padded(
    a_packed: &[u8],
    b_packed: &[u8],
    m: usize,
    k_skip: usize,
    ...
    assert_eq!(a_packed.len(), n_out * n_chunks);
    assert_eq!(b_packed.len(), n_out * n_chunks);
    assert!(!r_next.is_empty());
    assert_eq!(r_next.len(), n_pairs.trailing_zeros() as usize);

    let scaled_table = table.scaled_linear(r_fold);
    let eq = SplitEqGhash::with_n_hi(&r_next[1..], COMPACT_RECONSTRUCTION_N_HI);
    let lo_size = 1usize << eq.n_lo;
    let hi_size = 1usize << eq.n_hi;
    assert_eq!(lo_size * hi_size * 2, n_pairs);
    let chunk_size = 2 * lo_size;
    let eq_hi = &eq.hi;
    let eq_lo = &eq.lo;
    let (pair_in_block_mask, useful_pairs_inclusive) = round2_pair_skip(padding, k_skip);

    let mut a_out = crate::scratch::take_f128(n_pairs);
    let mut b_out = crate::scratch::take_f128(n_pairs);
    let mut partials: Vec<(F128, F128)> = vec![(F128::ZERO, F128::ZERO); hi_size];
    let a_base = crate::epool::SyncPtr(a_out.as_mut_ptr());
    let b_base = crate::epool::SyncPtr(b_out.as_mut_ptr());
    let partials_base = crate::epool::SyncPtr(partials.as_mut_ptr());
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        ...
        let (a_chunk, b_chunk) = unsafe {
            (
                std::slice::from_raw_parts_mut(a_base.ptr().add(base), chunk_size),
                std::slice::from_raw_parts_mut(b_base.ptr().add(base), chunk_size),
            )
        };
        ...
        let (p1, pinf) = unsafe {
            fold_round2_reconstruct_direct_chunk_neon_unchecked_8(
                table.data.as_ptr().cast::<u8>(),
                scaled_table.as_ptr().cast::<u8>(),
                a_packed.as_ptr().add(base * 2 * n_chunks),
                b_packed.as_ptr().add(base * 2 * n_chunks),
                a_chunk.as_mut_ptr(),
                b_chunk.as_mut_ptr(),
                eq_lo.as_ptr(),
                lo_size,
            ...
            for x_lo in 0..lo_size {
                ...
                for lane in 0..2 {
                    ...
                    if (index & pair_in_block_mask) >= useful_pairs_inclusive {
                        a_chunk[out + lane] = F128::ZERO;
                        b_chunk[out + lane] = F128::ZERO;
                        continue;
                    }

                    let row0 = 2 * index;
                    let row1 = row0 + 1;
                    let a0_bytes = &a_packed[row0 * n_chunks..(row0 + 1) * n_chunks];
                    let a1_bytes = &a_packed[row1 * n_chunks..(row1 + 1) * n_chunks];
                    let b0_bytes = &b_packed[row0 * n_chunks..(row0 + 1) * n_chunks];
                    let b1_bytes = &b_packed[row1 * n_chunks..(row1 + 1) * n_chunks];
                    let mut a_delta = F128::ZERO;
                    let mut b_delta = F128::ZERO;
                    for j in 0..n_chunks {
                        a_delta +=
                            scaled_table[j * 256 + (a0_bytes[j] ^ a1_bytes[j]) as usize];
                        b_delta +=
                            scaled_table[j * 256 + (b0_bytes[j] ^ b1_bytes[j]) as usize];
                    }
                    a_chunk[out + lane] = table.fold_one_row(a0_bytes) + a_delta;
                    b_chunk[out + lane] = table.fold_one_row(b0_bytes) + b_delta;
                }
...
mod tests {
    ...
    fn compact_round2_and_reconstruction_match_legacy_with_poisoned_scratch() {
        ...
        for donate_f128 in [false, true] {
            ...
            let (direct_m1, direct_mi) = round2_message_only_padded(
                &a_packed,
                &b_packed,
                M,
                K_SKIP,
            ...
            assert_eq!(
                (direct_m1, direct_mi),
                (legacy_m1, legacy_mi),
                "message-only round-two mismatch; donate_f128={donate_f128}"
            );
            ...
            for &rho in &rhos {
                ...
                let (direct_a, direct_b, direct_m1, direct_mi) =
                    fold_round2_reconstruct_direct_padded(
                        &a_packed,
                        &b_packed,
                        M,
                        K_SKIP,
                ...
                assert_eq!(
                    direct_a, expected_a,
                    "direct reconstructed A mismatch; donate_f128={donate_f128}, rho={rho:?}"
                );
                assert_eq!(
                    direct_b, expected_b,
                    "direct reconstructed B mismatch; donate_f128={donate_f128}, rho={rho:?}"
                );
                assert_eq!(
                    (direct_m1, direct_mi),
                    expected_msg,
                    "direct post-reconstruction message mismatch; donate_f128={donate_f128}, rho={rho:?}"
                );
