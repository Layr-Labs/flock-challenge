...
pub fn fold_and_compute_round_pair_into(
    ...
    // Hetero-queue drain: same contract as the compact round-2 conversions —
    // the idle efficiency cores claim chunks from a shared queue instead of
    // an equal-band split, each chunk writing only its own output ranges and
    // partials slot. XOR reduce is order-independent → bit-identical output.
    let mut partials: Vec<(F128, F128)> = vec![(F128::ZERO, F128::ZERO); hi_size];
    ...
    crate::epool::run_hetero_chunks(hi_size, |x_hi| {
        // SAFETY: the queue hands out each x_hi exactly once; chunk x_hi
        // exclusively owns its output ranges and partials[x_hi]; the queue's
        // completion join publishes writes before the reduction reads them.
        let (a_out, b_out) = unsafe {
            (
