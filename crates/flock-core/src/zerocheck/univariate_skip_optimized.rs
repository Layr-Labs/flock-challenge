...
fn process_one_x_hi_with_precomputed_ab(
    ...
    for x_outer_lo in 0..big_lo_size {
        ...
        kernels::accumulate_convert_with_s_hat_v_raw_c(
            &state.chunk_ab_bytes,
            &c_packed[chunk_byte_base..chunk_byte_base + n_b_med * 64],
            n_b_med,
            convert,
            eq_lo_val,
            c_tables,
...
mod tests {
    ...
    /// The direct raw-C composition must equal the incumbent two-stage path:
    /// first transpose every canonical C row, then cross-transpose `b_med`
    /// into the eight banks. Cover all padded boundary sizes and non-zero
    /// accumulators so layout or `+=` mistakes cannot cancel accidentally.
    #[test]
    fn raw_c_banks_match_two_stage_path() {
        let mut rng = Rng::new(0xD1EC_7C00);
        for n_b_med in 0..=(1 << N_MEDIUM) {
            let mut raw_c = [[0u8; 64]; 1 << N_MEDIUM];
            let mut transposed_c = [[0u8; 64]; 1 << N_MEDIUM];
            for b_med in 0..(1 << N_MEDIUM) {
                for byte in &mut raw_c[b_med] {
                    *byte = (rng.next_u64() & 0xff) as u8;
                }
                bit_transpose_64bytes(&raw_c[b_med], &mut transposed_c[b_med]);
            }
            let mask_tables = build_c_mask_tables(&[rng.f128()]);
            let seed: [[F128; ELL]; N_C_BANKS] =
                core::array::from_fn(|_| core::array::from_fn(|_| rng.f128()));
            ...
            let mut got = seed;
            kernels::accumulate_c_banks_raw(
                raw_c.as_flattened(),
                n_b_med,
                &mask_tables,
                &mut got,
            ...
            let mut want = seed;
            kernels::accumulate_c_banks_scalar(
                &transposed_c,
                n_b_med,
                &mask_tables,
                &mut want,
