...
// `AB_EQ_FOLD_TABLE_BITS` bits in the table index, so the ranked shape gets
// 128 tables (8 MiB total, L2-resident on the performance cluster) whose hot
// window is one 64 KiB table per 2^s consecutive chunks — exactly the
// incumbent convert table's residency — and 32 banks (32 KiB per worker,
// L2-resident on both core types). Both pools of `run_hetero_chunks` run the
// same `s`; nothing in the mechanism is core-type specific, and the E-core
// arm sees the same 64 KiB hot table it sees today. `FLOCK_ZC_AB_EQ_FOLD_S`
...
/// Default 7 (s = 5) is the mul-optimal split (786K vs 1.18M F128 muls per
/// prove at s=7) and the geometry the GPU AB arm is built for (banks = 32,
/// chunks_per_u = 4). `FLOCK_ZC_AB_EQ_FOLD_S` still overrides for A/B.
...
fn round1_shift_reduce_ab_packed_padded_with_precomputed_with_fold(
    ...
    match fold.bank_bits(eq.n_lo) {
        ...
        Some(bank_bits) => {
            ...
            // Round-one AB-completion GPU arm (macOS/aarch64 ranked shape
            // only, see `gpu_commit::launch_zc_ab_fold`): the first ranked
            // prove calibrates (a GPU prefix byte-compared against the CPU's
            // full sweep), later proves split the x_hi bands; any failure or
            // mismatch keeps the exact incumbent CPU completion.
            #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
            ...
            if let Some(job) = gpu_job {
                ...
                let cpu_partials = if calib {
                    Some(unsafe {
                        std::slice::from_raw_parts(partials_base.ptr(), hi_size)
                    })
                } else {
                    None
                };
                match crate::gpu_commit::zc_ab_wait(job, &eq_bot, &eq.hi, cpu_partials, hi_size)
                {
                    ...
                    crate::gpu_commit::ZcAbResult::Failed => {
                        // CPU-redo exactly the skipped prefix bands — slower,
                        // still exact (same byte-identical band arithmetic).
