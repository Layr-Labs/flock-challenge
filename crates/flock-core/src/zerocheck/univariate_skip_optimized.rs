...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
      // Traverse each complete fifteen-chunk group as three interleaved monotone
      // lanes: 0, 5, 10, 1, 6, 11, 2, 7, 12, 3, 8, 13, 4, 9, 14. This bounded
      // transpose is bijective; a shorter tail remains sequential and bounded.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                  let group_base = (visit / 15) * 15;
                  let group_visit = visit % 15;
                  let offset = if group_base + 14 < slab_len {
                      group_base + (group_visit % 3) * 5 + group_visit / 3
                  } else {
                      visit
                  };
                let x_outer = chunk_start + offset;
                  // SAFETY: each queue job is unique; the fifteen-chunk lane weave
                  // permutes complete groups, and a shorter tail is fixed.
