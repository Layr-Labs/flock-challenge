...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse sixteen-chunk windows through a coprime multiplicative permutation;
    // neighboring queue claims receive distinct phases without changing ownership.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Preserve Gray-coded four-band ownership while permuting windows by
            // multiplication by three modulo eight and a per-job phase.
            ...
            for burst_step in 0..N_BURSTS {
                let burst = (burst_step * 3 + job) % N_BURSTS;
                let burst_start = burst * BURST_CHUNKS;
                for band_step in 0..band_blocks {
                    let band = (band_step + job) % band_blocks;
                    let band_base = band * n_jobs;
                    let gray = job ^ (job >> 1);
                    let block = band_base + (gray + band * 257) % n_jobs;
                    if block >= n_blocks {
                        continue;
                    }
                    for offset in burst_start..(burst_start + BURST_CHUNKS).min(BLOCK_CHUNKS) {
