...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse each complete twenty-two-chunk group by interleaving its two
    // eleven-element halves: 0, 11, 1, 12, ... . This is a bijection; a
    // shorter tail remains sequential.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                let group_base = (visit / 22) * 22;
                let group_visit = visit % 22;
                let offset = if group_base + 21 < slab_len {
                    group_base + (group_visit % 2) * 11 + group_visit / 2
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; complete twenty-two-chunk
