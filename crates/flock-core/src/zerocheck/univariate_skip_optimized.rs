...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Interleave each queue claim's positional bands at chunk granularity. This
    // keeps ownership unchanged while preventing one claim from monopolizing a
    // single quartile for a full 128-chunk burst.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Preserve Gray-coded four-band ownership, visiting sixteen
            // contiguous chunks before rotating to the next positional band.
            ...
            for burst_start in (0..BLOCK_CHUNKS).step_by(BURST_CHUNKS) {
