...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse each complete nine-chunk group by taking the middle, then
    // alternating right and left: 4, 5, 3, 6, 2, 7, 1, 8, 0. This center-out
    // permutation is bijective; a shorter tail remains sequential and bounded.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                let group_base = (visit / 9) * 9;
                let group_visit = visit % 9;
                let offset = if group_base + 8 < slab_len {
                    group_base
                        + if group_visit == 0 {
                            4
                        } else if group_visit & 1 == 1 {
                            4 + group_visit.div_ceil(2)
                        } else {
                            4 - group_visit / 2
                        }
                } else {
                ...
                // SAFETY: each queue job is unique; the center-out mapping
                // permutes complete nonuples, and a shorter tail is fixed.
