...
/// they retain ordinary interleaved rows or block-split even rows and deltas.
...
impl Round1AbInner {
    ...
    /// Whether each packed 1 KiB A/B block was rewritten as 64 retained even
    /// rows followed by 64 adjacent-row XOR deltas while this transform was
    /// produced. The transform itself is unchanged; only the
    /// challenge-dependent round-two representation differs.
    #[inline]
...
/// Challenge-independent AB precompute which additionally rewrites every 1
/// KiB packed block as `[64 even rows][64 even XOR odd deltas]` after the block
/// has been fully consumed. The extra stores therefore hit the same
/// private-cache lines already read by the precompute, and round two can retain
/// the delta halves instead of serially writing a separate 512 MiB surface.
///
...
/// Challenge-independent AB precompute for inputs that the witness generator
/// already emitted as `[64 even rows][64 even XOR odd deltas]` per 1 KiB
/// block. The optimized round-one kernels still consume ordinary row order,
/// so each worker reconstructs one A/B block in private L1 scratch. Unlike
/// the post-precompute conversion path, this never dirties the resident packed
/// inputs and therefore adds no full-buffer writeback pass.
pub fn precompute_round1_ab_inner_packed_padded_from_pair_deltas(
    a_packed: &[u8],
    b_packed: &[u8],
    m: usize,
    k_skip: usize,
    ...
    precompute_round1_ab_inner_packed_padded_from_pair_deltas_flavor(
        a_packed,
        b_packed,
        m,
        k_skip,
...
fn form_outer_pair_deltas(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    const PAIRS_PER_OUTER: usize = OUTER_BYTES / 16;
    const HALF_BYTES: usize = OUTER_BYTES / 2;
    debug_assert_eq!(a_outer.len(), OUTER_BYTES);
    debug_assert_eq!(b_outer.len(), OUTER_BYTES);
    let mut a_split = [0u8; OUTER_BYTES];
    let mut b_split = [0u8; OUTER_BYTES];
    for local_pair in 0..PAIRS_PER_OUTER {
        let global_pair = x_outer * PAIRS_PER_OUTER + local_pair;
        let padded = pairs_per_block != 0 && (global_pair & pair_mask) >= useful_pairs;
        let src = local_pair * 16;
        let even_dst = local_pair * 8;
        let delta_dst = HALF_BYTES + even_dst;
        for lane in 0..8 {
            let a_even = a_outer[src + lane];
            let b_even = b_outer[src + lane];
            a_split[even_dst + lane] = a_even;
            b_split[even_dst + lane] = b_even;
            if !padded {
                a_split[delta_dst + lane] = a_even ^ a_outer[src + 8 + lane];
                b_split[delta_dst + lane] = b_even ^ b_outer[src + 8 + lane];
            }
        }
    }
    a_outer.copy_from_slice(&a_split);
    b_outer.copy_from_slice(&b_split);
}

#[inline]
fn reconstruct_outer_from_pair_deltas(split: &[u8], ordinary: &mut [u8]) {
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    const PAIRS_PER_OUTER: usize = OUTER_BYTES / 16;
    const HALF_BYTES: usize = OUTER_BYTES / 2;
    debug_assert_eq!(split.len(), OUTER_BYTES);
    debug_assert_eq!(ordinary.len(), OUTER_BYTES);
    for local_pair in 0..PAIRS_PER_OUTER {
        let even_src = local_pair * 8;
        let delta_src = HALF_BYTES + even_src;
        let dst = local_pair * 16;
        for lane in 0..8 {
            let even = split[even_src + lane];
            ordinary[dst + lane] = even;
            ordinary[dst + 8 + lane] = even ^ split[delta_src + lane];
        }
    }
}
...
fn precompute_round1_ab_inner_packed_padded_from_pair_deltas_flavor(
    a_packed: &[u8],
    b_packed: &[u8],
    m: usize,
    k_skip: usize,
    ...
    out_bytes
        .par_chunks_mut(OUTER_BYTES)
        .zip(a_packed.par_chunks(OUTER_BYTES))
        .zip(b_packed.par_chunks(OUTER_BYTES))
        .enumerate()
        .for_each_init(
            || {
                (
            ...
            |(a_col, b_col, a_ordinary, b_ordinary),
             (x_outer, ((out_outer, a_outer), b_outer))| {
                reconstruct_outer_from_pair_deltas(a_outer, a_ordinary);
                reconstruct_outer_from_pair_deltas(b_outer, b_ordinary);
                let within_hash_outer = x_outer & within_outer_mask;
                let n_b_med = b_med_counts[within_hash_outer] as usize;
                let mut tmp = [0u8; 64];
                for b_med in 0..n_b_med {
...
mod tests {
    ...
    /// A/B emitted in block-split pair-delta form must reconstruct the exact
    /// same challenge-independent round-one transform as ordinary row-major
    /// inputs. This pins the representation boundary independently of the
    /// round-two consumer oracle.
    #[test]
    fn precomputed_ab_from_pair_deltas_matches_ordinary() {
        for m in 13usize..=17 {
            let mut rng = Rng::new(0xAB5E_1000_u64.wrapping_add(m as u64));
            let a = pack_bits(&rng.bits(1 << m));
            let b = pack_bits(&rng.bits(1 << m));
            let inv_table = make_inv_table();
            let padding = PaddingSpec::dense(m);

            let ordinary = precompute_round1_ab_inner_packed_padded(
                &a, &b, m, K_SKIP, &inv_table, &padding,
            );
            let mut a_split = a.clone();
            let mut b_split = b.clone();
            form_in_place_pair_deltas_padded(&mut a_split, &mut b_split, K_SKIP, &padding);
            let split = precompute_round1_ab_inner_packed_padded_from_pair_deltas(
                &a_split,
                &b_split,
                m,
                K_SKIP,
            ...
            assert!(split.pair_deltas_in_place());
            assert_eq!(
                split.as_bytes(),
                ordinary.as_bytes(),
                "pair-delta round-one transform mismatch at m={m}"
            );
        }
    }
