...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete 65-chunk groups along the Chinese-remainder diagonal:
    // each visit selects its residues modulo 5 and 13, then maps that unique
    // residue pair into a 5-by-13 row-major slab. Incomplete tails stay sequential.
    const GROUP_CHUNKS: usize = 65;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    group_base + (group_visit % 5) * 13 + (group_visit % 13)
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique and CRT bijectivity for coprime
                // moduli 5 and 13 permutes 0..GROUP_CHUNKS exactly once.
