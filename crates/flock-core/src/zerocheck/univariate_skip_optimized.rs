...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse each complete thirty-one-chunk group in filtered base-3
    // digit-reversal order; a shorter tail remains sequential.
    const GROUP_CHUNKS: usize = 31;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    // Reverse four base-3 digits. Filtering the 81-point
                    // permutation to values below 31 remains a permutation.
                    ...
                    loop {
                        ...
                        for _ in 0..4 {
                            reversed = reversed * 3 + x % 3;
                            x /= 3;
                        }
                        if reversed < GROUP_CHUNKS {
                            if accepted == group_visit {
                                break group_base + reversed;
                            }
                            accepted += 1;
                        }
                        candidate += 1;
                ...
                // SAFETY: each queue job is unique; complete thirty-one-chunk
                // groups are filtered digit-reversal bijections, and a shorter tail is fixed.
