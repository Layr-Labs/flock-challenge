...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Swap adjacent visits within each slab. This exposes the second chunk of
    // every cache-neighbor pair first while retaining near-sequential locality.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                let offset = if visit + 1 < slab_len {
                    visit ^ 1
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique and adjacent swaps permute
                // offsets within its disjoint slab; an odd tail stays fixed.
