...
pub fn round1_shift_reduce_extract_c_packed_padded_with_precomputed_ab(
    ...
    // the closure below writes partials[x_hi] unconditionally (full-tuple
    // assignment, not `+=`) before the fold-reduce past the queue join reads
    // it, so the zero-fill was dead work.
    let mut partials: Vec<([F128; ELL], [F128; ELL], [F128; ELL])> =
        crate::alloc_uninit_vec(hi_size);
