...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Use 768 queue claims with an explicitly bijective affine ownership map.
    // The multiplier 5 is coprime to 768, so every complete positional band
    // assigns each block residue to exactly one queue claim.
    const RANKED_TAPERED_JOBS: usize = 768;
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Preserve sixteen-chunk phased traversal while replacing the unsafe
            // Gray-code reduction with affine ownership modulo 768.
            ...
            for burst_step in 0..N_BURSTS {
                let burst_start = (burst_step * BURST_CHUNKS + job * 5) % BLOCK_CHUNKS;
                for band_step in 0..band_blocks {
                    let band = (band_step + job) % band_blocks;
                    let band_base = band * n_jobs;
                    let owner = (job * 5 + band * 257) % n_jobs;
