...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse each complete twenty-five-chunk group with the coprime stride-9
    // cycle. Since gcd(9, 25) = 1, this visits every offset exactly once;
    // a shorter tail remains sequential.
    const GROUP_CHUNKS: usize = 25;
    const GROUP_STRIDE: usize = 9;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    group_base + (group_visit * GROUP_STRIDE) % GROUP_CHUNKS
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; complete twenty-five-chunk
                // groups are permuted bijectively, and a shorter tail is fixed.
