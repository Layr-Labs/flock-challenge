...
/// Assign ranked AB work as 1,024 cyclic stripes. Unlike every prior contiguous
/// slab geometry, job `j` owns chunks `j, j + n_jobs, ...`, so every worker
/// samples the full positional range early while ownership remains disjoint.
#[inline]
...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Cyclic ownership is complete and disjoint because every chunk has
            // exactly one residue modulo `n_jobs`.
            for x_outer in (job..n_chunks).step_by(n_jobs) {
                // SAFETY: this queue job exclusively owns residue `job`.
