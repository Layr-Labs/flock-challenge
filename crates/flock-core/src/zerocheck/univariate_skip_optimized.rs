...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete fifty-two-chunk groups in filtered two-digit base-8
    // reversal order; values 52..=63 are skipped and a shorter tail stays sequential.
    const GROUP_CHUNKS: usize = 52;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    let mut rank = group_visit;
                    let mut candidate = 0;
                    for value in 0..64 {
                        let reversed = (value % 8) * 8 + value / 8;
