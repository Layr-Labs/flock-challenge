...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete fifty-six-chunk groups in a perfect out-shuffle:
    // all even offsets followed by all odd offsets. A shorter tail stays sequential.
    const GROUP_CHUNKS: usize = 56;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    // 0,2,...,54,1,3,...,55: two monotone parity lanes.
                    group_base + (group_visit % 28) * 2 + group_visit / 28
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; the perfect out-shuffle visits
                // each complete group's offset once, and shorter tails are fixed.
