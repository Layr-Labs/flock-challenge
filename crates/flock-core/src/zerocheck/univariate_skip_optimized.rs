...
pub fn precompute_round1_ab_inner_packed_padded(
    ...
    // begins after this window). Drain outer positions through the
    // cooperative hetero queue so the four E-cores contribute; the
    // cooperative form submits one rayon task per claim so the concurrent
    // commit branch keeps its current share of the main pool. Restores
    // promoted submission `41df8458` (+3.02%), silently dropped by later
    // frontier rebases. `FLOCK_NO_AB_EPOOL` is an exact same-binary control.
