...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete 72-chunk groups as four contiguous 18-chunk quartiles
    // in the phase order 1, 3, 0, 2. This exposes broad positional skew early
    // while preserving sequential access inside each quartile; tails stay sequential.
    const GROUP_CHUNKS: usize = 72;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    const QUARTER: usize = GROUP_CHUNKS / 4;
                    const PHASE: [usize; 4] = [1, 3, 0, 2];
                    group_base + PHASE[group_visit / QUARTER] * QUARTER + group_visit % QUARTER
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; every ten-chunk row is
                // traversed bijectively, with odd rows reversed.
