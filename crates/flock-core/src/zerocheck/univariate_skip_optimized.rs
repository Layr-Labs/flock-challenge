...
/// Assign ranked AB work as 1,024 rotated four-band queues. Each job drains one
/// 128-chunk block from every input quartile, with a coprime 257-block rotation
/// between bands to decorrelate equal-offset positional costs while retaining
/// sequential runs inside each block.
#[inline]
...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // The ranked input has 4,096 128-chunk blocks. Each band applies
            // a coprime rotation, preserving one owner per block while pairing
            // different within-quartile offsets in every queue claim.
            const BLOCK_CHUNKS: usize = 128;
            const BAND_ROTATION: usize = 257;
            let n_blocks = n_chunks.div_ceil(BLOCK_CHUNKS);
            let band_blocks = n_blocks.div_ceil(n_jobs);
            for band in 0..band_blocks {
                let band_base = band * n_jobs;
                let block = band_base + (job + band * BAND_ROTATION) % n_jobs;
                if block >= n_blocks {
                    continue;
                }
