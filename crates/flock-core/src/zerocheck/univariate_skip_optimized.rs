...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete forty-six-chunk groups by alternating the two ends of
    // each adjacent quartet: 0,3,1,2. A shorter tail stays sequential.
    const GROUP_CHUNKS: usize = 46;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    let quartet_base = group_visit - (group_visit % 4);
                    let within = group_visit % 4;
                    // The final group has two elements, so preserve their order.
                    group_base
                        + if quartet_base + 4 <= GROUP_CHUNKS {
                            quartet_base + [0, 3, 1, 2][within]
                        } else {
                            group_visit
                        }
                } else {
                ...
                // SAFETY: each queue job is unique; complete forty-six-chunk
                // groups permute disjoint quartets, and shorter tails are fixed.
