...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Drain exclusively owned 120-chunk regions contiguously. Fifteen
    // eight-chunk groups alternate block starts between the two half-line
    // alignment classes while retaining a long sequential locality window.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Traverse one exclusively owned 120-chunk block sequentially.
            const BLOCK_CHUNKS: usize = 120;
