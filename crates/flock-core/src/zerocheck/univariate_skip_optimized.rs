...
fn precompute_round1_ab_inner_packed_padded_with_flavor(
    ...
    if zc_ab_pre_hetero_enabled() {
        ...
        // Diagnostic override: `FLOCK_GPU_AB_FORCE=1` opens the const-b GPU
        // launch gate even when the ranked 10+4 shape is not detected
        // (local bring-up on machines with fewer E-cores). Never set on
        // ranked runners; correctness gates (verified a-only bits, mask==1)
        // still apply.
        let gpu_ab_force = std::env::var_os("FLOCK_GPU_AB_FORCE").as_deref()
            == Some(std::ffi::OsStr::new("1"));
        if std::env::var_os("FLOCK_GPU_AB_DEBUG").is_some() {
            eprintln!(
                "ab-gpu: ranked={ranked_fast_policy} mask={within_outer_mask} a_only={} prepared={} m={m} k_skip={k_skip} n_chunks={n_chunks} k_log={} useful={} main={} helper={}",
                kernels::static_b_a_only_specialized(static_b_context),
                kernels::static_b_context_is_prepared(static_b_context),
                padding.k_log,
                padding.useful_bits_per_block,
                rayon::current_num_threads(),
        ...
        // GPU const-b offload for the SELECTED generic bstatic blocks:
        // launch before the queue drains so the GPU computes the 64-byte
        // rows in place while the CPU drains the remaining blocks. The
        // selection defaults to the highest-cost blocks (most varying B
        // bytes) and can be overridden with FLOCK_GPU_AB_BLOCKS="w.b,...".
        // The ranked measurement of the a-only variant was net-negative
        // (53e6714), so this stays opt-in (FLOCK_GPU_AB_GENERIC=1) until
        // it proves itself. Anything else keeps the incumbent CPU path; a
        // failed join falls back to a CPU recompute with the exact
        // dispatcher kernel, so a GPU fault can never corrupt the proof.
        let gpu_sel: Vec<(u8, u8)> = if within_outer_mask == 1
            && (ranked_fast_policy || gpu_ab_force)
            && crate::gpu_commit::gpu_ab_generic_enabled()
        {
            gpu_ab_block_selection()
        } else {
            Vec::new()
        };
        let ab_gpu_job = if !gpu_sel.is_empty() {
            let (tp, th) = inv_table.gpu_images();
            match (tp, th) {
                (Some(plain), Some(hs)) => crate::gpu_commit::launch_ab_const_b(
                    a_packed,
                    b_packed,
                    plain,
                    hs,
        ...
        let gpu_sel_mask: u32 = gpu_sel.iter().fold(0u32, |m, &(w, b)| {
            m | (1u32 << (w as u32 * 16 + b as u32))
        });
        ...
        let gpu_ran = ab_gpu_job.is_some();
        if let Some(job) = ab_gpu_job {
            if crate::gpu_commit::finish_ab_const_b(job).is_err() {
                // GPU join failed: recompute the selected blocks' rows with
                // the exact dispatcher path before any consumer reads the
                // matrix (correctness fallback, not a fast path).
                gpu_ab_recompute_selected(
                    a_packed,
                    b_packed,
                    inv_table,
                    within_outer_mask,
        ...
        // Diagnostic: verify the selected rows in the matrix against the
        // CPU dispatcher right after the join (integration check; ranked
        // runners run with a cleared environment so this never fires there).
        if std::env::var_os("FLOCK_GPU_AB_DEBUG").is_some() && gpu_ran {
            let mut expect = vec![0u8; n_chunks * 1024];
            gpu_ab_recompute_selected(
                a_packed,
                b_packed,
                inv_table,
            ...
            let mut bad = 0usize;
            for &(w, b_med) in &gpu_sel {
                for t in 0..n_chunks / 2 {
                    let x_outer = 2 * t + w as usize;
                    let off = x_outer * 1024 + b_med as usize * 64;
                    let got = &out_bytes[off..off + 64];
                    let want = &expect[off..off + 64];
                    if got != want {
                        bad += 1;
                        if bad <= 3 {
                            eprintln!("AB-GPU INTEGRATION MISMATCH w={w} b={b_med} t={t}");
                        }
                    }
                }
            }
            eprintln!("AB-GPU integration check: {bad} mismatched rows");
        }
...
/// GPU const-b block selection: `FLOCK_GPU_AB_BLOCKS="w.b,w.b,..."` overrides;
/// the default is the six offloadable blocks with the most varying B bytes
/// (the most expensive CPU rows). Specialized blocks (0, 1, 2, 29, 30) and
/// the dead window-1 b_med 15 are excluded.
fn gpu_ab_block_selection() -> Vec<(u8, u8)> {
    if let Ok(spec) = std::env::var("FLOCK_GPU_AB_BLOCKS") {
        let mut out = Vec::new();
        for part in spec.split(',') {
            let mut it = part.trim().split('.');
            if let (Some(w), Some(b)) = (it.next(), it.next()) {
                if let (Ok(w), Ok(b)) = (w.parse::<u8>(), b.parse::<u8>()) {
                    if w <= 1 && b < 15 {
                        out.push((w, b));
                    }
                }
    ...
    let costs = kernels::bstatic_block_varying_bytes();
    let mut ranked: Vec<(usize, u16)> = costs
        .iter()
        .enumerate()
        .filter(|(blk, _)| !matches!(blk, 0 | 1 | 2 | 29 | 30))
        .map(|(blk, c)| (blk, *c))
        .collect();
    ...
    ranked
        .iter()
        .take(6)
        .map(|&(blk, _)| ((blk / 16) as u8, (blk % 16) as u8))
        .collect()
}

/// CPU recompute of the selected const-b blocks' rows through the exact
/// dispatcher path (the join-failure fallback and the diagnostic
/// reference). `out_bytes` receives the 64-byte rows at their matrix
/// offsets `((2t + w)*1024 + b_med*64)`.
#[allow(clippy::too_many_arguments)]
fn gpu_ab_recompute_selected(
    a_packed: &[u8],
    b_packed: &[u8],
    inv_table: &InvNttTableByteSingleGf8,
    ...
    let mut row = [0u8; 64];
    for &(w, b_med) in sel {
        let (w, b_med) = (w as usize, b_med as usize);
        for t in 0..n_w0 {
            let x_outer = 2 * t + w;
            ...
            let off = x_outer * OUTER_BYTES + b_med as usize * 64;
            out_bytes[off..off + 64].copy_from_slice(&row);
        }
    }
}
...
fn precompute_ab_one_chunk<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    let mut tmp = [0u8; 64];
    for b_med in 0..n_b_med {
        // GPU const-b offload: rows of the selected blocks are computed by
        // the GPU in place (the join runs before any consumer reads the
        // matrix), so the CPU skips them.
        if ((gpu_sel_mask >> (within_hash_outer * 16 + b_med)) & 1) == 1 {
            continue;
        }
...
mod tests {
    ...
    /// Full-scale GPU const-b oracle: the generic kernel (selected bstatic
    /// blocks, A+B gathers + GF(2^8) mul) vs the CPU dispatcher at the
    /// ranked row count (n_w0 = 262,144) with real-sized buffers — catches
    /// t-dependent addressing and mul-table bugs the small oracle cannot.
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    #[test]
    fn ab_const_b_gpu_matches_cpu_kernel_full_scale() {
        use std::alloc::{alloc_zeroed, dealloc, Layout};
        const N_W0: usize = 262_144;
        const N_CHUNKS: usize = 524_288;
        const A_LEN: usize = 536_870_912; // 512 MiB, 16 KiB multiple
        const OUT_LEN: usize = 536_870_912;
        const SEL: [(u8, u8); 2] = [(0, 3), (1, 4)];
        let a_layout = Layout::from_size_align(A_LEN, 16384).unwrap();
        let out_layout = Layout::from_size_align(OUT_LEN, 16384).unwrap();
        let a_ptr = unsafe { alloc_zeroed(a_layout) };
        let b_ptr = unsafe { alloc_zeroed(a_layout) };
        let out_cpu_ptr = unsafe { alloc_zeroed(out_layout) };
        let out_gpu_ptr = unsafe { alloc_zeroed(out_layout) };
        let a_packed: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(a_ptr, A_LEN) };
        let b_packed: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(b_ptr, A_LEN) };
        let out_cpu: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(out_cpu_ptr, OUT_LEN) };
        let out_gpu: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(out_gpu_ptr, OUT_LEN) };
        // Deterministic pseudo-random witness covering the whole span,
        // including the largest t offsets.
        let mut rng = Rng::new(0x5EED_CAFE_F00D_4242);
        let mut i = 0usize;
        while i + 8 <= A_LEN {
            let w = rng.next_u64();
            a_packed[i..i + 8].copy_from_slice(&w.to_le_bytes());
            b_packed[i..i + 8].copy_from_slice(&rng.next_u64().to_le_bytes());
            i += 8;
        }
        let inv = make_inv_table();
        let (plain, hs) = match inv.gpu_images() {
            (Some(p), Some(h)) => (p, h),
            _ => panic!("gpu table images unavailable at ell=64"),
        };
        let ctx = kernels::prepare_static_b_context(&inv, true);
        gpu_ab_recompute_selected(
            a_packed,
            b_packed,
            &inv,
        ...
        let job = crate::gpu_commit::launch_ab_const_b(
            a_packed,
            b_packed,
            plain,
            hs,
        ...
        .expect("GPU const-b launch (Metal device required)");
        crate::gpu_commit::finish_ab_const_b(job).expect("GPU const-b join");
        let mut bad = 0usize;
        for &(w, b_med) in &SEL {
            for t in 0..N_W0 {
                let off = (2 * t + w as usize) * 1024 + b_med as usize * 64;
                let g = &out_gpu[off..off + 64];
                let c = &out_cpu[off..off + 64];
                if g != c {
                    bad += 1;
                    if bad <= 3 {
                        eprintln!("MISMATCH row w={w} b={b_med} t={t}");
                    }
                }
            }
        }
        unsafe {
            dealloc(a_ptr, a_layout);
            dealloc(b_ptr, a_layout);
            dealloc(out_cpu_ptr, out_layout);
            dealloc(out_gpu_ptr, out_layout);
        }
        assert_eq!(bad, 0, "{bad} rows differ at full scale");
    }

    /// GPU const-b kernel oracle: the generic MSL kernel must reproduce the
    /// CPU dispatcher's output byte-for-byte on a synthetic witness at the
    /// production table shape (ell = 64), for selected generic blocks.
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    #[test]
    fn ab_const_b_gpu_matches_cpu_kernel() {
        use std::alloc::{alloc_zeroed, dealloc, Layout};
        const N_W0: usize = 128;
        const N_CHUNKS: usize = 256;
        const A_LEN: usize = 262_144;
        const OUT_LEN: usize = 262_144;
        const SEL: [(u8, u8); 2] = [(0, 3), (1, 4)];
        let a_layout = Layout::from_size_align(A_LEN, 16384).unwrap();
        let out_layout = Layout::from_size_align(OUT_LEN, 16384).unwrap();
        let a_ptr = unsafe { alloc_zeroed(a_layout) };
        ...
        let out_cpu: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(out_cpu_ptr, OUT_LEN) };
        let out_gpu: &mut [u8] = unsafe { std::slice::from_raw_parts_mut(out_gpu_ptr, OUT_LEN) };
        let mut rng = Rng::new(0xABCD_1234_5678_9ABC);
        for i in 0..A_LEN {
            a_packed[i] = (rng.next_u64() >> 32) as u8;
            b_packed[i] = (rng.next_u64() >> 32) as u8;
        }
        let inv = make_inv_table();
        let (plain, hs) = match inv.gpu_images() {
            (Some(p), Some(h)) => (p, h),
        ...
        // Canary: distinguish "kernel never wrote" (stays 0xFF) from
        // "kernel ran with zero inputs" (becomes 0x00).
        out_gpu.fill(0xFF);
        let job = crate::gpu_commit::launch_ab_const_b(
            a_packed,
            b_packed,
        ...
        .expect("GPU const-b launch (Metal device required)");
        crate::gpu_commit::finish_ab_const_b(job).expect("GPU const-b join");
        let first = out_gpu[(0 * 1024) + 3 * 64];
        assert!(
            first != 0xFF,
            "GPU kernel never wrote: out_gpu still 0xFF canary"
        );
        for &(w, b_med) in &SEL {
            for t in 0..N_W0 {
                let off = (2 * t + w as usize) * 1024 + b_med as usize * 64;
                ...
                for w in 0..2usize {
                    ...
                    for b_med in 0..n_b_med {
                        ...
                        for variant in 0..10usize {
                            ...
                            let mut slow = [0u8; 64];
                            assert!(
                                kernels::aarch64::shift_reduce_inner_ab_bstatic::<false, false, true>(
                            ...
                            let mut fast = [0u8; 64];
                            assert!(
                                kernels::aarch64::shift_reduce_inner_ab_bstatic::<true, false, true>(
                                    &a_packed, &b, &table, 0, b_med, w, context, &mut fast, true,
                                ),
                                "arm (w={w}, b_med={b_med}) must be live"
                            );
                            let mut incumbent = [0u8; 64];
                            assert!(
                                kernels::aarch64::shift_reduce_inner_ab_bstatic::<true, false, false>(
                                    &a_packed,
                                    &b,
                                    &table,
                                    0,
                                ...
                                "incumbent arm (w={w}, b_med={b_med}) must be live"
                            );
                            assert_eq!(
                                incumbent, want,
                                "partial-load control drift (w={w}, b_med={b_med}, variant={variant})"
                            );
                            assert_eq!(
                                slow, want,
                            ...
                            if variant == 1 {
                                let mut pure = [0u8; 64];
                                assert!(
                                    kernels::aarch64::shift_reduce_inner_ab_bstatic::<true, true, true>(
