...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete fifty-one-chunk groups through three residue lanes,
    // reversing the middle lane. This is bijective; a shorter tail stays sequential.
    const GROUP_CHUNKS: usize = 51;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    let lane = group_visit % 3;
                    let lane_visit = group_visit / 3;
                    let lane_visit = if lane == 1 { 16 - lane_visit } else { lane_visit };
                    group_base + lane + 3 * lane_visit
                } else {
                    visit
                };
                    group_base + wave_offset
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; complete fifty-one-chunk
                // groups are traversed bijectively, and shorter tails are fixed.
