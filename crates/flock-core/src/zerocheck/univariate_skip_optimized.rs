...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
      // Drain each exclusively owned 136-chunk region contiguously, reversing odd
      // physical blocks. This preserves long sequential runs while alternating the
      // direction of adjacent memory streams without adding burst-level rotations.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
              // Traverse one exclusively owned 136-chunk block in parity-selected direction.
            ...
            for burst_step in 0..N_BURSTS {
                ...
                for band_step in 0..band_blocks {
                  ...
                  for step in 0..BLOCK_CHUNKS {
                      let offset = if block & 1 == 0 { step } else { BLOCK_CHUNKS - 1 - step };
