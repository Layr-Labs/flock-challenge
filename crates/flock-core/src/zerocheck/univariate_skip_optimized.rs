...
/// Compile-time default for the QS5 AB-precompute drain. The candidate
/// deliberately keeps this phase on the performance-core pool: the shared
/// E-core queue adds cross-cluster ownership and cache/coherency traffic to
/// the dominant write-once transform. Set exactly `FLOCK_ZC_AB_PRE_HETERO=1`
/// to re-enable the helper-pool arm as a same-binary control.
pub const ZC_AB_PRE_HETERO_DEFAULT: bool = false;
...
>(
    ...
    // Keep the dense shape separate from padded boundary windows.  The const
    // arm below exposes a fixed 16-row trip count to LLVM and removes the
    // per-row bound comparison from the overwhelmingly common path.
    let full_rows = b_med_counts[x_outer & within_outer_mask] as usize == (1 << N_MEDIUM);
    if full_rows {
        precompute_ab_one_chunk_impl::<FAST_POLICY, FORCE_DIRECT, STATIC_LAYOUT, COMPACT, true>(
            a_packed,
            b_packed,
            inv_table,
            within_outer_mask,
        ...
        precompute_ab_one_chunk_impl::<FAST_POLICY, FORCE_DIRECT, STATIC_LAYOUT, COMPACT, false>(
            a_packed,
            b_packed,
            inv_table,
            within_outer_mask,
...
#[inline(always)]
#[allow(clippy::too_many_arguments)]
fn precompute_ab_one_chunk_impl<
    const FAST_POLICY: u8,
    const FORCE_DIRECT: bool,
    const STATIC_LAYOUT: bool,
    const COMPACT: bool,
    const FULL_ROWS: bool,
>(
    a_packed: &[u8],
    b_packed: &[u8],
    inv_table: &InvNttTableByteSingleGf8,
    ...
    // The store flavor is invariant across every b_med row in this chunk.
    // Hoist the stack-bounce predicate out of the row loop so the ranked
    // direct-store specialization has one compile-time-false branch rather
    // than re-evaluating the same policy on every transformed row.
    let stack_bounce = nt && !direct;
    let mut tmp = [0u8; 64];
    let out_ptr = out_outer.as_mut_ptr();
    let row_count = if FULL_ROWS { 1 << N_MEDIUM } else { n_b_med };
    for b_med in 0..row_count {
        // The row address is a fixed 64-byte stride. Keep the direct arm on a
        // raw pointer so the hot loop does not rebuild a range and perform a
        // `try_into` conversion for every transformed row. The stack-bounce
        // arm still reuses one temporary, then commits that row to the same
        // disjoint destination.
        let dst: &mut [u8; 64] = if stack_bounce {
            &mut tmp
        } else {
            // SAFETY: `b_med < n_b_med ≤ OUTER_BYTES / 64`; every row starts
            // at a distinct in-bounds 64-byte region of `out_outer`, and the
            // kernel call completes before the next row is borrowed.
            unsafe { &mut *(out_ptr.add(b_med * 64) as *mut [u8; 64]) }
        };
        ...
        if stack_bounce {
            // SAFETY: `b_med < n_b_med ≤ OUTER_BYTES / 64`, so the 64
            // destination bytes are in-bounds of `out_outer`.
            unsafe { store_nt_64(tmp.as_ptr(), out_ptr.add(b_med * 64)) };
        }
    }
...
fn process_one_x_hi_with_precomputed_ab(
    ...
    let n_lo = n_lo_and_inner - N_INNER;
    let x_outer_base = x_hi << n_lo;
    let chunk_base_start = (x_hi << n_lo_and_inner) * N_CHUNKS;
    let chunk_step = N_CHUNKS << N_INNER;
    // Cache the fixed protocol tables as raw bases. This removes slice-index
    // bounds checks from the challenge-weighted window drain while preserving
    // the same masked/count-derived indices.
    let b_med_counts_ptr = b_med_counts.as_ptr();
    let eq_lo_scaled_ptr = eq_lo_scaled.as_ptr();
    // C rows follow the same affine order as `chunk_byte_base`. Keep a base
    // pointer so the hot C drain does not rebuild a bounded slice per window.
    let c_packed_ptr = c_packed.as_ptr();
    ...
    if split_ab_c {
        ...
        let mut chunk_byte_base = chunk_base_start;
        let mut mask_table_base = 0usize;
        for x_outer_lo in 0..big_lo_size {
            let x_outer = x_outer_base | x_outer_lo;
            let within_hash_outer = x_outer & within_outer_mask;
            // SAFETY: `within_hash_outer` is masked by the protocol domain,
            // exactly matching the count-table construction.
            let n_b_med = unsafe { *b_med_counts_ptr.add(within_hash_outer) as usize };
            if n_b_med == 0 {
                chunk_byte_base += chunk_step;
                mask_table_base += C_MASK_TABLE_STRIDE;
                continue;
            }
            ...
            kernels::accumulate_convert_ab(
                ...
                n_b_med,
                convert,
                // SAFETY: `x_outer_lo` is the loop index into the equality table.
                unsafe { *eq_lo_scaled_ptr.add(x_outer_lo) },
                &mut state.partial_ab,
            );
            chunk_byte_base += chunk_step;
            mask_table_base += C_MASK_TABLE_STRIDE;
        ...
        for x_outer_lo in 0..big_lo_size {
            ...
            let c_tables =
                &mask_tables[mask_table_base..mask_table_base + C_MASK_TABLE_STRIDE];
            // SAFETY: `chunk_byte_base` follows the original checked affine
            // layout, and each live window consumes sixteen 64-byte rows.
            let c_rows: &[u8; (1 << N_MEDIUM) * 64] = unsafe {
                &*(c_packed_ptr.add(chunk_byte_base)
                    as *const [u8; (1 << N_MEDIUM) * 64])
            };
        ...
        for x_outer_lo in 0..big_lo_size {
            ...
            // SAFETY: `x_outer_lo < big_lo_size`, so this is within the
            // challenge-derived equality table.
            let eq_lo_val = unsafe { *eq_lo_scaled_ptr.add(x_outer_lo) };
            let c_tables =
                &mask_tables[mask_table_base..mask_table_base + C_MASK_TABLE_STRIDE];
            // The C side reads the packed witness directly: the fused mask kernel
            // subsumes the per-`b_med` `bit_transpose_64bytes` this path used to run.
