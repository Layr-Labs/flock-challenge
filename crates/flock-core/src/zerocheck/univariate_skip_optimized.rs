...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Drain each exclusively owned 224-chunk region contiguously. Seven cache lines
    // avoid the tested 160/192/256 geometries while retaining long sequential runs
    // and exclusive affine block ownership.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Traverse one exclusively owned 224-chunk block contiguously.
            const BLOCK_CHUNKS: usize = 224;
