...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
      // Traverse complete sixty-chunk groups by six interleaved deciles.
      // Each lane is monotone: 0,10,...,50, then 1,11,...,51, etc.
      const GROUP_CHUNKS: usize = 60;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    group_base + (group_visit % 6) * 10 + group_visit / 6
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; filtered radix reversal
                // visits every offset in 0..58 exactly once.
