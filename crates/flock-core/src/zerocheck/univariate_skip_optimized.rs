...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Use 960 queue claims: a previously untested near-1,024 dispatch geometry.
    // The multiplier 7 is coprime to 960, so every complete positional band
    // assigns each block residue to exactly one claim.
    const RANKED_TAPERED_JOBS: usize = 960;
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            // specialized for the 960-claim geometry.
            ...
            for burst_step in 0..N_BURSTS {
                ...
                for band_step in 0..band_blocks {
                    ...
                    let owner = (job * 7 + band * 193) % n_jobs;
