...
enum HeteroDrain {
    ...
    Hetero,
    /// First call in the process: time both drains over the same buffer and
    /// publish the verdict.
    Calibrate,
}
...
const LATCH_CALIBRATING: usize = 3;
...
/// Test/trace hooks.
pub(crate) fn ab_hetero_latch_state() -> usize {
    AB_HETERO_LATCH.load(std::sync::atomic::Ordering::Relaxed)
}
#[cfg(test)]
...
/// Decide this call's drain: env override wins; then the latched verdict;
/// the first undecided call with a helper pool present calibrates both
/// drains over the identical buffer and publishes the winner.
fn ab_drain_decide() -> HeteroDrain {
    use std::sync::atomic::Ordering;
    let drain = ab_hetero_drain();
    if drain == HeteroDrain::Main
        ...
        && AB_HETERO_LATCH
            ...
            .is_ok()
    {
        return HeteroDrain::Calibrate;
    }
    drain
}

/// Run the chosen drain(s) over `out_base[0..n_outers*OUTER_BYTES]`.
/// Calibration times both drains back to back over the same buffer; the
/// second overwrites the first with bit-identical bytes, so the storage is
/// correct whichever wins.
    ...
    F: Fn(&mut ([F8; ELL], [F8; ELL]), usize, &mut [u8]) + Sync,
{
    ...
    match drain {
        ...
        HeteroDrain::Calibrate => {
            let t0 = std::time::Instant::now();
            drain_main();
            let t_main = t0.elapsed();
            let t1 = std::time::Instant::now();
            drain_hetero();
            let t_hetero = t1.elapsed();
            // Hetero wins only with an 8% margin; near-equal keeps the
            // incumbent.
            let on = t_hetero.as_secs_f64() * 1.08 < t_main.as_secs_f64();
