...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // A 96-chunk tile preserves cache-line alignment while breaking the power-of-two
    // scheduling period; it trades some tail granularity for longer local runs.
    const CHUNKS_PER_TILE: usize = 96;
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Sequential dispatch avoids permutation overhead; block-cyclic tiles still
            // assign every chunk to exactly one queue job.
            let mapped_job = job;
            let tile_stride = n_jobs * CHUNKS_PER_TILE;
            for tile_base in
                ((mapped_job * CHUNKS_PER_TILE)..n_chunks).step_by(tile_stride)
            {
                for x_outer in tile_base..(tile_base + CHUNKS_PER_TILE).min(n_chunks) {
