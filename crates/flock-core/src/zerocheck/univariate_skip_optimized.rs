...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Claim contiguous slabs from the high end of the domain first. Each slab
    // still traverses forward for locality, while queue order exposes a region
    // that the prior low-to-high contiguous scheduler reached only at the tail.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            let mapped_job = n_jobs - 1 - job;
            let chunk_start = mapped_job * chunks_per_job;
            let chunk_end = (chunk_start + chunks_per_job).min(n_chunks);
            for x_outer in chunk_start..chunk_end {
