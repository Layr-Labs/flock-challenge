...
/// Which pool topology drains the round-1 AB precompute outers. Diagnostic
/// only: the E-core drain engages solely under `FLOCK_ZC_AB_HETERO=1`
/// (`FLOCK_NO_ZC_AB_HETERO=1` forces the incumbent). Automatic engagement was
/// removed after the runner proved that drain-wall and join-wall calibration
/// both miss cross-resource bandwidth contention on hosts where the GPU
/// commit dominates the join; the default is therefore the exact incumbent.
