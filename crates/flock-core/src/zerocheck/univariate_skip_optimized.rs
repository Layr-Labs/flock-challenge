...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
      // Traverse each complete thirty-six-chunk group by alternating-direction
      // columns of a 6x6 row-major grid. This exposes all six regions early
      // while avoiding a full-width wrap between adjacent columns; a shorter
      // tail remains sequential.
      const GROUP_CHUNKS: usize = 36;
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                ...
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                      // 0,6,...,30,31,25,...,1,2,8,...: serpentine columns.
                      let column = group_visit / 6;
                      let row_visit = group_visit % 6;
                      let row = if column & 1 == 0 {
                          row_visit
                      } else {
                          5 - row_visit
                      };
                      group_base + 6 * row + column
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                  // SAFETY: each queue job is unique; complete thirty-six-chunk
                  // groups use a 6x6 serpentine-column bijection, and a shorter
                  // tail is fixed.
