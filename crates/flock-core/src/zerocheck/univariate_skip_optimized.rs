...
/// Assign ranked AB work as 1,024 Gray-coded four-band queues. Each job
/// drains one 128-chunk block from every input quartile; Gray coding makes
/// consecutive queue claims change one index bit while retaining a bijection
/// and sequential runs inside every owned block.
#[inline]
...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Drain each exclusively owned 136-chunk region as a rotated permutation
    // of eight 17-chunk tiles. The prime-sized runs walk cache-line alignment
    // classes instead of repeatedly ending on the same 16-chunk boundaries.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        || ([F8::ZERO; ELL], [F8::ZERO; ELL]),
        |(a_col, b_col), job| {
            // Traverse one exclusively owned 136-chunk block in prime-sized tiles.
            const BLOCK_CHUNKS: usize = 136;
            const BURST_CHUNKS: usize = BLOCK_CHUNKS;
            const N_BURSTS: usize = 1;
            let n_blocks = n_chunks.div_ceil(BLOCK_CHUNKS);
            let band_blocks = n_blocks.div_ceil(n_jobs);
            for burst_step in 0..N_BURSTS {
                let burst_start = 0;
                for band_step in 0..band_blocks {
                    let band = (band_step + job) % band_blocks;
                    let band_base = band * n_jobs;
                    let owner = (job * 5 + band * 513) % n_jobs;
                    let block = band_base + owner;
                    if block >= n_blocks {
                        continue;
                    }
                    const TILE_CHUNKS: usize = 17;
                    const N_TILES: usize = BLOCK_CHUNKS / TILE_CHUNKS;
                    for tile_rank in 0..N_TILES {
                        // Rotation is bijective, and 8 * 17 exactly covers the block.
                        let tile = (tile_rank + block) % N_TILES;
                        let tile_start = tile * TILE_CHUNKS;
                        let tile_len = TILE_CHUNKS;
                        for within in 0..tile_len {
                            let offset = tile_start + within;
                            let x_outer = block * BLOCK_CHUNKS + offset;
                            if x_outer >= n_chunks {
                                continue;
                            }
