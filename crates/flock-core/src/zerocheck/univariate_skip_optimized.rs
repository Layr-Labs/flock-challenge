...
/// exactly `FLOCK_NT_LOADS=1` to opt in; default is the incumbent plain cached loads
...
/// +0.84% p10 — the 128 B/row stack bounce is a prime suspect together with
/// the pad-v4 occupancy probe). Set `FLOCK_NT_LOADS=1` (exact string) to opt
/// in as a same-binary A/B on runner-matched hardware.
pub const ENV_NT_LOADS: &str = "FLOCK_NT_LOADS";

pub fn nt_loads_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        std::env::var_os(ENV_NT_LOADS).as_deref() == Some(std::ffi::OsStr::new("1"))
    })
}
