...
fn precompute_ab_hetero<const FAST_POLICY: u8, const FORCE_DIRECT: bool>(
    ...
    const OUTER_BYTES: usize = (1 << N_MEDIUM) * 64;
    // Traverse complete forty-two-chunk groups as six seven-chunk rows,
    // reversing every odd row: 0..6,13..7,14..20,27..21,... . This keeps
    // every inner run contiguous while changing boundary direction; a shorter
    // tail remains sequential.
    const GROUP_CHUNKS: usize = 42;
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        |(a_col, b_col), job| {
            ...
            for visit in 0..slab_len {
                let group_base = (visit / GROUP_CHUNKS) * GROUP_CHUNKS;
                let group_visit = visit % GROUP_CHUNKS;
                let offset = if group_base + GROUP_CHUNKS <= slab_len {
                    // Six contiguous seven-chunk rows, alternating direction.
                    let row = group_visit / 7;
                    let col = group_visit % 7;
                    group_base + row * 7 + if row & 1 == 0 { col } else { 6 - col }
                } else {
                    visit
                };
                let x_outer = chunk_start + offset;
                // SAFETY: each queue job is unique; complete forty-two-chunk
                // groups use a row-local reversal bijection, and a shorter tail is fixed.
