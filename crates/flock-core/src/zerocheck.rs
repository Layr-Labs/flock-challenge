...
/// Test-only forced-ON latch: with the cascade4 default demoted to opt-in,
/// the transcript-identity test engages the route through this instead of
/// mutating the process environment (which is OnceLock-latched and shared
/// across concurrently running tests).
#[cfg(test)]
pub(crate) static ZC_CASCADE4_FORCED_ON: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

/// Opt-in switch for the cascaded rounds-9/10 lookahead. DEFAULT OFF as of
/// draw 3: two ranked draws of the default-on tree clustered at −2.63%/−2.71%
/// versus a −0.06..−0.85% plain-redraw band the same evening — a systematic
/// shift, not dispersion — so the engagement is demoted to `FLOCK_ZC_CASCADE4=1`
/// (exact '1') pending the controlled default-off ranked sample. The route,
/// its tests, and its oracles stay in-tree; with the switch unset the tail
/// runs the promoted cascade3 route. OnceLock-latched: the environment is
/// read once per process. Bit-identical either way — same pure-reassociation
/// argument as the levels above, asserted by the cascade4
/// transcript-identity test.
#[inline]
...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    // Cascade one level deeper still (rounds 9+10): the composed 7+8 pass is
    // in the exact position the composed 5+6 pass was before cascade3 — each
    // output group materializes in registers before its store — so round
    ...
    // double-fold, deleting tail iterations i = 6 and i = 7 (their DRAM
    // passes and one of their FS-serialized round boundaries). Same
    // value-identity argument again: pure reassociation of exact F128
    // arithmetic, transcript order untouched.
    //
    // `r[k_skip+7] = 0` would make the lookahead's W-split unrecoverable from
    // the eq parity split (at every shape with m ≥ 15 that slot is a protocol
    // constant ≠ 0; for a sampled slot it is probability 2⁻¹²⁸); that case
    // falls back to the cascade3 route, which stays in the tree as the oracle
    // anyway. n_mlv ≥ 10 keeps the composed-9/10 input ≥ 16 (its own floor)
    // and every eq split at lo_size ≥ 2. Kill switch: FLOCK_NO_ZC_CASCADE4=1
    // (exact '1').
    let use_cascade4 =
    ...
    let (mut a_mlv, mut b_mlv, loop_start) = if let Some(la) = lookahead {
        ...
        if use_cascade {
            ...
            if use_cascade3 {
                ...
                if use_cascade4 {
                    // Level four: the composed 7+8 pass additionally carries
                    // the deferred round-nine quadratic — zero extra
                    // traversals.
                    ...
                    if tail_round_timing {
                        eprintln!(
                            "[zc-tail-rounds] composed rounds 7+8 fold (cascade +W''', out n={sixteenth}): {:.2} ms",
                    ...
                    // composed pass (ρ₇ and ρ₈ at once), replacing tail
                    // iterations i = 6 and i = 7: their 32 MiB + 16 MiB reads
                    // and 16 MiB + 8 MiB writes become one 32 MiB read +
                    // 8 MiB write.
                    ...
                    if tail_round_timing {
                        eprintln!(
                            "[zc-tail-rounds] composed rounds 9+10 fold (out n={sixtyfourth}): {:.2} ms",
...
mod tests {
    ...
    /// disengaged boundary; m=16 sits exactly on it.
    #[test]
...
// RealAdii cascade4 draw 2 on 55d2752.
// RealAdii cascade4 draw 3 (default-off diagnostic) on 55d2752.
