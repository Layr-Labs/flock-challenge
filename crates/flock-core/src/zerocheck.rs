...
/// mirroring [`ZC_CASCADE3_FORCED_OFF`].
#[cfg(test)]
...
/// (exact '1') restores the cascade3 i=6/i=7 tail route. OnceLock-latched.
/// Bit-identical either way — pure reassociation of exact F128 arithmetic.
#[inline]
...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    // Cascade one level deeper still (rounds 9+10): the composed 7+8 pass
    // reuses `fold2_plain_and_round67_into` (round-agnostic) so round nine's
    // message rides as a deferred quadratic in the not-yet-sampled ρ₇, and
    // rounds 9+10 collapse into one plain composed double-fold, deleting
    // tail iterations i = 6 and i = 7 (their DRAM passes and one FS barrier).
    // At the ranked shape that deletes ~32 MiB of traffic (see e1d note).
    //
    // `r[k_skip+7]` is the first sampled outer weight (not a β constant); the
    // zero check is probability 2⁻¹²⁸. n_mlv ≥ 10 keeps the composed-9/10
    ...
    let (mut a_mlv, mut b_mlv, loop_start) = if let Some(la) = lookahead {
        ...
        if use_cascade {
            ...
            if use_cascade3 {
                ...
                // 32 MiB write. With cascade4 the same pass also carries
                // the deferred round-nine quadratic (reusing the round-
                // agnostic `fold2_plain_and_round67_into`).
                ...
                if use_cascade4 {
                    ...
                    // Round nine: evaluate the deferred quadratic at ρ₇. No pass.
                    ...
                    // Rounds nine and ten fold together (ρ₇ and ρ₈), replacing
                    // tail i = 6 and i = 7: 32+16 MiB reads and 16+8 MiB
                    // writes become one 32 MiB read + 8 MiB write.
...
mod tests {
    ...
    /// C4-T5 — transcript identity for the fourth-level cascade (rounds 9+10):
    /// prove twice with cascade4 engaged and disabled (cascade3/2/lookahead
    /// stay on — cascade4's fallback IS the cascade3 route). m=14 (n_mlv=8)
    /// sits below cascade4's n_mlv ≥ 10 floor; m=16 engages.
    #[test]
    fn prove_transcript_identical_with_and_without_cascade4() {
        use std::sync::atomic::Ordering;
        for m in [14usize, 16, 18] {
