...
fn prove_packed_padded_inner<C: Challenger>(
    ...
    let (round1_ab_opt, round1_c_opt, s_hat_v_c) = if let Some(ab_inner) = precomputed_ab.as_ref() {
        ...
        if let Some(c_lincheck) = c_lincheck {
            ...
            //
            // r416b: overlap AB completion with the CPU C-suffix drain.
            // Prior code launched the GPU prefix, then ran AB to completion,
            // then drained the CPU C suffix serially — so the comment's
            // "AND with the CPU's own share" was aspirational only.
            //
            // IMPORTANT: on macOS/aarch64, `Round1CPrelude` owns a
            // `ZcFoldJob` that holds a `MutexGuard` (!Send). It must stay on
            // this thread. `rayon::join` moves both arms to worker threads and
            // failed the runner build (submission 4ada6f4). Use
            // `std::thread::scope`: child runs AB (Send captures only); parent
            // keeps the prelude and runs the C fold (CPU suffix + GPU join).
            // Byte ledger unchanged; schedule becomes wall≈max(AB, C_cpu).
            let c_prelude =
            ...
            let cpu_join = crate::pcs::commit::commit_cpu_ms();
            ...
            let (ab, c_bundle) = std::thread::scope(|scope| {
                ...
                let c_bundle = if fold8 {
                    let (c, s_hat_v_c, quad, fold8_tensor) =
                    ...
                    (c, s_hat_v_c, quad, None, Some(fold8_tensor))
                } else {
                ...
                let ab = ab_handle
                    .join()
                    .expect("AB completion thread did not panic");
                (ab, c_bundle)
            });
            if zc_timing {
                eprintln!(
                    "[zc-timing] round1 AB||lincheck-C scope-join: {:.2} ms cpu={:.1}",
                    t_join.elapsed().as_secs_f64() * 1e3,
                    crate::pcs::commit::commit_cpu_ms() - cpu_join,
                );
            }
