...
/// The direct row-major fold has no persistent stripe competing for L1.
/// Sixteen 4-KiB sum tables occupy 64 KiB on Apple performance cores and
/// halve accumulator interfaces relative to the legacy eight-stripe tile.
pub(crate) const ROW_MAJOR_NEON_TILE_T: usize = 16;
...
/// 64 consecutive canonical witness blocks per tile, transposes only one
/// 64-bit inner word into a 512-byte stack buffer, and immediately feeds the
/// existing eight-register NEON lookup kernel. No witness-sized stripe is
/// written or read.
#[cfg(target_arch = "aarch64")]
pub fn partial_fold_row_major_neon_oblock_padded(
    ...
    const TILE_T: usize = ROW_MAJOR_NEON_TILE_T;
