...
/// One claim owns a fixed strip of row groups within one block and evaluates
/// both radix-8 sweeps for that whole strip against a private staging tile,
/// so the intermediate that the layer-`L+3` pass used to reload from DRAM is
/// read out of cache instead. Each sweep runs its radix-8 group index outer
/// and the row group inner, so only eight source (resp. destination) streams
/// are live at once and each is a sequential run over the strip — the same
/// stream count the in-place passes present.
///
...
fn butterfly_interleaved_fused_6layer_staged_hetero(
    ...
    // The strip of row groups a claim carries. It is the tile's second
    // dimension, so it trades staging footprint against sequential run
    // length: 8 gives each of the eight live streams an 8 KiB sequential run
    // for a 512 KiB per-worker tile (~5 MiB across the pool, comfortably
    // L2-resident), where the claim's own footprint is what keeps the
    // heterogeneous tail short.
    ...
    crate::epool::run_hetero_chunks_stateful(
        ...
        // 64 tile rows of `ROWS_PER_TILE` row groups each, plus 8 rows
        // staging the second sweep's output rows for their store bursts.
        || vec![F128::ZERO; (64 * ROWS_PER_TILE + 8) * num_ntts],
        |tile, job| {
            let block = job / tiles_per_block;
