        ...
        F: Fn(usize, &[F128]) + Sync + Send,
    {
        ...
        while layer < n_top {
            ...
            if layer == 4
                ...
                && std::env::var_os("FLOCK_NTT_SLC_NEST").is_some_and(|v| v == "1")
            {
...
mod tests {
    ...
    /// Audit which layers of the ranked dim-20 standard basis have
    /// low-limb-only twiddles (candidates for the 2-PMULL kernel).
    #[test]
    #[ignore]
    fn audit_low_limb_twiddle_layers() {
        let ntt = AdditiveNttF128::standard(20);
        for layer in 0..20usize {
            let n = 1usize << layer;
            let low = (0..n).filter(|&b| ntt.twiddle(layer, b).hi == 0).count();
            eprintln!("layer {layer:2}: {low}/{n} low-limb");
        }
    }
    use super::*;
