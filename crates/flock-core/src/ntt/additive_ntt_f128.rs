...
/// Number of calibration/probe regions that require the promoted generic
/// suffix scheduler. This is process-global because the hybrid graph drains
/// its CPU arm on arbitrary Rayon workers.
...
impl AdditiveNttF128 {
        ...
        F: Fn(usize, &[F128]) + Sync + Send,
    {
        ...
        if ranked_flat_cpu_suffix_enabled() {
            // Flatten every selected (block,row) job into one Rayon region per
            // pass. The generic range driver opens a region for every block.
            self.forward_transform_interleaved_ranked_block_range_top_flat(
                data, b_start, b_end,
            );
        } else {
            // Calibration and exact same-binary control keep the promoted
            // generic scheduling curve.
    ...
    /// Absolute block indices preserve the generic range driver's twiddles.
        ...
        for layer in [START_LAYER, START_LAYER + 3] {
            ...
            (0..num_blocks * eighth)
                ...
                .for_each(|job| {
                    ...
                    // SAFETY: every job owns eight disjoint rows in exactly
                    // one selected block.
                    unsafe {
