...
/// Same-binary control for the flat ranked hybrid CPU suffix scheduler.
pub(crate) const ENV_NO_RANKED_FLAT_CPU_SUFFIX: &str =
    "FLOCK_NO_RANKED_FLAT_CPU_SUFFIX";

/// Number of active calibration/probe regions that require the incumbent
/// generic suffix scheduler. This is process-global (rather than thread-local)
/// because the hybrid graph's CPU arm is drained by arbitrary Rayon workers.
static RANKED_FLAT_CPU_SUFFIX_PROBE_DEPTH: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);

/// RAII suppression token used while the GPU/CPU split autotuner measures and
/// verifies candidates. Dropping it republishes the flat scheduler on every
/// exit path, including early returns.
#[must_use = "the guard must stay alive for the complete probe region"]
pub(crate) struct RankedFlatCpuSuffixProbeGuard {
    _private: (),
}

pub(crate) fn suppress_ranked_flat_cpu_suffix_for_probe() -> RankedFlatCpuSuffixProbeGuard {
    RANKED_FLAT_CPU_SUFFIX_PROBE_DEPTH
        .fetch_update(
            std::sync::atomic::Ordering::SeqCst,
            std::sync::atomic::Ordering::SeqCst,
            |depth| depth.checked_add(1),
        )
        .expect("ranked flat-suffix probe depth overflow");
    RankedFlatCpuSuffixProbeGuard { _private: () }
}

impl Drop for RankedFlatCpuSuffixProbeGuard {
    fn drop(&mut self) {
        RANKED_FLAT_CPU_SUFFIX_PROBE_DEPTH
            .fetch_update(
                std::sync::atomic::Ordering::SeqCst,
            ...
            .expect("ranked flat-suffix probe depth underflow");
    }
}

#[inline]
fn ranked_flat_cpu_suffix_enabled() -> bool {
    cfg!(all(
        target_os = "macos",
        target_arch = "aarch64",
        target_feature = "aes"
    )) && std::env::var_os(ENV_NO_RANKED_FLAT_CPU_SUFFIX).is_none()
        && RANKED_FLAT_CPU_SUFFIX_PROBE_DEPTH.load(std::sync::atomic::Ordering::SeqCst) == 0
}

#[cfg(test)]
fn ranked_flat_cpu_suffix_probe_depth() -> usize {
    RANKED_FLAT_CPU_SUFFIX_PROBE_DEPTH.load(std::sync::atomic::Ordering::SeqCst)
}
...
impl AdditiveNttF128 {
        ...
        F: Fn(usize, &[F128]) + Sync + Send,
    {
        ...
        if ranked_flat_cpu_suffix_enabled() {
            // Timed path: flatten every selected (block,row) job into one
            // Rayon region per pass. The generic range driver opens one
            // region per block (5 + 40 joins at the ranked default k=5).
            self.forward_transform_interleaved_ranked_block_range_top_flat(
                data, b_start, b_end,
            );
        } else {
            // Calibration/control path. The autotuner must observe the exact
            // promoted 0d8 scheduler while choosing and verifying its split;
            // its RAII probe guard reaches every Rayon worker through the
            // process-global atomic above.
...
mod tests {
    ...
    #[test]
    fn flat_suffix_probe_guard_is_global_nested_and_restores() {
        let baseline_depth = ranked_flat_cpu_suffix_probe_depth();
        let baseline_enabled = ranked_flat_cpu_suffix_enabled();

        fn return_early(expected_depth: usize) {
            let _guard = suppress_ranked_flat_cpu_suffix_for_probe();
            assert_eq!(ranked_flat_cpu_suffix_probe_depth(), expected_depth + 1);
        }

        return_early(baseline_depth);
        assert_eq!(ranked_flat_cpu_suffix_probe_depth(), baseline_depth);
        assert_eq!(ranked_flat_cpu_suffix_enabled(), baseline_enabled);

        let outer = suppress_ranked_flat_cpu_suffix_for_probe();
        assert_eq!(ranked_flat_cpu_suffix_probe_depth(), baseline_depth + 1);
        assert!(!ranked_flat_cpu_suffix_enabled());
        let (left_depth, right_depth) = rayon::join(
            ranked_flat_cpu_suffix_probe_depth,
            ranked_flat_cpu_suffix_probe_depth,
        );
        assert_eq!(left_depth, baseline_depth + 1);
        assert_eq!(right_depth, baseline_depth + 1);

        {
            let _inner = suppress_ranked_flat_cpu_suffix_for_probe();
            assert_eq!(ranked_flat_cpu_suffix_probe_depth(), baseline_depth + 2);
            assert!(!ranked_flat_cpu_suffix_enabled());
        }
        assert_eq!(ranked_flat_cpu_suffix_probe_depth(), baseline_depth + 1);
        drop(outer);
