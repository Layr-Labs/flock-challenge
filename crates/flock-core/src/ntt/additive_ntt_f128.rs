...
#[inline]
fn use_ranked_hybrid_suffix_radix64(
    log_d: usize,
    num_ntts: usize,
    start_layer: usize,
    stop_layer: usize,
    ...
    is_ranked_hybrid_suffix_radix64_shape(log_d, num_ntts, start_layer, stop_layer)
        ...
        && !HYBRID_SUFFIX_RADIX64_AUTOTUNE_SUPPRESSED
            .load(std::sync::atomic::Ordering::Relaxed)
}

/// Keep the experimental radix-64 schedule out of the untimed hybrid split
/// sweep. The sweep executes every candidate share repeatedly and was found
/// to perturb the following scored graph enough to erase the fixed-k6 win.
/// The incumbent schedule still chooses the host's split; the optimized
/// suffix is enabled again before the timed prove uses that published split.
static HYBRID_SUFFIX_RADIX64_AUTOTUNE_SUPPRESSED: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

pub(crate) fn set_hybrid_suffix_radix64_autotune_suppressed(suppressed: bool) {
    HYBRID_SUFFIX_RADIX64_AUTOTUNE_SUPPRESSED
