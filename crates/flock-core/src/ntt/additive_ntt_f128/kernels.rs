...
/// AArch64 specialization for a fused pair whose two inner twiddles have a
/// zero high limb. The outer twiddle remains on the ordinary field-multiply
/// path; only the two independent inner products are paired.
#[allow(clippy::too_many_arguments)]
#[inline]
pub(super) fn butterfly_fused_2layer_low_inner_twiddles(
    a: &mut [F128],
    b: &mut [F128],
    c: &mut [F128],
    d: &mut [F128],
    ...
    // SAFETY: the cfg gate guarantees PMULL through the aes feature.
    unsafe {
        aarch64::butterfly_fused_2layer_low_inner_twiddles(
            a, b, c, d, t_outer, t_inner_a, t_inner_b,
        );
    }
