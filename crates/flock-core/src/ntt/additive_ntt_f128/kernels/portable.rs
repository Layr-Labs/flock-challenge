...
/// of `num_ntts` lanes, that `tile` is writable for
/// `64 * strip * num_ntts + 8 * num_ntts` elements where
/// `strip = row_end - row_start` (this form uses only the first 64 tile rows;
/// the AArch64 form stages store bursts in the rest)
...
pub(super) unsafe fn butterfly_fused_6layer_staged_rows(
    ...
    unsafe {
        ...
        // Layers L..L+3: gather from the block, scatter into the tile. Group
        // index outer, row group inner, so only eight source streams are live.
        for i0 in 0..8 {
            for rr in 0..strip {
                let r = row_start + rr;
                for lane in 0..num_ntts {
                    ...
                    for (j, value) in values.iter().enumerate() {
                        *tile.add((i0 + 8 * j) * tile_row + rr * num_ntts + lane) = *value;
                    }
                }
            }
        }
        ...
        for g in 0..8 {
            for rr in 0..strip {
                ...
                for lane in 0..num_ntts {
                    let mut values = [F128::ZERO; 8];
                    for (j, value) in values.iter_mut().enumerate() {
                        *value = *tile.add((8 * g + j) * tile_row + rr * num_ntts + lane);
                    }
