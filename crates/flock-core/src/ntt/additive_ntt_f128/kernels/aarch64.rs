...
/// `72 * num_ntts` elements, and disjoint row ranges across concurrent calls.
#[target_feature(enable = "aes")]
#[allow(clippy::too_many_arguments)]
pub(super) unsafe fn butterfly_fused_6layer_staged_rows(
    ...
    unsafe {
        ...
        // Rows 0..64 of `tile` stage the fused row group; rows 64..72 stage
        // the second sweep's eight output rows for their store bursts.
        let burst = tile.add(64 * num_ntts);
        ...
        for r in row_start..row_end {
            ...
            // emitted as one sequential burst per destination row.
            for g in 0..8 {
