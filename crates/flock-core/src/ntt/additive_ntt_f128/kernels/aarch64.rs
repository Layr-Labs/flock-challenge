...
///
/// Loads the eight source rows from `msg`, runs the zero-root network into
/// `out0` (block 0 — all eight rows stored, since the destination is
/// uninitialized) and the generic network into `out1` (block 1). One source
/// read serves both rate-1/2 codeword halves.
///
/// The destinations are cold, uninitialized DRAM that is not consumed until
/// the following pass, so ordinary stores would pay a read-for-ownership on
/// every line across 16 concurrent multi-MiB-strided store streams —
/// measured as a net regression on quiet machines despite the traffic saved
/// on the read side. Lanes are therefore processed in pairs so every output
/// is a 32-byte `stnp` non-temporal pair store (the same best-effort
/// no-write-allocate hint the round-two producers use), and the L1-hot
/// source rows are simply reloaded between the two networks to keep the
/// live-register set bounded.
///
/// # Safety
/// As [`butterfly_fused_3layer_row`] for all three buffers, the caller
/// guarantees `tw0[0] == tw0[1] == tw0[3] == 0`, and `num_ntts` is even.
#[target_feature(enable = "aes")]
pub(super) unsafe fn butterfly_fused_3layer_pass1_from_message_row(
    ...
    unsafe fn store_pair_nt(dst: *mut F128, x: uint64x2_t, y: uint64x2_t) {
        // SAFETY: caller provides a valid 32-byte destination.
        unsafe {
            core::arch::asm!(
                "stnp {x:q}, {y:q}, [{dst}]",
                dst = in(reg) dst,
    ...
    unsafe {
        ...
        // Zero-root network over one lane's eight values (t0 = t1 = t3 = 0).
        #[inline(always)]
        unsafe fn zero_root_net(
        ...
        // Generic network over one lane's eight values.
        #[inline(always)]
        unsafe fn generic_net(
            v: &mut [core::arch::aarch64::uint64x2_t; 8],
        ...
        while lane < num_ntts {
            ...
            // Block 0: zero-root network for the lane pair.
            let mut va: [uint64x2_t; 8] =
                core::array::from_fn(|i| vld1q_u64(msg.add(off + i * step).cast::<u64>()));
            let mut vb: [uint64x2_t; 8] =
                core::array::from_fn(|i| vld1q_u64(msg.add(off + 1 + i * step).cast::<u64>()));
            zero_root_net(&mut va, z_t2, z_t4, z_t5, z_t6);
            zero_root_net(&mut vb, z_t2, z_t4, z_t5, z_t6);
            for i in 0..8 {
                store_pair_nt(out0.add(off + i * step), va[i], vb[i]);
            }

            // Block 1: generic network on the (L1-hot) reloaded source.
            let mut va: [uint64x2_t; 8] =
                core::array::from_fn(|i| vld1q_u64(msg.add(off + i * step).cast::<u64>()));
            let mut vb: [uint64x2_t; 8] =
                core::array::from_fn(|i| vld1q_u64(msg.add(off + 1 + i * step).cast::<u64>()));
            generic_net(&mut va, &g_t);
            generic_net(&mut vb, &g_t);
            for i in 0..8 {
                store_pair_nt(out1.add(off + i * step), va[i], vb[i]);
            }
