...
/// in vector registers across the tile. The top NTT queues already hand out
/// 128-row tiles, so this removes the same seven L1 loads from every row call
/// without changing row ownership or the butterfly order.
#[target_feature(enable = "aes")]
