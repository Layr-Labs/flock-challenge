...
//!
//! The assembly template is compiler-generated from the same BLAKE3 1.8.5
//! NEON primitives linked by this crate, with `-O3 -mcpu=apple-m3`. A checked-
//! in generator expands its fixed seven-round schedule without changing the
//! compiler's state allocation or spill plan. The kernel fixes the exact
//! Merkle-leaf contract used here: twelve contiguous 1024-byte unkeyed chunks,
//! counter zero, `CHUNK_START | CHUNK_END`, 32 output bytes each.
