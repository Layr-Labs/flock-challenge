...
/// `false` (caller must use the CPU path) when the GPU is unavailable, the
/// level is too small to win, or the dispatch fails.
pub(super) fn hash_leaves_1024(data: &[u8], out: &mut [Hash]) -> bool {
    debug_assert_eq!(data.len(), out.len() * 1024);
    if out.len() < MIN_LEAVES {
        return false;
    }
    match gpu() {
