...
fn sparse_row_fold_alpha_batched(
    ...
    // scheduling changes. Per-chunk accumulators sit behind a lock (the
    // hetero closure is `Fn(usize) + Sync` and cannot capture `&mut`);
    // contention is one lock acquisition per chunk.
    let partials: Vec<std::sync::Mutex<Vec<F128>>> = (0..n_chunks)
        .map(|_| std::sync::Mutex::new(vec![F128::ZERO; n_cols]))
        .collect();
