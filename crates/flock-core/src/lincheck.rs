              ...
              for word_offset in 0..f128_step {
                  ...
                  let words = unsafe {
                    ...
                    for byte_lane in 0..8 {
                        ...
                        let mut lo_rows = 0u64;
                        let mut hi_rows = 0u64;
                        for (r, word) in words.iter().enumerate() {
                            lo_rows |= ((word.lo >> shift) & 0xff) << (r * 8);
                            hi_rows |= ((word.hi >> shift) & 0xff) << (r * 8);
                        }
                        let lo_cols = transpose_8x8_bits(lo_rows).to_le_bytes();
                        let hi_cols = transpose_8x8_bits(hi_rows).to_le_bytes();
                        chunk[output_base + shift..output_base + shift + 8]
                            .copy_from_slice(&lo_cols);
                        chunk[output_base + 64 + shift..output_base + 64 + shift + 8]
                            .copy_from_slice(&hi_cols);
                    }
              }
        } else {
