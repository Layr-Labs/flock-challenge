...
fn sumcheck_round_eval_par(c: &[F128], z: &[F128]) -> (F128, F128) {
    ...
    // same XOR partial merge, no writes to shared tables.
    const EVAL_CHUNK: usize = 2048;
    if half >= 16 * EVAL_CHUNK
        && half % EVAL_CHUNK == 0
        && lincheck_eval_hetero_enabled()
        && crate::epool::epool().is_some()
    {
        let n_chunks = half / EVAL_CHUNK;
        ...
        crate::epool::run_hetero_chunks(n_chunks, |ci| {
            ...
            unsafe {
                let cp = c_base.ptr();
                let zp = z_base.ptr();
                let start = ci * EVAL_CHUNK;
                let mut e1 = F128::ZERO;
                let mut einf = F128::ZERO;
                for i in start..start + EVAL_CHUNK {
                    let clo_i = *cp.add(i);
        ...
        return partials.into_iter().fold(
            (F128::ZERO, F128::ZERO),
