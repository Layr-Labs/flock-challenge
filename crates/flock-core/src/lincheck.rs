...
fn sumcheck_round_eval_par(c: &[F128], z: &[F128]) -> (F128, F128) {
    ...
    // Accumulate several products per Rayon fold rather than emitting one pair
    // per element. Four independent chains hide field-multiply latency while
    // the fold also removes most of the per-element reduction traffic.
    chi.par_iter()
        .zip(zhi.par_iter())
        .zip(clo.par_iter().zip(zlo.par_iter()))
        .fold(
            || ([F128::ZERO; 4], [F128::ZERO; 4], 0usize),
            |mut acc, ((&chi_i, &zhi_i), (&clo_i, &zlo_i))| {
                let lane = acc.2 & 3;
                acc.0[lane] += chi_i * zhi_i;
                acc.1[lane] += (chi_i + clo_i) * (zhi_i + zlo_i);
                acc.2 += 1;
                acc
            },
        )
