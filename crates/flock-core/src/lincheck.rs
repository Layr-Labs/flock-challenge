...
impl LincheckCircuit for CscCircuit {
    ...
    fn fold_alpha_batched(&self, alpha: F128, eq_inner: &[F128]) -> Vec<F128> {
        ...
        // Hetero E-core drain: each column is independent (disjoint write to
        // out[c], no reduce), so the E-cores can claim column ranges without
        // any byte-identity risk. Same per-column arithmetic, same output
        // vector — only which pool claims a column differs.
        // `FLOCK_NO_LINCHECK_CSC_FOLD_HETERO=1` (exactly `"1"`) restores the
        // incumbent rayon-only path.
        const CSC_FOLD_HETERO_CHUNK: usize = 64;
        if self.n_cols >= 16 * CSC_FOLD_HETERO_CHUNK
            && lincheck_csc_fold_hetero_enabled()
            && crate::epool::epool().is_some()
        {
            let n_chunks = self.n_cols.div_ceil(CSC_FOLD_HETERO_CHUNK);
            let out_base = crate::epool::SyncPtr(out.as_mut_ptr());
            crate::epool::run_hetero_chunks(n_chunks, |ci| {
                let start = ci * CSC_FOLD_HETERO_CHUNK;
                let end = (start + CSC_FOLD_HETERO_CHUNK).min(self.n_cols);
                // SAFETY: the queue claims each `ci` exactly once; column
                // ranges [start, end) are disjoint per chunk and the two-pool
                // join publishes every write before `out` is read below.
                unsafe {
                    let p = out_base.ptr();
                    for c in start..end {
                        p.add(c).write(one_col(c));
                    }
                }
            });
            return out;
...
/// E-core hetero scheduling for the CSC circuit's `fold_alpha_batched`.
///
/// Each column is independent (disjoint write to `out[c]`, no reduce), so
/// the E-cores can claim column ranges without any byte-identity risk.
/// `FLOCK_NO_LINCHECK_CSC_FOLD_HETERO=1` (exactly `"1"`) restores the
/// incumbent rayon-only path as the same-binary A/B control.
fn lincheck_csc_fold_hetero_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        !std::env::var("FLOCK_NO_LINCHECK_CSC_FOLD_HETERO").is_ok_and(|v| v == "1")
    })
}
