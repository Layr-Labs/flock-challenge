...
/// Byte-pool counterpart of [`f128_pool_big_entries`], for the round-2
/// compact-delta backing (`ScratchBytes`).
#[cfg(all(target_os = "macos", target_arch = "aarch64"))]
pub(crate) fn u8_pool_big_entries(min_bytes: usize) -> Vec<(usize, usize)> {
    let pool = POOL_U8.lock().unwrap();
    pool.iter()
        .filter(|v| v.capacity() >= min_bytes)
