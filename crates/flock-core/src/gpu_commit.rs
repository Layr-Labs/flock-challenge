...
"#;
    ...
    const WARMUP_CACHE_MAGIC: u64 = 0x464C_4B5F_574C_4331; // "FLK_WLC1"
    ...
    fn warmup_cache_path() -> std::path::PathBuf {
        std::env::temp_dir().join("flock-warmup-latch-v1.bin")
    }
    ...
    fn read_warmup_cache(log_d: usize, n_leaves: usize) -> Option<WarmupCache> {
        ...
        let tree_nodes = take_u64(&bytes)? as usize;
        let expect_nodes = if latch_on { 2 * n_leaves - 1 } else { 0 };
        if tree_nodes != expect_nodes || !cpu_wall_ms.is_finite() || tuned_k > 16 {
            return None;
        }
        let tree_bytes = bytes.get(off..)?;
        if tree_bytes.len() != tree_nodes * core::mem::size_of::<Hash>() {
            return None;
        }
        let mut cpu_tree = vec![[0u8; 32]; tree_nodes];
        // SAFETY: Hash is [u8; 32]; lengths were just checked.
        unsafe {
            core::ptr::copy_nonoverlapping(
                tree_bytes.as_ptr(),
                cpu_tree.as_mut_ptr().cast::<u8>(),
                tree_bytes.len(),
            );
        }
        Some(WarmupCache { latch_on, tuned_k, cpu_wall_ms, cpu_tree })
    }
    ...
    fn write_warmup_cache(
        ...
        let tree: &[Hash] = if latch_on { cpu_tree } else { &[] };
        let mut buf =
            Vec::with_capacity(64 + tree.len() * core::mem::size_of::<Hash>());
        for v in [
            WARMUP_CACHE_MAGIC,
        ...
        // SAFETY: Hash is [u8; 32].
        buf.extend_from_slice(unsafe {
            core::slice::from_raw_parts(
                tree.as_ptr().cast::<u8>(),
                core::mem::size_of_val(tree),
            )
        });
