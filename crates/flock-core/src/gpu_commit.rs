...
pub const ENV_NO_LEAF_VEC4: &str = "FLOCK_NO_LEAF_VEC4";
...
pub(crate) fn gpu_leaf_vec4_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        gpu_leaf_vec4_value_enabled(std::env::var_os(ENV_NO_LEAF_VEC4).as_deref())
    })
}
...
mod leaf_vec4_gate_tests {
    ...
    #[test]
    fn default_on_exact_one_kill_switch() {
...
pub const ENV_NO_TUNER_ARM_TIMING: &str = "FLOCK_NO_TUNER_ARM_TIMING";
...
fn tuner_arm_timing_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        tuner_arm_timing_value_enabled(std::env::var_os(ENV_NO_TUNER_ARM_TIMING).as_deref())
    })
}
...
mod tuner_arm_timing_gate_tests {
    ...
    #[test]
    fn exact_one_is_the_only_join_wall_value() {
        assert_eq!(super::ENV_NO_TUNER_ARM_TIMING, "FLOCK_NO_TUNER_ARM_TIMING");
...
"#;
    ...
    pub(crate) struct Gpu {
        ...
        /// `pso_parent3`. `NIL` when `FLOCK_NO_LEAF_VEC4=1` or when their
    ...
    fn autotune_hybrid_split(
        ...
        let contended_run = |k: usize| -> Result<f64, String> {
            ...
            // (`ENV_NO_TUNER_ARM_TIMING` docs; `1` restores the join wall).
            let t0 = std::time::Instant::now();
    ...
    pub(crate) fn retune_ranked_hybrid_with_exact_contention(
        ...
        let sample = |k: usize| -> Result<f64, String> {
            ...
            // so score by the graph arm's own wall (`ENV_NO_TUNER_ARM_TIMING`
