...
"#;
    ...
    /// clear GPU straggling at `15·hi/16`, so the full path has its own half cap.
    ///
    ...
    pub(crate) fn zc_r2_gate_share(ratio: f64, hi_size: usize) -> usize {
        ...
        // The full eight-product kernel is deliberately capped at half. Its
        // warmup ratio can be optimistic enough to select the legacy 15/16
        // cap, while same-host forced-share sweeps put its latency minimum near
        // half and show clear GPU straggling at 15/16. The incumbent four-
...
"#;
    ...
    fn zc_r2_gate_share_policy() {
        ...
        // The default full-lookahead arm uses its measured half cap.
        assert_eq!(zc_r2_gate_share(0.57, 2048), 1024);
        assert_eq!(zc_r2_gate_share(0.38, 2048), 1024);
        assert_eq!(zc_r2_gate_share(0.62, 2048), 1024);
        assert_eq!(zc_r2_gate_share(1.0, 2048), 1024);
        // Above the cap's crossover, use the exact rounded balance solution.
