...
fn gpu_leafless_l0_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        std::env::var_os(ENV_NO_GPU_LEAFLESS_L0).is_none()
            // Leaf hashes are regenerated by the direct-fold8 ranked opener.
            // Keep the ordinary complete-tree layout if its explicit
            // fallback is selected before the process-wide dispatch latches.
...
"#;
    ...
    pub(crate) fn finish_from_z_first_pass_or_fallback(
        ...
        // Snapshot the selected hybrid split once for both graph submission
        // and the resulting zero-copy tree metadata.  A different split is
        // already handled by draining/rebuilding the early prefix below.
        let k_cpu16 = hybrid_cpu_sixteenths();
        ...
        let codeword_len = params.codeword_len_f128();
        // The ranked streamed path executes the same hybrid graph as the
        // ordinary steady path.  Its GPU prefix omits leaf hashes only after
        // the supplemental warmup has byte-validated the parent level, so the
        // zero-copy tree view must carry the identical omitted-prefix extent
        // to the opening code.  A zero here would make the opener consume the
        // intentionally absent leaf slots as ordinary hashes.
        let leafless_prefix = if leafless_l0_is_verified() && k_cpu16 > 0 {
            (16 - k_cpu16) * (stream.n_leaves / 16)
        } else {
            0
        };
        let tree = unsafe {
