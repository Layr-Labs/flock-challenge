...
mod fused_leaf_geometry_tests {
    ...
    fn final_tile_bijects_to_contiguous_blake3_leaf_words() {
        ...
        // The mixed chain needs 12 base + 192 table uint4s, 80 uint nibble
        // entries, and 1024 tile uint4s: comfortably below the 32 KiB cap.
        assert_eq!((12 + 192 + 1024) * 16 + 80 * 4, 19_968);
        assert!(19_968 <= 32 * 1024);
    }
}
...
// sixteen contiguous 1 KiB leaves. It preserves the incumbent mixed final
// pass (`ntt_pass5_mixed`) before feeding the already-resident tile to the
// unchanged BLAKE3 chunk chain. This deletes the subsequent leaf_hash read
// without replacing the current short-twiddle arithmetic.
...
{
    constexpr uint F = 4u, NF = 1u << F;
    constexpr uint NNIB_A = 10u;
    constexpr uint NNIB_B = 5u;
    threadgroup uint4 bases[3u * 4u];
    threadgroup uint4 tabs[3u * 64u];
    threadgroup uint nibA[4u * NNIB_A];
    threadgroup uint nibB[8u * NNIB_B];
    ...
    pub(crate) unsafe fn encode_ntt_passes_prefix(
        ...
        unsafe {
            ...
            for (l, f) in super::plan_passes(log_d, start_layer) {
                ...
                let s = log_d - l - f;
                let fused_final = fused_leaf.filter(|_| {
                    f == 4
                        && l + f == log_d
                        && s == 0
                        && super::gpu_mixed_final_selected(log_d, l, f)
