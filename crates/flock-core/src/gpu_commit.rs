...
"#;
    ...
    // the untimed warmup prove in the common case). L2 builds directly in a
    // pooled page-aligned CPU allocation; L1 retains the promoted persistent
    // shared output plus parallel copy after ten-thread decomposition found
    // its per-call output wrap neutral-to-negative.
    // =======================================================================
...
"#;
    ...
    pub(crate) fn gpu_recursive_merkle_blake3(data: &[u8], num_leaves: usize) -> Option<Vec<Hash>> {
        ...
        let direct = if num_leaves == 1usize << 16 && rec_merkle_direct_output_enabled() {
