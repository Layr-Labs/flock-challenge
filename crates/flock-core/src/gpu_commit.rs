...
/// Shared lifetime guard for the two zero-copy views returned by a latched
/// GPU commit.  The next commit may overwrite both persistent Metal buffers,
/// so release the in-use flag only after the codeword AND tree views are gone.
struct GpuCommitLease;

impl Drop for GpuCommitLease {
    fn drop(&mut self) {
...
/// PCS open are ordinary cached reads). The shared commit lease releases the
/// staging and tree buffers after both zero-copy views are dropped.
pub struct GpuCodeword {
    ptr: *const F128,
    len: usize,
    _lease: std::sync::Arc<GpuCommitLease>,
}
...
impl GpuCodeword {
    ...
    pub(crate) unsafe fn new(ptr: *const F128, len: usize) -> Self {
        Self {
            ptr,
            len,
            _lease: std::sync::Arc::new(GpuCommitLease),
        }
    }
}
...
/// Read-only view of the flat L0 Merkle tree in the persistent shared Metal
/// buffer.  This removes the timed path's 64 MiB copy-out; opening reads the
/// same host-visible bytes directly.
pub struct GpuTree {
...
// SAFETY: the process-lifetime shared Metal allocation is immutable while
// either lease holder exists; the GPU path checks the same in-use flag before
// issuing a new graph.
unsafe impl Send for GpuTree {}
unsafe impl Sync for GpuTree {}

impl core::ops::Deref for GpuTree {
    type Target = [crate::merkle::Hash];
    fn deref(&self) -> &[crate::merkle::Hash] {
        // SAFETY: contract of `new_commit_views` below.
        unsafe { core::slice::from_raw_parts(self.ptr, self.len) }
    }
}

/// Construct the two views of one completed GPU commit under a shared lease.
///
/// SAFETY: both pointers must remain valid and immutable until both returned
/// values are dropped.  The latched Metal buffers satisfy that contract while
/// `STAGING_IN_USE` remains set.
#[cfg_attr(
    not(all(target_os = "macos", target_arch = "aarch64")),
    allow(dead_code)
)]
pub(crate) unsafe fn new_commit_views(
    codeword_ptr: *const F128,
    codeword_len: usize,
    tree_ptr: *const crate::merkle::Hash,
    tree_len: usize,
) -> (GpuCodeword, GpuTree) {
    let lease = std::sync::Arc::new(GpuCommitLease);
    (
        GpuCodeword {
            ptr: codeword_ptr,
            len: codeword_len,
...
"#;
    ...
    /// 0 disables (pure-GPU graph). Default 7 with the cache-local suffix:
    /// measured graph wall 110.45–111.92 ms, versus 117.03 at k=6, 120.03 at
    /// k=8, and roughly 142–145 ms for the old k=2 frontier on the same box.
    /// The original streaming suffix takes 240.89 ms at k=7; the deep-pair
    /// schedule and cache-hot leaf callback are what make the wider CPU share
    /// viable.
    ...
    /// buffers), return the pooled input codeword to the scratch pool, and
    /// hand back guarded zero-copy views of the staging and tree buffers.
    fn run_latched(
        latch: &mut LatchState,
        z_packed: &[F128],
        mut codeword: Vec<F128>,
        ...
        // Exact same-binary control for the previous 64 MiB copy-out path.
        // Production keeps a guarded view of the host-visible Metal buffer.
        let force_copyout = std::env::var_os("FLOCK_GPU_TREE_COPYOUT").is_some();
        let copied_tree = force_copyout.then(|| {
        ...
        if std::env::var_os("FLOCK_COMMIT_TIMING").is_some() || debug_enabled() {
            eprintln!(
                "[commit-timing] gpu-commit: graph {graph_ms:.2} ms + tree-handoff {:.2} ms{}",
        ...
        if let Some(tree) = copied_tree {
            ...
            (CodewordBuf::Gpu(gpu_codeword), MerkleTreeBuf::Cpu(tree))
        } else {
            let (gpu_codeword, gpu_tree) = unsafe {
                super::new_commit_views(
                    gpu.buffer_contents(staging).cast::<F128>(),
                    codeword_len,
                    gpu.buffer_contents(tree_buf).cast::<Hash>(),
                    total_nodes,
            ...
            (CodewordBuf::Gpu(gpu_codeword), MerkleTreeBuf::Gpu(gpu_tree))
        }
    }
...
mod tests {
    ...
    fn gpu_latched_commit_end_to_end_at_ranked_shape() {
        ...
        eprintln!("latched commit (replicate+gpu graph+zero-copy views): {latched_ms:.2} ms");

        assert_eq!(c1.root, c2.root, "roots differ between warmup and latched");
        assert_eq!(tree1.as_slice(), &*pd2.merkle_tree, "trees differ");
