...
"#;
    ...
    /// `FLOCK_NO_HYBRID_TUNE_ARM=1` scores on the join wall (`max(arm)`) like
    /// the incumbent instead of on the k-dependent graph arm.
    #[inline]
    fn hybrid_tune_arm_scoring_enabled() -> bool {
        use std::sync::OnceLock;
        static ON: OnceLock<bool> = OnceLock::new();
        *ON.get_or_init(|| std::env::var_os("FLOCK_NO_HYBRID_TUNE_ARM").is_none())
    }
    ...
    ///
    /// Two independent corrections, each with its own kill switch:
    ///
    /// 1. **Score the arm the choice controls.** `k` sets the CPU share of the
    ///    commit graph; the A/B replay arm is *k-independent*. Scoring on the
    ///    join wall means every sample in which the replay outlasts the graph
    ///    contributes a number containing no information about `k`, and the
    ///    two arms are near-balanced on the ranked runner (A/B branch 45.65 ms
    ///    against a 49.96 ms deferred stripe, join tail 0.02 ms), so which arm
    ///    dominates flips on noise. Minimising the graph arm is never the
    ///    wrong objective: where it is critical, minimising it minimises the
    ///    join; where it is not, minimising it is harmless.
    ///
...
"#;
    ...
    mod split_select_tests {
        ...
        /// Default scoring: per-candidate min of the k-dependent arm.
        #[test]
        fn exact_selection_reduces_arm_with_minimum() {
            let mut samples = [[s(100.0, 100.0); 2]; RANKED_EXACT_TUNE_CANDIDATES.len()];
            // Candidate 1 is genuinely fastest on its own arm, but a
            // k-independent replay pins its join wall to 100 in both passes.
            samples[1] = [s(100.0, 90.0), s(100.0, 92.0)];
            assert_eq!(score_ranked_exact_samples(samples).unwrap()[1], 90.0);
        }
