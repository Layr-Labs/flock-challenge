...
/// (the exact fallback). Unlike the refuted whole-phase tail offload, this arm
/// computes the four parity-split round-two products and all four remaining
/// cross-pair lookahead products for a measured prefix of hi-chunks. The CPU
/// still produces byte-identical anchors and packed deltas for every chunk, so
/// the GPU's entire output surface is only 128 reduced bytes per chunk.
/// The per-chunk values are bit-exact by construction (the fold is the same
/// F2-linear byte-table XOR, products use the oracle-proven emulated carry-less
/// multiply, and unreduced 256-bit accumulation is order-independent XOR), and
/// warmup calibration refuses to publish a share until all eight GPU outputs
/// compare equal to the CPU on the target machine itself.
...
/// Full-lookahead selector shared with the driver. The full kernel remains
/// oracle-covered but is parked after three official share experiments; the
/// scored path isolates the parity SIMD-reduction change.
fn gpu_zc_r2_full_lookahead_enabled() -> bool {
    // Three official share experiments all regressed. Keep the oracle-covered
    // full kernel parked; the scored path isolates the SIMD reduction on the
    // promoted four-product contract.
    false
}
...
// Metal SIMD shuffles do not accept 64-bit vectors. Split each 256-bit XOR
// accumulator into low/high uint4 halves, reduce those halves independently,
// then join the two totals after the shuffle network.
static inline uint4 zc_r2_simd_parity_reduce(uint4 v) {
...
static inline ulong4 zc_r2_join(uint4 lo, uint4 hi) {
    return ulong4(ulong(lo.x) | (ulong(hi.x) << 32ul),
                  ulong(lo.y) | (ulong(hi.y) << 32ul),
                  ulong(lo.z) | (ulong(hi.z) << 32ul),
                  ulong(lo.w) | (ulong(hi.w) << 32ul));
}

struct ZcR2Params { uint lo_size; uint xpt; uint mask; uint useful; uint full; };
...
// even and odd x_lo halves stay apart; thread 0 reduces and writes eight
// REDUCED, eq_hi-weighted partials: parity-split round-two products followed
// by `[W0,W3,W4,W5]`. Slots 2/3 plus 4..7 are exactly the six deferred
// round-three lookahead aggregates.
    ...
    uint lid  [[thread_index_in_threadgroup]])
{
    ...
    // Four accumulators per lane. The first two retain the round-two products;
    // the latter two carry the four deferred lookahead products, multiplexed by
    // x_lo parity: even lanes hold (W3,W4), odd lanes hold (W0,W5).
    ...
    for (uint k = 0u; k < p.xpt; k++) {
        ...
        bool valid = (pair_idx & p.mask) < p.useful;
        ...
        // Round-two products keep their own eq2 lane weights.
        ...
        if (p.full != 0u) {
            // Adjacent x_lo lanes are one lookahead group and are always
            // adjacent inside an Apple SIMD-group (256 is even). All lanes,
            // including padding lanes, participate in the shuffle; padding
            // contributes zero rows but its odd eq2 weight still correctly
            // weights a half-padded group. Even lanes compute W3/W4, odd lanes
            // W0/W5. The uniform flag is only a same-binary A/B kill switch.
            uint4 pa0 = simd_shuffle_xor(a0, 1u);
            uint4 pa1 = simd_shuffle_xor(a1, 1u);
            uint4 pb0 = simd_shuffle_xor(b0, 1u);
            uint4 pb1 = simd_shuffle_xor(b1, 1u);
            uint4 pe  = simd_shuffle_xor(e, 1u);
            uint4 gx, gy, w;
            if ((x_lo & 1u) == 0u) {
                w = pe; // the odd lane's eq2 weight = r1 * eq3(group)
                gx = gf_reduce(clmul128(a0 ^ pa0, b0 ^ pb0)); // W3
                gy = gf_reduce(clmul128(a1 ^ pa1, b1 ^ pb1)); // W4
            } else {
                w = e;
                gx = gf_reduce(clmul128(a0, b0)); // W0
                gy = gf_reduce(clmul128(a0 ^ a1 ^ pa0 ^ pa1,
                                        b0 ^ b1 ^ pb0 ^ pb1)); // W5
            }
            U256k mx = clmul128(w, gx);
            U256k my = clmul128(w, gy);
            accx ^= ulong4(mx.r0, mx.r1, mx.r2, mx.r3);
            accy ^= ulong4(my.r0, my.r1, my.r2, my.r3);
        }
    }

    // Convert the four 256-bit accumulators into shuffle-supported halves.
    uint4 a1lo = uint4(uint(acc1.x), uint(acc1.y), uint(acc1.z), uint(acc1.w));
    uint4 a1hi = uint4(uint(acc1.x >> 32ul), uint(acc1.y >> 32ul),
                       uint(acc1.z >> 32ul), uint(acc1.w >> 32ul));
    uint4 ailo = uint4(uint(acci.x), uint(acci.y), uint(acci.z), uint(acci.w));
    uint4 aihi = uint4(uint(acci.x >> 32ul), uint(acci.y >> 32ul),
                       uint(acci.z >> 32ul), uint(acci.w >> 32ul));
    uint4 axlo = uint4(uint(accx.x), uint(accx.y), uint(accx.z), uint(accx.w));
    uint4 axhi = uint4(uint(accx.x >> 32ul), uint(accx.y >> 32ul),
                       uint(accx.z >> 32ul), uint(accx.w >> 32ul));
    uint4 aylo = uint4(uint(accy.x), uint(accy.y), uint(accy.z), uint(accy.w));
    uint4 ayhi = uint4(uint(accy.x >> 32ul), uint(accy.y >> 32ul),
                       uint(accy.z >> 32ul), uint(accy.w >> 32ul));

    // Reduce inside each 32-lane SIMD-group first. This replaces four full
    // 256-thread reductions (31 threadgroup barriers including scratch reuse)
    // with uint4 register shuffles plus one barrier.
    a1lo = zc_r2_simd_parity_reduce(a1lo);
    a1hi = zc_r2_simd_parity_reduce(a1hi);
    ailo = zc_r2_simd_parity_reduce(ailo);
    aihi = zc_r2_simd_parity_reduce(aihi);
    if (p.full != 0u) {
        axlo = zc_r2_simd_parity_reduce(axlo);
        axhi = zc_r2_simd_parity_reduce(axhi);
        aylo = zc_r2_simd_parity_reduce(aylo);
        ayhi = zc_r2_simd_parity_reduce(ayhi);
    }
    uint slid = lid & 31u;
    uint sgid = lid >> 5u;
    if (slid < 2u) {
        uint rb = (sgid * 2u + slid) * 4u;
        red[rb]      = zc_r2_join(a1lo, a1hi);
        red[rb + 1u] = zc_r2_join(ailo, aihi);
        red[rb + 2u] = zc_r2_join(axlo, axhi);
        red[rb + 3u] = zc_r2_join(aylo, ayhi);
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    if (lid == 0u) {
        ulong4 chunk1e = ulong4(0ul), chunk1o = ulong4(0ul);
        ulong4 chunkie = ulong4(0ul), chunkio = ulong4(0ul);
        ulong4 chunkw0 = ulong4(0ul), chunkw3 = ulong4(0ul);
        ulong4 chunkw4 = ulong4(0ul), chunkw5 = ulong4(0ul);
        ...
        uint4 eh = eq_hi[tgid];
        uint base = tgid * 8u;
        partials[base]      = gf_reduce(clmul128(eh, gf_reduce(u1e)));
        partials[base + 1u] = gf_reduce(clmul128(eh, gf_reduce(uie)));
        partials[base + 2u] = gf_reduce(clmul128(eh, gf_reduce(u1o)));
        partials[base + 3u] = gf_reduce(clmul128(eh, gf_reduce(uio)));
        partials[base + 4u] = gf_reduce(clmul128(eh, gf_reduce(uw0)));
        partials[base + 5u] = gf_reduce(clmul128(eh, gf_reduce(uw3)));
        partials[base + 6u] = gf_reduce(clmul128(eh, gf_reduce(uw4)));
        partials[base + 7u] = gf_reduce(clmul128(eh, gf_reduce(uw5)));
    }
}
"#;
    ...
    /// chunk (folds and stores, no products); the share first solves
    /// `(hi - (1-ALPHA) g) c_f = g u_g` with no CPU-ward bias.
    ///
    /// The promoted four-product kernel historically raised its overshoot cap
    /// from `hi/2` through `3·hi/4` and `7·hi/8` to `15·hi/16`: its ranked
    /// ratio near 0.57 left the GPU idle while the residual product-heavy CPU
    /// prefix bound. That history still applies to the same-binary kill-switch
    /// path. It does NOT transfer to the default eight-product kernel, whose
    /// GPU unit cost and clock-ramp error are much larger. A same-host forced
    /// sweep of that kernel put the latency minimum near `hi/2` and showed
    /// clear GPU straggling at `15·hi/16`. The official half-cap result still
    /// lost 0.83%, so the full path uses a conservative one-quarter cap: just
    /// above CPU break-even versus the incumbent, with substantially less GPU work.
    ///
    ...
    pub(crate) fn zc_r2_gate_share(ratio: f64, hi_size: usize) -> usize {
        ...
        // The full eight-product kernel is deliberately capped at one quarter. Its
        // warmup ratio can be optimistic enough to select the legacy 15/16
        // cap. The official half-share experiment still straggled; one quarter
        // keeps GPU work below the incumbent while barely clearing the CPU
        // break-even point. The incumbent four-
        // product kill-switch path retains its independently tuned cap.
        let cap = if super::gpu_zc_r2_full_lookahead_enabled() {
            hi_size / 4
        } else {
            hi_size * 15 / 16
        };
    ...
    pub(crate) enum ZcR2Result {
        ...
        Calibrated,
        /// Timed-prove prefix partials, bit-exact per chunk: four parity-split
        /// round-two products followed by lookahead `[W0, W3, W4, W5]`, all
        /// eq_hi-weighted.
        Prefix(Vec<[F128; 8]>),
    ...
    unsafe fn zc_r2_submit(
        ...
        unsafe {
            ...
            let params = P {
                ...
                mask,
                useful,
                full: u32::from(super::gpu_zc_r2_full_lookahead_enabled()),
            };
    ...
    pub(crate) fn launch_zc_r2_products(
        ...
        unsafe {
            ...
            // Eight F128 per chunk: parity-split round-two products followed
            // by lookahead `[W0, W3, W4, W5]`.
            let need_part = hi_size * 128;
            if state.part_cap < need_part {
                if state.part_cap > 0 {
                    gpu.release(state.part_buf);
    ...
    /// aggregates. The default arm checks all six against the corresponding
    /// GPU lanes, not merely the summed round-two pair; the same-binary kill
    /// switch checks the incumbent odd-parity pair only.
    pub(crate) fn zc_r2_wait(
        ...
        unsafe {
            ...
            // Both the summed round-two pair and all six deferred lookahead
            // aggregates are checked, so a kernel with a correct total but a
            // wrong parity/cross-pair product can never be admitted.
            for c in 0..job.chunks {
                let summed = (out[c][0] + out[c][2], out[c][1] + out[c][3]);
                let split_ok = cpu_la.is_none_or(|la| {
                    if super::gpu_zc_r2_full_lookahead_enabled() {
                        la[c]
                            == [
                                out[c][2], out[c][3], out[c][4], out[c][5], out[c][6], out[c][7],
                            ]
                    } else {
...
"#;
    ...
    fn zc_r2_gate_share_policy() {
        ...
        // The scored, promoted four-product path retains its 15/16 policy.
        assert_eq!(zc_r2_gate_share(0.57, 2048), 1920);
        assert_eq!(zc_r2_gate_share(0.38, 2048), 1920);
        assert!(zc_r2_gate_share(0.62, 2048) < 2048);
        assert_eq!(zc_r2_gate_share(1.0, 2048), 1412);
        assert_eq!(zc_r2_gate_share(2.0, 2048), 836);
    ...
    fn gpu_zc_r2_products_match_cpu_oracle() {
        ...
        // CPU reference for all eight GPU lanes. Round-two products retain
        // their own eq2 weights; the four cross-pair lookahead products use
        // the odd lane's weight, exactly as the ranked lookahead driver does.
        ...
        for x_hi in 0..hi_size {
            ...
            for u in 0..lo_size / 2 {
                ...
                let [a0, a1, b0, b1] = fold_pair(xe);
                let [a2, a3, b2, b3] = fold_pair(xo);
                acc[0] ^= eq_lo[xe].mul_unreduced(a1 * b1);
                acc[1] ^= eq_lo[xe].mul_unreduced((a0 + a1) * (b0 + b1));
                acc[2] ^= eq_lo[xo].mul_unreduced(a3 * b3);
                acc[3] ^= eq_lo[xo].mul_unreduced((a2 + a3) * (b2 + b3));
                let w = eq_lo[xo];
        ...
        let cpu_la: Vec<[F128; 6]> = cpu_split
            ...
            .collect();
        // The scored four-product contract leaves the parked cross slots zero.
        for v in &mut cpu_split {
            v[4..].fill(F128::ZERO);
        }
