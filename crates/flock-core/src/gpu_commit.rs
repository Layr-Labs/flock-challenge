...
// DRAM by a later dispatch. Ping-pong banks keep each level's writes separate
// from its reads, so the level needs one barrier instead of a read barrier plus
// a write barrier.
kernel void parent_hash8(device const uint* children [[buffer(0)]],
                         ...
                         uint lid  [[thread_index_in_threadgroup]])
{
    // Two 4 KiB banks: level 0 is written to bank 0, then levels alternate.
    // The extra 4 KiB buys seven fewer barriers without changing DRAM traffic.
    threadgroup uint cvs[2u * 128u * 8u];
    uint block[16];
    ...
    for (uint level = 1u; level < 8u; level++) {
        ...
        const uint src_base = (level & 1u) ? 0u : 1024u;
        const uint dst_base = src_base ^ 1024u;
        uint next_block[16];
        if (lid < next_len) {
            for (uint i = 0u; i < 16u; i++) {
                next_block[i] = cvs[src_base + lid * 16u + i];
            }
        }
        if (lid < next_len) {
            uint next_cv[8];
            ...
            for (uint i = 0; i < 8u; i++) {
                ...
                cvs[dst_base + lid * 8u + i] = next_cv[i];
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        local_len = next_len;
