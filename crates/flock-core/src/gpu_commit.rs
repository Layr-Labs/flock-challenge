...
// Single-dispatch parent tail: ONE threadgroup consumes the remaining L <= 64
...
// launch. Node indices are absolute (the tree buffer is bound at offset 0).
struct TailParams {
    uint child_start;   // absolute node index of the first child
    uint n_children;    // power of two, 2..=64
    uint out_start[6];  // absolute node index of each output level's slot 0
};
...
kernel void parent_hash_tail(device uint* tree      [[buffer(0)]],
                             ...
                             uint lid [[thread_index_in_threadgroup]])
{
    threadgroup uint stage[2][32u * 8u];
    uint len = T.n_children;
    uint level = 0u;
    uint src_sel = 0u;

    // Level 0 reads the children from device memory.
    {
        const uint n_out = len >> 1;
        if (lid < n_out) {
            uint block[16];
            device const uint* src = tree + (T.child_start + 2u * lid) * 8u;
            for (uint i = 0u; i < 16u; i++) block[i] = src[i];
            uint cv[8];
            for (uint i = 0u; i < 8u; i++) cv[i] = B3_IV[i];
            b3_compress(cv, block, 64u, B3_PARENT);
            device uint* dst = tree + (T.out_start[0] + lid) * 8u;
            for (uint i = 0u; i < 8u; i++) {
                dst[i] = cv[i];
                stage[0][lid * 8u + i] = cv[i];
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        len = n_out;
...
"#;
    ...
    unsafe fn encode_parent_tail(
        ...
        debug_assert!((2..=64).contains(&local_len));
    ...
    pub(crate) unsafe fn encode_merkle_subtree_impl(
        ...
        unsafe {
            ...
            let floor = if parent_tail { 64 } else { 1 };
