    ...
    uint lid  [[thread_index_in_threadgroup]])
{
    ...
    // Keep threadgroup footprint at the incumbent size (nib + red only). A
    // prior attempt staged folded rows in TG for the group products and
    // blew past the usable local-memory budget on the ranked M3 Max
    // (−38% official). Even lids re-fold the odd pair from device memory
    // instead; the table is hot in cache from the odd lid's own pass.
    for (uint k = 0u; k < p.xpt; k++) {
        ...
        // Round-three group products. Even lids own the group (x_lo, x_lo+1)
        // with weight = eq_lo[odd], matching the CPU lookahead monomorphization.
        if ((x_lo & 1u) == 0u) {
            uint x_odd = x_lo + 1u;
            uint pair_odd = tgid * p.lo_size + x_odd;
            bool pad_odd = (pair_odd & p.mask) >= p.useful;
            if (!(pad && pad_odd)) {
                uint4 a2 = uint4(0u);
                uint4 a3 = uint4(0u);
                uint4 b2 = uint4(0u);
                uint4 b3 = uint4(0u);
                if (!pad_odd) {
                    uint4 ar1 = a_in[pair_odd];
                    uint4 br1 = b_in[pair_odd];
                ...
                uint4 a0w = pad ? uint4(0u) : gf_reduce(clmul128(wt, a0));
                uint4 a1w = pad ? uint4(0u) : gf_reduce(clmul128(wt, a1));
                uint4 a2w = pad_odd ? uint4(0u) : gf_reduce(clmul128(wt, a2));
                uint4 a3w = pad_odd ? uint4(0u) : gf_reduce(clmul128(wt, a3));
                if (!pad_odd) {
