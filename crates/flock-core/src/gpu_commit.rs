...
"#;
    ...
    /// chunks stream into an atomic published counter; the helper pool drains
    /// it while the NTT threads keep publishing and then join the queue.
    /// The suffix is on the commit-phase critical path, so engaging E-cores
    /// here shortens the join directly. Self-calibrating on the first commit
    /// in the process by replaying the suffix with the offload forced and
    /// comparing suffix walls (byte-identical rewrite, so the replay is free
    /// of correctness risk).
    ...
    fn suffix_offload_active() -> bool {
        use std::sync::atomic::Ordering;
        match suffix_offload_env_override() {
            Some(v) => return v,
            None => {}
        }
    ...
    fn publish_suffix_offload(wall_off_ms: f64, wall_on_ms: f64) {
        use std::sync::atomic::Ordering;
        // Absolute 1.0 ms noise floor on the suffix wall.
    ...
    unsafe fn run_commit_graph_from_z_hybrid_impl(
        ...
        unsafe {
            ...
            let r = (|| {
                ...
                let suffix_off = suffix_offload_active();
                let mut run_suffix = |off: bool| {
                ...
                let t_graph0 = std::time::Instant::now();
                run_suffix(suffix_off);
                ...
                finish_top()?;
                let wall_off_graph = t_graph0.elapsed().as_secs_f64() * 1e3;

                if suffix_offload_calibration_pending() {
                    // Replay the WHOLE graph with the offload forced: a fresh
                    // byte-identical cb2 plus the suffix rewrite (leaves and
                    // parents are rewritten bit-identically, no allocations),
                    // so both arms' walls are max(gpu, suffix) under live
                    // contention.
                    let cb2b = encode_hybrid_prefix_cb2(
                        gpu, staging, tw_buf, tree_buf, log_d, n_leaves, k_cpu16,
                    )?;
                    gpu.commit_async(cb2b);
                    let t1 = std::time::Instant::now();
                    run_suffix(true);
                    gpu.wait_cb(cb2b)?;
                    finish_top()?;
                    let wall_on_graph = t1.elapsed().as_secs_f64() * 1e3;
                    publish_suffix_offload(wall_off_graph, wall_on_graph);
                }
                Ok(())
            })();
            gpu.pool_pop(pool);
