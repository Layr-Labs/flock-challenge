...
/// Strict kill switch for queueing the pure-GPU continuation directly behind
/// the final streamed from-z tile. Only exact value `1` restores the incumbent
/// finish-time encode/commit path. The queued command buffer contains the same
/// layers 4+ and Merkle work and executes in the same serial Metal queue.
...
"#;
    ...
    pub(crate) struct FromZFirstPassStream {
        ...
        /// Continuation command buffer (retained) committed directly behind
        /// the final streamed tile, with the split it was encoded for. Queue
    ...
    impl FromZFirstPassStream {
        pub(crate) fn submit_ready_range(&mut self, r_start: usize, r_count: usize) {
            ...
            // Final tile queued: encode the selected continuation now and
            // commit it directly behind that tile on the same (serial) queue.
            // The GPU then flows from the last first-pass tile straight into
            // the remaining graph with no host round-trip, and `finish` skips
            // the encode on the critical path. Bit-identical: the encoded
            // work is exactly what `finish` would have encoded.
                ...
                && self.next_r == total_r
            {
                ...
                let result = unsafe {
                    let pool = self.gpu.pool_push();
                    let result = (|| -> Result<Id, String> {
                        let cb2 = if early_k == 0 {
                            encode_commit_graph_after_from_z_cb(
                                self.gpu,
                                self.staging,
                                self.tw_buf,
                                self.tree_buf,
                ...
                match result {
                    Ok(cb2) => {
                        ...
                        if debug_enabled() {
                            eprintln!(
                                "[gpu-commit] early GPU continuation committed behind final tile (k={early_k})"
                            );
                        }
                    }
                    ...
                    Err(e) => {
                        if debug_enabled() {
                            eprintln!("[gpu-commit] early GPU continuation encode failed ({e})");
                        }
                    }
                }
            }
