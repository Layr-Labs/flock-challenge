...
"#;
    ...
    /// Bind a caller-owned ranked tree directly to Metal when the allocator
    /// happened to provide the page alignment required by
    /// `newBufferWithBytesNoCopy`.  The normal Vec remains the owner; the
    /// temporary Metal wrapper is released only after the command buffer has
    /// completed.
    unsafe fn wrap_host_tree(gpu: &Gpu, tree: &mut Vec<Hash>) -> Option<Id> {
        let bytes = tree
            .capacity()
            .checked_mul(core::mem::size_of::<Hash>())?;
        if bytes == 0 || bytes % 16_384 != 0 {
            return None;
        }
        unsafe { gpu.wrap_buffer(tree.as_mut_ptr().cast::<u8>(), bytes).ok() }
    }

    /// Run one commit graph and materialize its tree in `tree`.  A page-aligned
    /// host allocation is used as the graph's tree target, eliminating the
    /// 64 MiB shared-buffer copy-out; unaligned allocations retain the
    /// persistent GPU-buffer fallback.
    unsafe fn run_graph_to_host_tree(
        gpu: &Gpu,
        z_buf: Id,
        staging: Id,
        tw_buf: Id,
        fallback_tree_buf: Id,
        log_d: usize,
        n_leaves: usize,
        fuse_leaf: bool,
        tree: &mut Vec<Hash>,
    ) -> Result<(), String> {
        let direct = unsafe { wrap_host_tree(gpu, tree) };
        let direct_ok = direct.is_some();
        let target = direct.unwrap_or(fallback_tree_buf);
        let result = unsafe {
            run_commit_graph_from_z(
                gpu, z_buf, staging, tw_buf, target, log_d, n_leaves, fuse_leaf,
            )
        };
        ...
        if !direct_ok {
            unsafe {
                copy_bytes_parallel(
                    gpu.buffer_contents(fallback_tree_buf),
                    core::slice::from_raw_parts_mut(
                        tree.as_mut_ptr().cast::<u8>(),
                        (2 * n_leaves - 1) * core::mem::size_of::<Hash>(),
                    ),
                );
            }
        }
