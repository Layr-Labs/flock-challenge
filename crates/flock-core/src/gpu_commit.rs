...
/// the CPU AB completion (~9.7 ms on the M4 calibration record) while the
/// GPU folds the C claims under it; this arm folds the precomputed AB inner
/// state (16 rows x 64 lanes per 1 KiB chunk) on the GPU through the
/// F2-linear pre-scaled convert tables (nibble tables, XOR-only — no PMULL),
/// with a warmup calibration that byte-compares the GPU's band partials
/// against the CPU's before any share is published. Any failure, mismatch,
/// or kill switch keeps the exact incumbent CPU completion.
pub const ENV_NO_GPU_ZC_AB: &str = "FLOCK_NO_GPU_ZC_AB";

pub(crate) fn zc_ab_gpu_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> =
        std::sync::LazyLock::new(|| std::env::var_os(ENV_NO_GPU_ZC_AB).is_none());
    *ON
}
...
"#;
    ...
    // `eq_hi[band]` folds.
    //
    // Bit-exactness: identical argument to zc-r2 — the F2-linear nibble fold
    // reproduces the same XOR chain, and the warmup calibration refuses to
    // publish a share until the GPU's band partials compare equal to the
    // CPU's on the target machine itself (any mismatch leaves the arm off
    // for the process; the CPU path is authoritative).
    // -----------------------------------------------------------------------
...
"#;
    ...
    unsafe fn zc_ab_submit(
        ...
        unsafe {
            ...
            gpu.dispatch(enc, (bands * banks * 64) as u64, 256);
            gpu.end_encoding(enc);
            let cb = gpu.retain(cb);
            gpu.commit_async(cb);
            Ok(cb)
    ...
    pub(crate) fn zc_ab_wait(
        ...
        unsafe {
            ...
            if std::env::var_os("FLOCK_ZC_AB_GPU_DEBUG").is_some() {
                eprintln!(
                    "[zc-ab] bands={} submit-to-drain={:.2}ms",
                    job.bands,
                    job.submitted.elapsed().as_secs_f64() * 1e3
                );
            }
            ...
            if job.calibration {
                ...
                if ok {
                    ...
                    // Kernel-correctness gate: byte mismatch means the arm is
                    // wrong for this process's data; disable it permanently.
                    ZC_AB_TUNED.store(0, Ordering::Relaxed);
                }
