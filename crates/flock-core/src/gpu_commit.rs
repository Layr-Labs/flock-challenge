...
/// layer-4 pass. Four field lanes share each 64-thread group and the complete
/// 256-position tile remains in threadgroup memory until the prefix is done.
...
impl FromZFirstPassStream {
    /// Whether publication must cover all sixteen radix-16 source stripes for
    /// each fused radix-256 tile.
    #[doc(hidden)]
    pub fn uses_radix256_layout(&self) -> bool {
        self.inner.uses_radix256_layout()
    }
...
"#;
    ...
    pub(crate) struct Gpu {
        ...
        /// Threadgroup-tiled radix-256 from-z prefix and its compact 255 x 256
        /// F128 multiplication image.
        ...
        unsafe fn encode_range(
            ...
            unsafe {
                if self.radix256 {
                    ...
                    // Callers publish ranges in the f=4 r-space.
                    // Sixteen adjacent published r values are one f=8 tile.
    ...
    impl FromZFirstPassStream {
        /// The fused radix-256 prefix consumes sixteen radix-16 `r` stripes
        /// for each output tile. Witness producers use this to publish bands
        /// in tile-ready rather than flat-radix-16 order.
        pub(crate) fn uses_radix256_layout(&self) -> bool {
            FromZFirstPassPlan::new(self.gpu, self.log_d).radix256
        }
        ...
        pub(crate) fn submit_ready_range(&mut self, r_start: usize, r_count: usize) {
            ...
            let plan = FromZFirstPassPlan::new(self.gpu, self.log_d);
    ...
    /// Validation replay for a supplemental first pass. It uses the ranked
    /// witness publisher's exact tapered range geometry, including every
    /// nonzero buffer offset, before running the ordinary suffix and Merkle
    /// graph. This runs only in the untimed warm-up dual-run.
    unsafe fn run_commit_graph_from_z_streamed_validation(
        gpu: &Gpu,
        z_buf: Id,
        staging: Id,
        tw_buf: Id,
        ...
        unsafe {
            ...
            debug_assert_eq!(r_start, 1usize << (log_d - 4));
        ...
        unsafe {
            ...
            let r = (|created: &mut Vec<Id>| -> Result<WarmupRun, String> {
                ...
                if FromZFirstPassPlan::new(gpu, log_d).radix256 {
                    run_commit_graph_from_z_streamed_validation(
    ...
    fn warmup_and_decide(
        ...
        // dual-run unchanged.
        // A supplemental full-shape prefix must first match the complete CPU
        // codeword and tree on this Metal device. The static latch checks only
        // deterministic replay, so bypass it for this kernel and use the full
        // dual-run below before publishing an ON state.
        if super::static_warmup_latch_enabled()
            && !super::gpu_from_z_radix256_selected(params.k_code())
        {
