                            ...
                            uint lid  [[thread_index_in_threadgroup]])
{
    ...
    if (lid < 16u) {
        ...
        for (uint b = 0; b < 16u; b++) {
          ...
          // Reuse the tile as a local parent frontier after publishing the leaf.
          tile[lid * 2u] = uint4(cv[0], cv[1], cv[2], cv[3]);
          tile[lid * 2u + 1u] = uint4(cv[4], cv[5], cv[6], cv[7]);
      }
      threadgroup_barrier(mem_flags::mem_threadgroup);

      // Materialize the first four parent levels while the 16 leaves remain in
      // threadgroup memory. The flat offsets match parent_hash8; later levels
      // start at the level-four frontier and retain every opening node.
      uint local_len = 16u;
      uint level_offset = 1u << P.log_d;
      #pragma clang loop unroll(full)
      for (uint level = 1u; level <= 4u; level++) {
          const uint next_len = local_len >> 1u;
          uint block[16];
          if (lid < next_len) {
              const uint4 left0 = tile[lid * 4u];
              const uint4 left1 = tile[lid * 4u + 1u];
              const uint4 right0 = tile[lid * 4u + 2u];
              const uint4 right1 = tile[lid * 4u + 3u];
              block[0] = left0.x;  block[1] = left0.y;
              block[2] = left0.z;  block[3] = left0.w;
              block[4] = left1.x;  block[5] = left1.y;
              block[6] = left1.z;  block[7] = left1.w;
              block[8] = right0.x; block[9] = right0.y;
              block[10] = right0.z; block[11] = right0.w;
              block[12] = right1.x; block[13] = right1.y;
              block[14] = right1.z; block[15] = right1.w;
          }
          threadgroup_barrier(mem_flags::mem_threadgroup);
          if (lid < next_len) {
              uint next_cv[8];
              for (uint i = 0u; i < 8u; i++) next_cv[i] = B3_IV[i];
              b3_compress(next_cv, block, 64u, B3_PARENT);
              const uint out_id = level_offset + tgid * next_len + lid;
              for (uint i = 0u; i < 8u; i++) tree[out_id * 8u + i] = next_cv[i];
              tile[lid * 2u] = uint4(next_cv[0], next_cv[1], next_cv[2], next_cv[3]);
              tile[lid * 2u + 1u] = uint4(next_cv[4], next_cv[5], next_cv[6], next_cv[7]);
          }
          threadgroup_barrier(mem_flags::mem_threadgroup);
          local_len = next_len;
          level_offset += (1u << P.log_d) >> level;
      }
    }
}
...
"#;
    ...
    /// than eight levels use the single-level kernel.
      pub(crate) unsafe fn encode_merkle_parents(
          ...
          encode_merkle_parents_from_level(gpu, enc, tree_buf, n_leaves, 0);
      }

      /// Continue parent reduction from an already-materialized flat-tree
      /// frontier. The fused leaf kernel uses this to skip rereading its first
      /// four parent levels from device memory.
      pub(crate) unsafe fn encode_merkle_parents_from_level(
          gpu: &Gpu,
          enc: Id,
          tree_buf: Id,
          n_leaves: usize,
    ...
    /// Copy a GPU-produced flat tree back in bounded parallel chunks. The
    /// destination is one uniquely borrowed allocation; using a small fixed
    /// number of disjoint streams keeps the 64 MiB copy off the critical
    /// single-core path without recreating one Rayon task per cache line.
    fn copy_bytes_parallel(src: *const u8, dst: &mut [u8]) {
        use rayon::prelude::*;
        if dst.is_empty() {
            return;
        }
        const COPY_CHUNK: usize = 1 << 24;
        let src_addr = src as usize;
        dst.par_chunks_mut(COPY_CHUNK)
            .enumerate()
            .for_each(|(i, chunk)| {
                // SAFETY: callers guarantee `src` points at least `dst.len()`
                // bytes; chunks are disjoint and each source offset matches its
                // unique destination chunk.
                unsafe {
                    std::ptr::copy_nonoverlapping(
                        (src_addr as *const u8).add(i * COPY_CHUNK),
                        chunk.as_mut_ptr(),
                        chunk.len(),
                    );
                }
            ...
            let r = (|| {
                ...
                if fuse_leaf {
                    ...
                    encode_fused_leaf_pass(gpu, enc, staging, tw_buf, tree_buf, log_d, final_l, 4);
                      encode_merkle_parents_from_level(gpu, enc, tree_buf, n_leaves, 4);
                } else {
