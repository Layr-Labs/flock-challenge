...
// One threadgroup per hi-chunk (256 threads, xpt = lo_size/256 x_lo pairs
...
// unreduced accumulate.
//
// Parity-split reduce for the round-two products: stop one step short so the
// even and odd x_lo halves stay apart. Separately, even lids also own the
// round-three group products W0/W3/W4/W5 over the pair-of-pairs
// `(x_lo, x_lo+1)`, weighted by the odd lane's `eq_lo` exactly as the CPU
// lookahead kernel does. Thread 0 writes eight REDUCED eq_hi-weighted
// partials per chunk:
//   [p1_even, pinf_even, p1_odd, pinf_odd, W0, W3, W4, W5]
// Slot0^slot2 / slot1^slot3 are the CPU's per-chunk `(eq_hi*p1, eq_hi*pinf)`;
// slots 2..7 are its full six-slot lookahead state (W1/W2 = odd p1/pinf).
    ...
    uint lid  [[thread_index_in_threadgroup]])
{
    ...
    // Per-pair folded rows live in threadgroup memory so the even lid of each
    // group can form W0/W3/W4/W5 without re-folding the odd pair (the odd lid
    // already folded it for the round-two products).
    threadgroup uint4 fa0[256];
    threadgroup uint4 fa1[256];
    threadgroup uint4 fb0[256];
    threadgroup uint4 fb1[256];
    threadgroup uint pad_tg[256];
    for (uint k = 0u; k < p.xpt; k++) {
        ...
        fb0[lid] = b0;
        fb1[lid] = b1;
        pad_tg[lid] = pad ? 1u : 0u;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Round-three group products. Even lids own the group (lid, lid+1)
        // with weight = eq_lo[odd], matching the CPU lookahead monomorphization.
        // Folded rows come from the barrier above — no second table pass.
        if ((lid & 1u) == 0u) {
            uint pad_even = pad_tg[lid];
            uint pad_odd = pad_tg[lid + 1u];
            if (!(pad_even && pad_odd)) {
                uint4 a0g = fa0[lid];
                uint4 a1g = fa1[lid];
                uint4 b0g = fb0[lid];
                ...
                uint4 b3 = fb1[lid + 1u];
                uint x_odd = x_lo + 1u;
                uint4 wt = eq_lo[x_odd];
                uint4 a0w = pad_even != 0u ? uint4(0u) : gf_reduce(clmul128(wt, a0g));
                uint4 a1w = pad_even != 0u ? uint4(0u) : gf_reduce(clmul128(wt, a1g));
                uint4 a2w = pad_odd != 0u ? uint4(0u) : gf_reduce(clmul128(wt, a2));
                uint4 a3w = pad_odd != 0u ? uint4(0u) : gf_reduce(clmul128(wt, a3));
                if (pad_odd == 0u) {
                    U256k mw0 = clmul128(a2w, b2);
                    acc_w0 ^= ulong4(mw0.r0, mw0.r1, mw0.r2, mw0.r3);
                }
                uint4 e_aw = a0w ^ a2w;
                uint4 o_aw = a1w ^ a3w;
                uint4 e_b  = b0g ^ b2;
                ...
                acc_w3 ^= ulong4(mw3.r0, mw3.r1, mw3.r2, mw3.r3);
                acc_w4 ^= ulong4(mw4.r0, mw4.r1, mw4.r2, mw4.r3);
                acc_w5 ^= ulong4(mw5.r0, mw5.r1, mw5.r2, mw5.r3);
            }
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    // Parity-split reduce for p1 / pinf. `x_lo = k*256 + lid` and 256 is even,
    // so a thread only ever visits x_lo of a single parity. Every halving step
    // from 128 down to 2 pairs equal-parity indices; only the discarded
    // `s == 1` step would mix them.
    red[lid] = acc1;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    // Full reduce for the four group-product accumulators (even lids only
    // contribute; odd lids hold zero).
    ulong4 cw0;
    ulong4 cw3;
    ulong4 cw4;
    ulong4 cw5;
    ...
    if (lid == 0u) {
        ...
        U256k uw0; uw0.r0 = cw0.x; uw0.r1 = cw0.y; uw0.r2 = cw0.z; uw0.r3 = cw0.w;
        U256k uw3; uw3.r0 = cw3.x; uw3.r1 = cw3.y; uw3.r2 = cw3.z; uw3.r3 = cw3.w;
        U256k uw4; uw4.r0 = cw4.x; uw4.r1 = cw4.y; uw4.r2 = cw4.z; uw4.r3 = cw4.w;
        U256k uw5; uw5.r0 = cw5.x; uw5.r1 = cw5.y; uw5.r2 = cw5.z; uw5.r3 = cw5.w;
        uint4 e = eq_hi[tgid];
...
"#;
    ...
    pub(crate) enum ZcR2Result {
        ...
        Calibrated,
        /// Timed-prove prefix partials, bit-exact per chunk, all eq_hi-weighted:
        /// `[p1_even, pinf_even, p1_odd, pinf_odd, W0, W3, W4, W5]`.
        /// Slots 0..3 are the parity-split round-two products; slots 4..7 are
        /// the four group products the CPU lookahead kernel used to recompute
        /// on every offloaded chunk.
        Prefix(Vec<[F128; 8]>),
    ...
    pub(crate) fn launch_zc_r2_products(
        ...
        unsafe {
            ...
            // four group products `[W0, W3, W4, W5]`.
    ...
    /// aggregates `[p1_odd, pinf_odd, W0, W3, W4, W5]` so the oracle can check
    /// the parity split **and** the four group products, not merely the sum.
    pub(crate) fn zc_r2_wait(
        ...
        unsafe {
            ...
            // Checked: (1) the summed round-two pair, (2) the odd-parity half
            // (W1/W2), and (3) the four group products W0/W3/W4/W5 whenever
            // `cpu_la` is provided with those slots populated.
            for c in 0..job.chunks {
...
"#;
    ...
    fn gpu_zc_r2_products_match_cpu_oracle() {
        ...
        // CPU reference partials: parity-split round-two products (per-pair
        // eq_lo weight, matching the GPU) plus the four group products
        // (odd-lane weight, matching the CPU lookahead monomorphization).
        ...
        for x_hi in 0..hi_size {
            ...
            for u in 0..lo_size / 2 {
                let x_even = 2 * u;
                let x_odd = x_even + 1;
                let pair_even = x_hi * lo_size + x_even;
                let pair_odd = x_hi * lo_size + x_odd;
                let pad_even = (pair_even & mask) >= useful;
                let pad_odd = (pair_odd & mask) >= useful;
                if pad_even && pad_odd {
                    continue;
                }
                let fold_pair = |pair_idx: usize, pad: bool| {
                    if pad {
                ...
                let (a0, a1, b0, b1) = fold_pair(pair_even, pad_even);
                let (a2, a3, b2, b3) = fold_pair(pair_odd, pad_odd);
                let wt = eq_lo[x_odd];
        ...
        let cpu_partials: Vec<(F128, F128)> = cpu_split
            ...
            .collect();
        // Full six-slot lookahead state: odd-parity pair + four group products.
