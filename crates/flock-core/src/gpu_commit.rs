...
pub const ENV_NO_ZC_R2_W_OFFLOAD: &str = "FLOCK_NO_ZC_R2_W_OFFLOAD";

pub(crate) fn zc_r2_w_offload_enabled() -> bool {
    static ON: std::sync::LazyLock<bool> = std::sync::LazyLock::new(|| {
        std::env::var_os(ENV_NO_ZC_R2_W_OFFLOAD).is_none_or(|v| v != *"1")
...
pub const ENV_NO_ZC_R2_RETUNE: &str = "FLOCK_NO_ZC_R2_RETUNE";
...
pub(crate) fn zc_r2_retune_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| zc_r2_retune_value_enabled(std::env::var_os(ENV_NO_ZC_R2_RETUNE).as_deref()))
}
...
pub const ENV_NO_ZC_R2_RECONVERGE: &str = "FLOCK_NO_ZC_R2_RECONVERGE";
...
pub(crate) fn zc_r2_reconverge_enabled() -> bool {
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        zc_r2_reconverge_value_enabled(std::env::var_os(ENV_NO_ZC_R2_RECONVERGE).as_deref())
    })
}
...
mod zc_r2_retune_gate_tests {
    ...
    #[test]
    fn exact_one_is_the_only_retune_and_reconverge_kill_value() {
...
"#;
    ...
    impl ZcR2Job {
        ...
        /// W-aggregates (see [`super::ENV_NO_ZC_R2_W_OFFLOAD`]).
