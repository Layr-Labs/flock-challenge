                                  ...
                                  uint lid  [[thread_index_in_threadgroup]])
{
    ...
    // j = 0 (bpos = 3): every butterfly pairs a live element (e < 8) with a
    // zero element (e >= 8): (u, 0) -> (u, u) for any twiddle. Pure copy; no
    // table multiplies at all. Identical arithmetic; 8 of 32 muls deleted.
    for (uint b = 0; b < NF / 2u; b++) {
        elems[b + NF / 2u] = elems[b];
    }
    for (uint j = 1; j < F; j++) {
        uint bpos = F - 1u - j;
        for (uint b = 0; b < (NF >> 1); b++) {
