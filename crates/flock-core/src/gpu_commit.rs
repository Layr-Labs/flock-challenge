...
/// Opt-in gate for the GPU NTT of the recursive Ligerito L1 commitment
/// inside the PCS open: set `FLOCK_OPEN_L1_GPU=1` (exact `1`) to enable.
/// DEMOTED TO OPT-IN by ranked evidence (submissions 217a62ab / 02be153a:
/// two independent draws at −1.26%/−1.29% with p10 +0.84%/+0.88% while
/// same-hour rival draws of the parent tree sat at −0.0..−0.3% — the
/// regression survived removing the other flagged suspects, leaving this
/// arm as the prime suspect by elimination: its warmup byte-oracle proves
/// correctness but is economically blind, and on a 10-P-core runner the CPU
/// L1 NTT is likely cheaper than the GPU roundtrip it latched in). Output is
/// bit-exact either way: the offload reuses the promoted metallib NTT pass
/// kernels under an exact 8-lane -> 64-lane index remap, the per-process
/// warmup byte-oracle latches the GPU path ON only after it reproduces the
/// incumbent CPU codeword bit-for-bit on THIS machine, and any refusal or
/// Metal failure falls back to the untouched CPU dispatch.
pub const ENV_OPEN_L1_GPU: &str = "FLOCK_OPEN_L1_GPU";

fn gpu_open_l1_value_enabled(value: Option<&std::ffi::OsStr>) -> bool {
    value == Some(std::ffi::OsStr::new("1"))
}

/// Test-only force-on latch so the byte-oracle tests can exercise the
/// (default-off) opt-in arm directly without racing process env state.
#[cfg(test)]
pub(crate) static OPEN_L1_TEST_FORCE_ON: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);

pub(crate) fn gpu_open_l1_enabled() -> bool {
    #[cfg(test)]
    if OPEN_L1_TEST_FORCE_ON.load(std::sync::atomic::Ordering::Relaxed) {
        return true;
    }
    static ON: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *ON.get_or_init(|| {
        gpu_open_l1_value_enabled(std::env::var_os(ENV_OPEN_L1_GPU).as_deref())
    })
}
...
mod tests {
    ...
    fn gpu_open_l1_ntt_remap_matches_cpu_small() {
        super::OPEN_L1_TEST_FORCE_ON.store(true, std::sync::atomic::Ordering::Relaxed);
        let num_ntts = 8usize;
