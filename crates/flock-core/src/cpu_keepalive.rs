...
mod imp {
    ...
    fn spin_until_stopped(deadline: Instant) {
        ...
        while RUNNING.load(Ordering::Relaxed) {
            // Keep the batch deliberately short. These threads occupy the
            // same P-cores the timed Rayon proof needs immediately after the
            // seed arrives, so stop latency matters more than the negligible
            // extra private atomic loads during the untimed ready wait. A
            // 64-step dependent chain still keeps every core continuously
            // runnable, but releases it about 16x sooner than the old 1024-op
            // batch after `keepalive_signal` clears RUNNING.
            for _ in 0..64 {
                x = x
