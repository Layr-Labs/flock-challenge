...
/// Wait until every signalled spinner has stopped executing, without joining
/// its OS-thread handle. Unlike ten sequential joins this is a single shared
/// completion barrier; handles can be reaped after proof publication.
pub fn keepalive_quiesce() {
    #[cfg(all(target_arch = "aarch64", target_os = "macos"))]
    imp::quiesce();
}
...
mod imp {
    ...
    /// Number of successfully spawned keep-alive threads that have not yet
    /// exited. Reserved before `spawn`, so a delayed first schedule cannot
    /// make the quiescence barrier return early.
    ...
    /// Wait for execution to cease, but leave handle destruction for an
    /// unscored tail. Acquire pairs with each spinner's release decrement, so
    /// return proves no keep-alive instruction can overlap the real proof.
    ...
    mod tests {
        ...
        /// The measured ordering proves execution has ceased before handles
        /// are reaped in the unscored tail.
        #[test]
        fn signal_quiesce_then_join_is_clean() {
            start();
            signal();
            quiesce();
            assert!(!RUNNING.load(Ordering::SeqCst));
