...
fn compute_combined_basis_and_target<Ch: Challenger>(
    ...
    let (mut round0_u0, mut round0_u2) = if use_fast {
        ...
        // The round-0 coefficients are dot products. GHASH reduction is
        // F2-linear, so reduce once after the parallel XOR instead of once
        // for every witness/basis product.
        (u0.reduce(), u2.reduce())
    } else {
