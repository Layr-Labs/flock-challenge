...
fn batch_inverse_in_place(values: &mut [F128]) {
    ...
    values.par_chunks_mut(CHUNK).for_each_init(
        ...
        |original, chunk| {
            ...
            for (o, v) in chunk.iter_mut().zip(original.iter()) {
                debug_assert!(!v.is_zero(), "batch_inverse: zero input");
                *o = acc; // prefix product within this chunk
                acc *= *v;
            }
            acc = acc.inv(); // (∏ chunk)⁻¹
            for (o, v) in chunk.iter_mut().zip(original.iter()).rev() {
                *o *= acc;
                acc *= *v;
            }
        },
...
pub fn prove<C: Challenger>(
    ...
    // Materializing the four zerocheck views is independent.  Keep the existing
    // parallel strided gathers for `a`/`b`, and overlap them with the two
    // contiguous copies instead of serializing all four proof-scale passes.
    let ((a, b), (c, v0)) = rayon::join(
        || {
            rayon::join(
                || (0..n).into_par_iter().map(|i| v[2 * i]).collect::<Vec<F128>>(),
                || (0..n).into_par_iter().map(|i| v[2 * i + 1]).collect::<Vec<F128>>(),
            )
        },
        || rayon::join(|| v[n..2 * n].to_vec(), || v[..n].to_vec()),
    );
