...
fn hash_leaves(data: &[u8], leaf_size: usize, out: &mut [Hash], kind: HashKind) {
    ...
    match kind {
        ...
        HashKind::Sha256 => {
            ...
            const SHA256_LEAVES_PER_CHUNK: usize = 128; // 32 four-way quads
            let n = out.len();
            let n_chunks = n / SHA256_LEAVES_PER_CHUNK;
            let tail = n % SHA256_LEAVES_PER_CHUNK;
            if sha256_hetero && crate::epool::epool().is_some() && n_chunks >= 16 {
                let out_base = crate::epool::SyncPtr(out.as_mut_ptr());
    ...
    match kind {
        ...
        HashKind::Sha256 => {
            ...
            if serial {
                ...
                // Gate at the epool's own engagement floor (EPOOL_MIN_CHUNKS):
                // levels below it would post a drain the helper skips anyway.
                if sha256_hetero && crate::epool::epool().is_some() && n_chunks >= 16 {
                    let out_base = crate::epool::SyncPtr(write.as_mut_ptr());
                    let read_base = read_bytes.as_ptr() as usize;
                    crate::epool::run_hetero_chunks(n_chunks, |i| {
