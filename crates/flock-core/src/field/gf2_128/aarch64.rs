...
pub unsafe fn ghash_mul_unreduced_neon(a: F128, b: F128) -> F256Unreduced {
    ...
    unsafe {
        let p_ll = pmull(a.lo, b.lo);
        let p_hh = pmull(a.hi, b.hi);
        // Karatsuba: the middle coefficient is
        // (a_lo+a_hi)(b_lo+b_hi) + a_lo*b_lo + a_hi*b_hi. This saves one
        // PMULL versus the schoolbook pair of cross products; the unreduced
        // worker accumulator exposes enough independent products to hide the
        // extra XOR dependency.
        let p_sum = pmull(a.lo ^ a.hi, b.lo ^ b.hi);
        let cross = veorq_u64(p_sum, veorq_u64(p_ll, p_hh));
