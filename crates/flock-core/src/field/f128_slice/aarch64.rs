...
pub(super) unsafe fn fold_banked_slots2<const BANKS: usize>(
    ...
    unsafe {
        ...
        while bank < main {
            ...
            xor_karatsuba_const_pair(&mut a0, &mut b0, x00, x10, w0);
            xor_karatsuba_const_pair(&mut a1, &mut b1, x01, x11, w1);
            first.ll = xor3_u64(first.ll, a0.ll, a1.ll);
            first.hh = xor3_u64(first.hh, a0.hh, a1.hh);
            first.mm = xor3_u64(first.mm, a0.mm, a1.mm);
            second.ll = xor3_u64(second.ll, b0.ll, b1.ll);
            second.hh = xor3_u64(second.hh, b0.hh, b1.hh);
            second.mm = xor3_u64(second.mm, b0.mm, b1.mm);
            bank += 2;
        }
        if bank < BANKS {
...
// Literal incumbent control for reproducible arithmetic diagnostics.
#[cfg(test)]
#[inline]
#[target_feature(enable = "aes")]
pub(super) unsafe fn fold_banked_slots2_baseline<const BANKS: usize>(
    weight: &[F128; BANKS],
    input: &[F128],
) -> [F128; 2] {
    unsafe {
...
pub(super) unsafe fn fold_two_and_msg_with_deferred_basis_and_scaled_local_addend(
    ...
    unsafe {
        ...
        // The ranked caller supplies one `eq_lo`-sized output pair: 2,048
        // entries per chunk at L1/L2 and 1,024 at L3. Scan the just-written
        // 64 KiB or 32 KiB working set while it is cache-hot, keeping the four
        // wide message accumulators out of the substantially higher-pressure
        // corrected-fold loop above.
        let zero = vdupq_n_u64(0);
        let mut u0 = WideNeon { lo: zero, hi: zero };
        let mut u2 = WideNeon { lo: zero, hi: zero };
        let mut t = 0usize;
