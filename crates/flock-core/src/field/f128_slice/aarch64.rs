...
pub(super) unsafe fn fold2_two_and_msgs(
    ...
    unsafe {
        ...
        // a_u2a = c2 (also u_2 term 1), a_u2b = u_2 term 2, plus c1/c3/c4/c5.
        ...
        while t < wf.len() {
            ...
            // c1 = w0f*(s0b) ^ w0b*(s0f)   (ad + bc form)
            xor_wide(&mut a_c1, mul_unreduced(w_regs_f[0], s0b));
            xor_wide(&mut a_c1, mul_unreduced(w_regs_b[0], s0f));
            ...
            xor_wide(&mut a_c4, mul_unreduced(e_f, se_b));
            xor_wide(&mut a_c4, mul_unreduced(e_b, se_f));
        ...
        (u_0, u_2, [c0, red(a_c1), c2v, red(a_c3), red(a_c4), red(a_c5)])
    }
}

/// Ticket-14 NEON bootstrap: single bind + lookahead coefficients.
///
/// # Safety
/// Caller guarantees PMULL and: `f.len() == b.len()`, `nf.len() == nb.len()`,
/// `nf.len() % 4 == 0`, `base % 4 == 0`, and `2 * (base + nf.len()) <= f.len()`.
pub(super) unsafe fn fold1_two_and_lookahead(
    f: &[F128],
    b: &[F128],
    base: usize,
    nf: &mut [F128],
    ...
    unsafe {
        ...
        let mut v_regs_f = [zero; 4];
        let mut v_regs_b = [zero; 4];
        let mut t = 0;
        while t < nf.len() {
            let mut q = 0;
            while q < 4 {
                ...
                let pf = mul_const_vec2(r_q, veorq_u64(fe0, fo0), veorq_u64(fe1, fo1));
                let vf0 = veorq_u64(fe0, pf[0]);
                let vf1 = veorq_u64(fe1, pf[1]);
                let pb = mul_const_vec2(r_q, veorq_u64(be0, bo0), veorq_u64(be1, bo1));
                let vb0 = veorq_u64(be0, pb[0]);
                let vb1 = veorq_u64(be1, pb[1]);

                vst1q_u64(nf.as_mut_ptr().add(t + q).cast::<u64>(), vf0);
                vst1q_u64(nf.as_mut_ptr().add(t + q + 1).cast::<u64>(), vf1);
                vst1q_u64(nb.as_mut_ptr().add(t + q).cast::<u64>(), vb0);
                vst1q_u64(nb.as_mut_ptr().add(t + q + 1).cast::<u64>(), vb1);
                v_regs_f[q] = vf0;
                v_regs_f[q + 1] = vf1;
                v_regs_b[q] = vb0;
                v_regs_b[q + 1] = vb1;
            ...
            let s0f = veorq_u64(v_regs_f[0], v_regs_f[1]);
            let s0b = veorq_u64(v_regs_b[0], v_regs_b[1]);
            let s1f = veorq_u64(v_regs_f[2], v_regs_f[3]);
            let s1b = veorq_u64(v_regs_b[2], v_regs_b[3]);
            xor_wide(&mut a_u0a, mul_unreduced(v_regs_f[0], v_regs_b[0]));
            xor_wide(&mut a_u0b, mul_unreduced(v_regs_f[2], v_regs_b[2]));
            xor_wide(&mut a_u2a, mul_unreduced(s0f, s0b));
            xor_wide(&mut a_u2b, mul_unreduced(s1f, s1b));
            xor_wide(&mut a_c1, mul_unreduced(v_regs_f[0], s0b));
            xor_wide(&mut a_c1, mul_unreduced(v_regs_b[0], s0f));
            let e_f = veorq_u64(v_regs_f[0], v_regs_f[2]);
            let o_f = veorq_u64(v_regs_f[1], v_regs_f[3]);
            let e_b = veorq_u64(v_regs_b[0], v_regs_b[2]);
            let o_b = veorq_u64(v_regs_b[1], v_regs_b[3]);
            let se_f = veorq_u64(e_f, o_f);
            let se_b = veorq_u64(e_b, o_b);
            xor_wide(&mut a_c3, mul_unreduced(e_f, e_b));
            xor_wide(&mut a_c4, mul_unreduced(e_f, se_b));
