...
pub fn replicate_message_fill(codeword: &mut [F128], msg: &[F128]) {
        ...
        && codeword.len() / msg_len >= rayon::current_num_threads().saturating_mul(2)
    {
        ...
        // split once the F128 destination is at least 1 MiB and there are two
        // blocks per worker, keeping tiny Ligerito levels on the cheap serial
        // path.
        codeword
