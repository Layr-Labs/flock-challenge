...
fn transpose_forward_ntt_sparse(
    ...
    // Remaining dense steps, except the final `log_d - log_keep` layers.
    // We only retain the top output of those final transpose butterflies;
    // top = a+b is twiddle-free, so all discarded-bottom multiplications can
    // be replaced by one XOR collapse across the rate-expanded segments.
    let n_threads = rayon::current_num_threads().max(1);
    let final_layers = log_d - log_keep;
    for layer in (final_layers..(log_d - k)).rev() {
        let num_blocks = 1usize << layer;
        let block_size = 1usize << (log_d - layer);
        let bsh = block_size >> 1;
        if num_blocks >= n_threads {
    ...
    if final_layers > 0 {
        let keep_len = 1usize << log_keep;
        let (kept, discarded) = data.split_at_mut(keep_len);
        if final_layers == 1 {
            kept.par_iter_mut()
                .zip(discarded.par_iter())
                .for_each(|(a, &b)| *a += b);
        } else {
            for segment in discarded.chunks_exact(keep_len) {
                kept.par_iter_mut()
                    .zip(segment.par_iter())
                    .for_each(|(a, &b)| *a += b);
            }
        }
        data.truncate(keep_len);
    }
    data
}
...
/// ARM chunk kernel for [`fold_and_msg_lsb`]. Folds two adjacent output
/// values from each input with the existing two-lane PMULL primitive, stores
/// them, and consumes the same register values for the next message before
/// advancing. This avoids immediately rereading both freshly written arrays.
///
/// # Safety
/// Requires the `aes` target feature. `f` and `b` must contain the `2 *
/// (base + out.len())` source range; `fc`, `bc` must have equal even length.
#[cfg(target_arch = "aarch64")]
#[target_feature(enable = "aes")]
unsafe fn fold_and_msg_lsb_neon_chunk(
    f: &[F128],
    b: &[F128],
    base: usize,
    fc: &mut [F128],
    ...
    debug_assert_eq!(fc.len() & 3, 0);
    let mut u0_a = F128::ZERO;
    let mut u0_b = F128::ZERO;
    let mut u2_a = F128::ZERO;
    let mut u2_b = F128::ZERO;
    let mut k = 0;
    macro_rules! fold_message_pair {
        ($offset:expr, $u0:ident, $u2:ident) => {{
            let out = k + $offset;
            let s = 2 * (base + out);
            let fe0 = f[s];
            let fo0 = f[s + 1];
            ...
            let fp = unsafe { ghash_mul_vec2_neon([r, r], [fe0 + fo0, fe1 + fo1]) };
            let bp = unsafe { ghash_mul_vec2_neon([r, r], [be0 + bo0, be1 + bo1]) };
            let f0 = fe0 + fp[0];
            let f1 = fe1 + fp[1];
            let b0 = be0 + bp[0];
            let b1 = be1 + bp[1];
    ...
    while k < fc.len() {
        fold_message_pair!(0, u0_a, u2_a);
        fold_message_pair!(2, u0_b, u2_b);
        k += 4;
    }
    (u0_a + u0_b, u2_a + u2_b)
}
...
fn fold_and_msg_lsb(f: &[F128], b: &[F128], r: F128) -> (Vec<F128>, Vec<F128>, SumcheckMessage) {
    ...
    let (u_0, u_2) = nf
        ...
        .map(|(ci, (fc, bc))| {
            let base = ci * CHUNK;
            #[cfg(target_arch = "aarch64")]
            // SAFETY: power-of-two chunk geometry gives an even output slice
            // and two in-bounds source values per output. The helper carries
            // its AES target feature.
            let (u0, u2) = unsafe { fold_and_msg_lsb_neon_chunk(f, b, base, fc, bc, r) };
