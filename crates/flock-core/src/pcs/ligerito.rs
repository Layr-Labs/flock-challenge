...
///
/// Mechanism isolated by Youssef23Youssef (`2967b5f`). Composed onto fuse-init.
...
/// `s=a+b; top=s; bot=t·s+b`, applied in **reverse** layer order. Three
/// adjacent layers are fused so each eight-value row group crosses memory
...
fn transpose_forward_ntt_sparse(
    ...
    // Same three-layer fusion as the dense transpose.
    let n_threads = rayon::current_num_threads().max(1);
...
mod tests {
    ...
    /// Fused three-layer transpose matches the serial one-layer reference.
    #[test]
    fn transpose_fused_matches_single_layer_reference() {
        ...
        let mut rnd = || {
            state = state
                .wrapping_mul(6364136223846793005)
                .wrapping_add(1);
            F128::new(state, state.rotate_left(13))
        };
        for &log_d in &[3usize, 4, 5, 8, 12] {
            let ntt = AdditiveNttF128::standard(log_d);
            let mut expected: Vec<F128> = (0..(1usize << log_d)).map(|_| rnd()).collect();
            let mut actual = expected.clone();
            transpose_single_layer_reference(&ntt, &mut expected, log_d);
            super::transpose_forward_ntt(&ntt, &mut actual, log_d);
