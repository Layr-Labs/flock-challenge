...
fn materialize_direct_fold8(
    ...
    // Four adjacent slots can share each 64-entry weight load while keeping
    // their product sums independent. Exact `=1` restores the pair kernel
    // above, giving the candidate a same-binary A/B control.
    let quad_fold64 = deferred_reduce
        && cfg!(all(target_arch = "aarch64", target_feature = "aes"))
        && std::env::var_os("FLOCK_NO_DIRECT_FOLD8_QUAD").as_deref()
            != Some(std::ffi::OsStr::new("1"));
