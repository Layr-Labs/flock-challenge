...
fn materialize_direct_fold4(
    ...
    // OPT-IN (`FLOCK_OPEN_FOLD4_HETERO=1`): ranked submission 08031c76
    // measured this stack at -1.58% against an unmoved bar (outside the
    // runner's ~+-0.4% same-tree dispersion), and the isolation verdict
    // falls on this arm: in the GPU-hybrid commit era the E-core pool's
    // memory traffic contends with the GPU's unified-memory streams, so
    // widening a compute-bound open-phase block onto E-cores is no longer
    // the safe trade it was in the CPU-only phases. Default is the
    // incumbent main-pool rayon arm; the queue arm stays for re-pricing.
    let hetero = n_blocks >= 16
        && std::env::var_os("FLOCK_OPEN_FOLD4_HETERO").is_some_and(|v| v == "1");
    let stats = if hetero {
