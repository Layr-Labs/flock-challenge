...
fn materialize_direct_fold8(
    ...
    // Quad (slots4) doubles the pair width: one weight load fans out to four
    // independent deferred Karatsuba accumulators. Kill switches fall back
    // pair→single; FLOCK_NO_DIRECT_FOLD8_SLOTS4 keeps pair but disables quad.
    let pair_fold64 = deferred_reduce
        && cfg!(all(target_arch = "aarch64", target_feature = "aes"))
        && std::env::var_os("FLOCK_NO_DIRECT_FOLD8_PAIR").is_none();
    let quad_fold64 = pair_fold64
        && std::env::var_os("FLOCK_NO_DIRECT_FOLD8_SLOTS4").is_none();
