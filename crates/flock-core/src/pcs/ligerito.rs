...
pub(crate) fn induce_sumcheck_poly(
    ...
    let partials: Vec<(Vec<F128>, F128)> = (0..n_threads)
        ...
        .map(|t| {
            ...
            // Per-thread scratch reused across this chunk's queries. Uninit is
            // sound here: `evaluate_scaled_basis_inplace` writes every slot
            // before any read — `basis[0] = alpha`, then each doubling level
            // writes `[2^k, 2^{k+1})` from the already-written lower half —
            // the same write-before-read contract as `alloc_uninit_vec`'s
            // other callers. Deletes one length-n memset per worker per call.
