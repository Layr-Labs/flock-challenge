...
fn induce_sumcheck_poly_via_pruned_rate8_ntt(
    ...
    let ntt = AdditiveNttF128::standard(log_block);
    let coeffs =
        transpose_forward_ntt_sparse_rate8_low_eighth(&ntt, queries, &alpha_pows, log_block);
...
mod tests {
    ...
    #[test]
    fn l2_pruned_rate8_induction_matches_dense_at_production_shape() {
        use crate::challenger::Challenger;
        ...
        let pruned = induce_sumcheck_poly_via_pruned_rate8_ntt(
            LOG_MSG,
            LOG_RATE,
            &opened_rows,
            &v_challenges,
        ...
        assert_eq!(pruned.1, dense.1, "enforced sum changed");
        assert_eq!(pruned.0, dense.0, "retained L2 basis bytes changed");
    }
