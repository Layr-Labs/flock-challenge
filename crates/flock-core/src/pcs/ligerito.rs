...
fn fold2_and_msgs_lsb(
    ...
    // zerocheck tail's NT rounds). 2^21 F128 = 32 MiB per polynomial (both polynomials together exceed LLC).
    #[cfg(all(target_arch = "aarch64", target_feature = "aes"))]
    let nt_stores = {
        ...
        quarter >= (1usize << 21)
            && *NT_ENABLED
                .get_or_init(|| {
                    std::env::var_os("FLOCK_LIG_NT_LEGACY").is_none()
                        && std::env::var_os("FLOCK_NO_REAPPLY_D1").is_none()
                })
    };
    let acc = wf
        .par_chunks_mut(CHUNK)
