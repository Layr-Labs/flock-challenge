...
fn recursive_prover_with_basis_impl<Ch: Challenger>(
    ...
    let _t = std::time::Instant::now();
    // Scope the NTT table so its twiddles (2^{log_msg+log_inv}-1 F128s) drop
    // immediately after ligero_commit — they are not consulted by open/induce
    // and would otherwise stack under wtns_1.mat + L0 through OOD/open.
    let wtns_1 = {
    ...
    for i in 0..r {
        ...
        let _t = std::time::Instant::now();
        // Drop NTT twiddles as soon as the codeword+tree exist (commit-only).
        let wtns_next = {
...
fn recursive_prover_inner<Ch: Challenger>(
    ...
    for i in 0..r {
        ...
        let log_inv_rate_next = config.log_inv_rates[i + 2];
        let t = std::time::Instant::now();
        // Drop NTT twiddles + temporary f_evals copy with the commit.
        let wtns_next = {
            let ntt_next = AdditiveNttF128::standard(log_msg_cols_next + log_inv_rate_next);
