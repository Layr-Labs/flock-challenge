...
/// ```
///
/// Therefore one pass over `(f, w)` can accumulate `a = sum f_0 w` and
/// `s = sum (f_0 + f_1) w`, from which the incumbent full-table outputs are
/// exactly `u_0 = (1 + z_0)a`, `u_2 = s`, and `y = a + z_0 s`.
fn round_msg_and_eval_lsb_factorized_eq(
    ...
    let n = f.len();
    debug_assert!(n.is_power_of_two() && n >= 2);
    debug_assert_eq!(eq_tail.len(), n / 2);
    let half = n / 2;

    const PAR_THRESHOLD: usize = 4096;
    let (a, s) = if half < PAR_THRESHOLD {
        crate::field::f128_slice::round0_factorized_eq(f, eq_tail)
    } else {
        // Match the consuming fold's production granularity. Each chunk is a
        // complete independent product sum, so the AArch64 path reduces only
        // twice per 2,048 tail values instead of twice per value.
        const CHUNK: usize = 2048;
        f.par_chunks(2 * CHUNK)
            .zip(eq_tail.par_chunks(CHUNK))
            .map(|(f_chunk, tail_chunk)| {
                crate::field::f128_slice::round0_factorized_eq(f_chunk, tail_chunk)
            })
            .reduce(
                || (F128::ZERO, F128::ZERO),
...
/// Fold the incumbent `(f, combined_basis)` state while injecting a
/// factorized OOD equality basis into the freshly-folded basis before the
/// next-round message is accumulated.
///
/// If the deferred OOD term is `beta * eq([z_0, z_tail...])`, binding the LSB
/// challenge `r` turns it into
///
/// ```text
/// gamma * eq(z_tail),  gamma = beta * (1 + z_0 + r).
/// ```
///
/// The architecture-selected kernel computes the ordinary folds, adds
/// `d = gamma * eq_tail` to each folded basis value, stores the final state,
/// and accumulates the message against that corrected state in one traversal.
/// Its portable fallback implements the identical algebra.
fn fold_and_msg_lsb_into_with_lazy_ood_eq(
    ...
    assert!(
        ...
        "lazy OOD fold input shape changed"
    );
    assert_eq!(b.len(), n, "lazy OOD fold polynomial lengths differ");
    let half = n / 2;
    assert_eq!(eq_tail.len(), half, "lazy OOD tail/fold length mismatch");
    assert!(
        nf.capacity() >= half && nb.capacity() >= half,
        "lazy OOD spare capacity is insufficient"
    );
    assert!(
        half >= 2 && half.is_multiple_of(2),
        "lazy OOD consuming fold requires an even next-round length"
    );
    // SAFETY: capacities are hard-checked above and F128 is Copy. Every slot
    // is overwritten by the fused helper before either output slice is read.
    unsafe {
        nf.set_len(half);
        nb.set_len(half);
    }
    ...
    const PAR_THRESHOLD: usize = 4096;
    if half < PAR_THRESHOLD {
        let (u_0, u_2) = crate::field::f128_slice::fold_two_and_msg_with_scaled_basis_addend(
            f, b, eq_tail, 0, nf, nb, r, gamma,
        );
        return SumcheckMessage { u_0, u_2 };
    }

    // `half` and CHUNK are powers of two, so each chunk begins at an even
    // folded-domain index and no message pair crosses a chunk boundary.
    const CHUNK: usize = 2048;
    let (u_0, u_2) = nf
        .par_chunks_mut(CHUNK)
        .zip(nb.par_chunks_mut(CHUNK))
        .enumerate()
        .map(|(chunk_index, (f_chunk, b_chunk))| {
            let base = chunk_index * CHUNK;
            crate::field::f128_slice::fold_two_and_msg_with_scaled_basis_addend(
                f, b, eq_tail, base, f_chunk, b_chunk, r, gamma,
            )
        })
        .reduce(
            || (F128::ZERO, F128::ZERO),
...
pub struct SumcheckProver {
    ...
    /// Ranked L1 explicit-OOD equality kept as `(eq(z[1..]), z[0], beta)`
    /// until the next fold. Ordinary induced-basis introduce/glue operations
    /// proceed independently through `pending_glue` in the meantime.
...
impl SumcheckProver {
    ...
    /// Introduce `eq(z, ·)` without materializing its LSB pair dimension.
    /// Returns `None` without changing state for unsupported geometry or when
    /// another lazy OOD introduction is already pending, so callers can use
    /// the incumbent full-table path as an exact fallback.
    fn introduce_new_ood_factorized(
        ...
        let eq_tail = crate::lincheck::build_eq_table_optimized(&z[1..]);
        let z_0 = z[0];
        let (msg, h_new) =
            round_msg_and_eval_lsb_factorized_eq(&self.f, &eq_tail, z_0);
        self.transcript.push(msg);
    ...
    /// Apply the OOD separation challenge while retaining the equality tail
    /// for the next fold. This mirrors [`Self::glue`]'s target update but does
    /// not write a full equality table into `combined_basis`.
...
fn recursive_prover_with_basis_impl<Ch: Challenger>(
    ...
    // the interleaved list before any of L0's queries are drawn.
    {
        ...
        for _ in 0..ood_count(1) {
            ...
            // Ranked L1 keeps the equality table factorized across its LSB
            // pair and defers materialization through the ordinary induced
            // basis introduce/glue below. Any unsupported state falls back to
            // the incumbent full equality table before transcript mutation.
...
mod tests {
    ...
    /// also crosses both factorized-intro and correction parallel thresholds.
    #[test]
    ...
    fn lazy_ood_timing_condition_cache(words: &[u64], salt: u64) {
        ...
        // it models the much larger commit/open/induce working set that evicts
        // the retained 4 MiB equality tail in the real prover.
