...
fn cpu_blake3_pow_window_inner(
    ...
    // can return. Keep its work quantum to five full 48-nonce register-kernel
    // batches: this bounds the post-abort straggler on every P/E worker without
    // introducing a partial SIMD batch. Calibration drains the same finer queue,
    // so its CPU timing remains an exact measurement of this production arm.
    const CHUNK: u64 = 240;
    const NO_MATCH: u64 = u64::MAX;
