...
mod tests {
    ...
    /// The two-pool early-exit grind must emit exactly the brute-force
    /// smallest satisfying nonce — proof bytes depend on it. bits = 14
    ...
    #[test]
    fn grind_two_pool_matches_brute_force_smallest() {
        for kind in KINDS {
            ...
            let mut expect = 0u64;
            while !pow_has_leading_zero_bits(&digest, expect, bits, kind) {
                expect += 1;
            }
