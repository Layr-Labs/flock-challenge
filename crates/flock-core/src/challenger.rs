...
// ---------------------------------------------------------------------------
// Software-pipelined SHA-256 absorb.
//
// Replaces the `sha2::Sha256` hasher inside the FsChallenger's transcript
// state with an inline, hand-rolled SHA-256 whose full-block path overlaps
// the next block's load/LE-decode with the current block's G-round chain.
//
// Fission: per 64-byte block, the standard fused loop interleaves
//
//     decode[i+1] -> compress[i] -> decode[i+2] -> compress[i+1] -> ...
//
// so the integer byteswap that unpacks the 16 message words (LE on a
// little-endian host, byteswapped to the BE form SHA-256 consumes) sits
// squarely in front of `compress_in_place`. With the chain fused, that
// byteswap stalls the start of the next round. We hoist the decode into
// a separate `#[inline] decode_block_le` helper that writes into a
// caller-owned `[u32; 16]` stack buffer, and we keep TWO such buffers;
// while `compress_in_place` mutates the state against `w_curr`, the
// prologue already populates `w_next` from `blocks[i+1]`. The pipelined
// prologue thus hides the byteswap under the G-round latency, and the
// `#[inline]` boundary over a raw `*const u8` input keeps the
// dependence only on the input pointer and the output buffer — letting
// SROA/materialization hoist the loop induction fully out of the way of
// the compress chain.
//
// One buffer, not two stacks per loop, on the first/last block: the
// `blocks.len() < 2` and tail-block branches are short and not
// pipelined — they cost nothing relative to the n-block main loop on
// the realistic F128-slice absorb (the round-1 vector is many MiB).
//
// Determinism: the produced digest is byte-for-byte identical to
// `sha2::Sha256` on every input we feed it, because (a) the SHA-256 IV,
// (b) the message-schedule schedule, (c) the G-round function and
// constants K32, and (d) the trailing 0x80/0x00/64-bit length padding
// all match the FIPS-180-4 spec that `sha2` also implements.
// ---------------------------------------------------------------------------
...
/// Hand-rolled SHA-256 with the loop-fission prologue over a 2-deep ping-pong
/// message-word buffer. FIPS-180-4 spec-compatible with `sha2::Sha256` so the
...
impl InlineSha256 {
    ...
    /// Absorb `bytes` into the running SHA-256 state. The full-block path is
    /// software-pipelined: the load/LE-decode of block `i+1` runs ahead of
    /// block `i`'s compress chain. Bytes that do not fill a whole 64-byte
    /// block land in an internal buffer drained by the next call or by
    /// [`Self::finalize`].
    #[inline]
    fn update(&mut self, mut bytes: &[u8]) {
        self.total_len = self.total_len.wrapping_add(bytes.len() as u64);

        // 1) Top off the leftover buffer with the head of `bytes`.
        if self.buffer_len > 0 {
            ...
            if self.buffer_len == 64 {
                // Buffer just filled: compress it through the prologue. There
                // is no `blocks[i+1]` yet, so the single-block path applies.
                let w = decode_block_le(self.buffer.as_ptr());
                compress_block(&mut self.state, &w);
                self.buffer_len = 0;
            }
        }

        // 2) Pipelined full-block main loop. `blocks` is a raw `*const u8`
        //    so the per-iteration decode can be hoisted above the G-round
        //    chain without carrying an array slice through the loop.
        let n_full = bytes.len() / 64;
        if n_full >= 2 {
            // SAFETY: `bytes.len() >= 2 * 64` so the prologue reads of
            // `p` and `p.add(64)` are both inside the source slice. The
            // main loop guards the `w_next = decode(p.add(64))` reload
            // with an explicit in-bounds test on the *current* `p` —
            // a successor block must exist at `p + 64` before the
            // reload runs.
            let p0 = bytes.as_ptr();
            let end = unsafe { p0.add(n_full * 64) };

            // Prologue: decode block 0, then decode block 1 into the second
            // buffer so the chain of the first compress already has its
            // successor's words materialized.
            let mut p = p0;
            let mut w_curr = decode_block_le(p);
            let mut w_next = decode_block_le(unsafe { p.add(64) });

            // Steady state: compress `w_curr` while `w_next` already holds
            // the next block's decoded words. After each compress we
            // advance; the loop guard re-fetches `w_next` only while a
            // successor block (block at `p + 64`) is still in-bounds.
            loop {
                compress_block(&mut self.state, &w_curr);
                let next_p = unsafe { p.add(64) };
                if unsafe { next_p.add(64) } >= end {
                    // The just-compressed block is the second-to-last
                    // (or earlier): one more block remains at `next_p`.
                    p = next_p;
                    w_curr = w_next;
                    break;
                }
                p = next_p;
                w_curr = w_next;
                w_next = decode_block_le(unsafe { p.add(64) });
            }

            // Final compress in the pipelined stream.
            compress_block(&mut self.state, &w_curr);

            // 3) Stash the leftover tail (0..63 bytes) in the buffer.
            let tail = &bytes[n_full * 64..];
            debug_assert!(tail.len() < 64);
            self.buffer[..tail.len()].copy_from_slice(tail);
            self.buffer_len = tail.len();
        } else if n_full == 1 {
            // Single full block: not worth the pipelined prologue — just
            // decode + compress directly.
            let w = decode_block_le(bytes.as_ptr());
            compress_block(&mut self.state, &w);
            // `bytes.len() - 64 == 0` here, so no tail to buffer.
    ...
    /// Finalize the hash: pad the trailing partial block per FIPS-180-4
    /// (`0x80`, zeros, 64-bit big-endian bit-length) and produce the
    /// 32-byte digest. Consumes the hasher.
    fn finalize(mut self) -> [u8; 32] {
        let bit_len = self.total_len.wrapping_mul(8);
        // Append the 0x80 sentinel.
        self.buffer[self.buffer_len] = 0x80;
        self.buffer_len += 1;

        if self.buffer_len > 56 {
            // Not enough room for the 8-byte length: pad the rest of this
            // block with zeros, compress it, then fall through to a clean
            // length-only block.
        ...
        // 8-byte big-endian bit length into the last 8 bytes of the
        // terminal block, then compress.
...
/// Decode 64 source bytes (interpreted as 16 little-endian `u32` words on
/// a little-endian host) into the 16 big-endian SHA-256 message words.
/// Kept as a separate `#[inline]` helper over a raw `*const u8` so the
/// call site can hold the output in a caller-owned stack buffer and the
/// dependency on the input pointer stays first-class — exactly what the
/// SROA/scheduler need to hoist the decode ahead of the G-round chain.
///
/// On a big-endian host this is the canonical `from_be_bytes` (the input
/// is already in SHA-256's preferred byte order), so the "LE→BE byteswap"
/// reduces to a no-op for non-F128 absorbs there. The intent of the
/// helper is unchanged: load 16 words, leave them in BE form for the
/// message schedule.
#[inline]
fn decode_block_le(p: *const u8) -> [u32; 16] {
    let mut w = [0u32; 16];
    let mut q = p;
    let mut i = 0;
    while i < 16 {
        // SAFETY: the caller (`update`) bounds `p` to a 64-byte block (or
        // to the start of an `n_full * 64` run, in which case every
        // address we touch stays within the source slice).
        let b0 = unsafe { *q } as u32;
        let b1 = unsafe { *q.add(1) } as u32;
        let b2 = unsafe { *q.add(2) } as u32;
        let b3 = unsafe { *q.add(3) } as u32;
        q = unsafe { q.add(4) };
        w[i] = match cfg!(target_endian = "little") {
            true => (b3 << 24) | (b2 << 16) | (b1 << 8) | b0,
            false => (b0 << 24) | (b1 << 16) | (b2 << 8) | b3,
        };
        i += 1;
    }
    w
...
/// Run the 64 G-rounds of SHA-256 over `w[0..16]`, extending the message
/// schedule in place and updating `state` in place. The schedule and
/// round function are exactly the FIPS-180-4 spec; constants and the
/// `Sigma0`/`Sigma1`/`sigma0`/`sigma1` rotations match `sha2::Sha256`'s
/// soft implementation.
#[inline]
fn compress_block(state: &mut [u32; 8], w: &[u32; 16]) {
    let mut schedule = [0u32; 64];
    schedule[..16].copy_from_slice(w);
    ...
    let mut round = 0;
    while round < 64 {
        // Extend the schedule past the first 16 words. Hoisted out of the
        // per-round critical path: the standard fused loop computes
        // `schedule[t]` lazily; here we already have a contiguous schedule
        // buffer so the extension cost is amortized across rounds.
        if round >= 16 {
            let s0 = (schedule[round - 15].rotate_right(7))
                ^ (schedule[round - 15].rotate_right(18))
                ^ (schedule[round - 15] >> 3);
            let s1 = (schedule[round - 2].rotate_right(17))
        ...
        let mj = (a & b) ^ (a & c) ^ (b & c);
        let temp2 = big_s0.wrapping_add(mj);
...
impl FsChallenger {
    ...
    pub fn with_hash(domain: &[u8], kind: HashKind) -> Self {
        let mut c = Self {
            state: match kind {
                HashKind::Sha256 => FsState::Sha256(InlineSha256::new()),
    ...
    fn state_digest(&self) -> [u8; 32] {
        ...
        match &self.state {
            FsState::Sha256(h) => h.clone().finalize(),
