    ...
    F: Fn(&S, &mut [u64], &mut [u64], &mut [u64]) + Sync,
{
    ...
    z.par_chunks_mut(8 * f128_per_block)
        ...
        // Each item emits eight blocks and touches hundreds of KiB across
        // z/a/b/stripe. Finer tasks let work stealing smooth per-core
        // variation without scheduling each group separately.
        .with_max_len(32)
        .enumerate()
