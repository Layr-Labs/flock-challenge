...
fn fold_alpha_batched(&self, alpha: F128, eq_inner: &[F128]) -> Vec<F128> {
        ...
        // reduction is bit-identical to the serial loop. The pin-row loop
        // above and this chi build both read only `eq_inner`, so they overlap
        // via `rayon::join` instead of running serially (mirroring the
        // chain-common fold's two-traversal overlap idiom).
        use rayon::prelude::*;
        let chi: Vec<(Vec<F128>, Vec<F128>, F128)> = rayon::join(
            || {
                (0..N_T)
                    .into_par_iter()
                    .map(|r| {
            ...
            || {
                // Overlap: re-walk the pin rows (both loops only read
                // `eq_inner`); the second pass's marginal is discarded —
                // `comb[Z_CONST]` was already credited above.
                let mut dummy = F128::ZERO;
                for j in 0..STATE_BITS {
                    let row = z_pos_state(24, j);
                    dummy += eq_inner[row];
                }
