...
/// materialized deferred stripe; `1` and an unset variable select the candidate
...
/// Exact-value emergency kill for the candidate ranked row-major lincheck path.
...
mod tests {
    ...
    fn row_major_lincheck_selector_is_default_on_killable_and_fold8_only() {
        ...
        // A cleared official environment enables the exact ranked path.
        ...
        // control. Non-exact kill values do not silently disable the candidate.
        assert!(!row_major_lincheck_env_enabled(
            None,
            Some(OsStr::new("1"))
        ));
