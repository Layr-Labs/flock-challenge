...
pub(crate) mod witgen_simd {
    ...
    // USEFUL_BITS ends in word 481, but zerocheck's coarse 512-bit windows
    // still read the complete b_med=14 window (words 480..495). Only the
    // final b_med=15 window (words 496..511) is skipped by every consumer.
    const AB_DUMP_GROUPS: usize = 62;
    ...
    pub(crate) unsafe fn build_quad_witness_ab_stream_neon(
        ...
        unsafe {
            ...
            // ---- drain stages: per-block ascending bursts ----
            if z_nt {
                dump::<true, 64>(zs, z);
            } else {
                dump::<false, 64>(zs, z);
