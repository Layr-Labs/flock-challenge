...
#[inline]
fn should_request_ranked_k56_retest(call: usize, ranked_row_major_enabled: bool) -> bool {
    call == 0 && ranked_row_major_enabled
}
...
impl Blake3Setup {
    ...
    pub fn prove_fast<Ch: Challenger>(
        ...
        // Call zero is the untimed benchmark warmup, but it intentionally
        // materializes a Ready lincheck stripe because the GPU stream/latch
        // is not established yet. Issue the exact-contention ticket here,
        // before witness generation, rather than waiting for the timed call's
        // first RowMajorRanked enum. The commit/AB join consumes this ticket;
        // call one can never become the first calibration requester.
        if should_request_ranked_k56_retest(
            call,
            use_ranked_row_major_lincheck(self.n_blocks_log(), &self.pcs_params),
        ) {
            let _ = flock_core::gpu_commit::request_ranked_k56_contention_retest();
        }
...
mod tests {
    ...
    #[test]
    fn ranked_k56_retest_is_warmup_call_only() {
        assert!(should_request_ranked_k56_retest(0, true));
        assert!(!should_request_ranked_k56_retest(1, true));
        assert!(!should_request_ranked_k56_retest(2, true));
        assert!(!should_request_ranked_k56_retest(0, false));
    }
