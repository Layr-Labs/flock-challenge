...
pub fn stash_pre_encoded_prefix(
    ...
    if crate::seed_pipe::is_ranked_worker() {
        // `with_capacity` reserves address space but does not establish the
        // backing pages. Touch one byte per 16 KiB Apple-Silicon page on the
        // helper thread while PCS open is running, so appending the ~433 KiB
        // opening does not first-fault output pages on the measured
        // publication tail.
        for page in bytes.spare_capacity_mut().chunks_mut(16 * 1024) {
            page[0].write(0);
        }
    }
