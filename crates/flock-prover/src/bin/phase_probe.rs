...
fn main() {
    ...
    let mut setup = Blake3Setup::with_profile(steps, Default::default());
    // The ranked benchmark's Merkle hash; HashKind::default() is Sha256,
    // which disables every ranked fast path (GPU commit, from-message,
    // hot codeword). Match the scored configuration.
    setup.pcs_params.merkle_hash = flock_core::hash::HashKind::Blake3;
    for run in 0..2 {
        let mut ch = FsChallenger::new(b"flock_chain-cli");
        let t = Instant::now();
        let (_proof, _commitment, _claim) = setup.prove_fast(&blocks, &mut ch);
