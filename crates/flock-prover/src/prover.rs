...
pub(crate) fn open_claims_with_precomputed_ligerito<Ch: Challenger>(
    ...
    // The ranked batch always opens AB and C together. Keep both owned point
    // vectors and their borrowed views in fixed-size stack arrays; the point
    // coordinates still own their storage, but the two-element outer Vec and
    // its allocator round-trip are unnecessary on every proof.
    assert_eq!(claims.len(), 2, "ranked Ligerito opening expects AB and C");
    let x_fulls = [
        quirky_x_outer_full(&claims[0].point),
        quirky_x_outer_full(&claims[1].point),
    ];
...
pub fn prove_ligerito<Ch: Challenger>(
    ...
    let (((a_packed_f128, b_packed_f128), c_packed_f128), z_packed_lincheck) = rayon::join(
        || {
            rayon::join(
                || {
                    rayon::join(
