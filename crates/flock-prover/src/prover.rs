...
pub fn prove_ligerito<Ch: Challenger>(
    ...
    let z_packed_lincheck = if std::env::var_os("FLOCK_PHASE_TIMING").is_some() {
        let t0 = std::time::Instant::now();
        let v = pack_z_lincheck_from_packed(&z_packed, r1cs.m, r1cs.k_log);
        eprintln!(
            "[flock-phase] lincheck_pack_z_elapsed_us={} bytes={}",
        ...
        || match commit_codeword {
            ...
            CommitCodeword::NeedsReplication(buf) => {
                if std::env::var_os("FLOCK_PHASE_TIMING").is_some() {
                    ...
                    let r = pcs::commit_into(z_packed, pcs_params, buf);
                    eprintln!(
                        "[phase-timing] commit_into (inner-lane) wall: {:.2} ms",
...
fn prove_fast_core_with_commit_codeword<Ch: Challenger>(
    ...
    if phase_timing {
        let wall = t_commit
            .expect("commit timer must exist when phase timing is enabled")
            .elapsed()
            .as_secs_f64()
            * 1e3;
    ...
    if phase_timing {
        let wall = t_zc
            .expect("zerocheck timer must exist when phase timing is enabled")
            .elapsed()
            .as_secs_f64()
            * 1e3;
    ...
    if phase_timing {
        let wall = t_lc
            .expect("lincheck timer must exist when phase timing is enabled")
            .elapsed()
            .as_secs_f64()
            * 1e3;
