...
/// Proof file opened during the untimed warm-up and handed to the speculative
/// prover. Keeping the descriptor live removes create/truncate/path-lookup
/// work from the publication tail; the final rename remains atomic.
...
pub(crate) fn arm(log2_size: u32, setup_addr: usize, run: fn(usize, &[Compression]) -> ProveOut) {
    ...
    // Also remove proof-file creation from the scored publication tail. This
    // runs before the worker publishes ready, and failure simply leaves the
    // existing create-at-publish fallback armed.
...
/// Resolve and open the ranked proof's private temporary file during the
/// untimed warm-up. A failed pre-open is not fatal: publication falls back to
/// the original timed `File::create` path.
...
fn publish_speculative_result(
    ...
    let (file, temporary, proof_path) = match prepared {
        Some(target) => (target.file, target.temporary, target.proof_path),
        None => {
            let Some((proof_path, temporary)) = speculative_publish_paths() else {
                return Ok(false);
            };
            let file = std::fs::File::create(&temporary)?;
            (file, temporary, proof_path)
        }
    };
    let result = (|| {
