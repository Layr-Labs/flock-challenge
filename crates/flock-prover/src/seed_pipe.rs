...
/// Exact-`1` kill for the ILP-unrolled generator; anything else leaves
...
/// Debug selection is captured by [`arm`] during the untimed warm-up so the
/// publishing path never performs an environment lookup.
static SEED_PIPE_DEBUG: AtomicBool = AtomicBool::new(false);
...
pub(crate) fn arm(log2_size: u32, setup_addr: usize, run: fn(usize, &[Compression]) -> ProveOut) {
    ...
    SEED_PIPE_DEBUG.store(
        std::env::var_os("FLOCK_SEED_PIPE_DEBUG").is_some(),
        Ordering::Relaxed,
    );
...
fn speculative_main(
    ...
    // [`arm`] resolved this control before the measured interval.
    if defer_join {
...
pub(crate) fn try_adopt(blocks: &[Compression]) -> Option<ProveOut> {
    ...
    if SEED_PIPE_DEBUG.load(Ordering::Relaxed) {
