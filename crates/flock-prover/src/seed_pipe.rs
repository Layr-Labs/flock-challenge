...
struct SpecPublishTarget {
    ...
    temporary: PathBuf,
    proof_path: PathBuf,
    rename: Option<PreparedRename>,
}

/// Directory handle and leaf names resolved during warm-up. `renameat` keeps
/// the same atomic replacement semantics without walking the shared parent
/// path twice after the proof completes.
struct PreparedRename {
    directory: std::fs::File,
    temporary_name: std::ffi::CString,
    proof_name: std::ffi::CString,
}
...
impl MappedProofWriter {
    ...
    /// Append one complete encoder segment. Unlike a general `Write`, this
    /// sink never makes partial progress: it stores the entire slice or
    /// returns an error before changing `len`.
    #[inline]
...
pub(crate) fn arm(log2_size: u32, setup_addr: usize, run: fn(usize, &[Compression]) -> ProveOut) {
    ...
    // Shape and stable-buffer length are known before readiness. `SPEC_BASE`
    // stays zero until the seed-specific init is ready, so this length is
    // unobservable until the later release store publishes the identity.
    SPEC_LEN.store(scratch.len(), Ordering::Relaxed);
    ...
    let options = RankedOptions {
        ...
        gen_ilp,
        n_total: scratch.len(),
        generator_mask: u64::from(log2_size).rotate_left(29),
    };
        ...
        let (blocks, endpoints, deferred_endpoint_init) = if lazy {
            ...
            if options.defer_forward {
                ...
                let endpoints = (
                    ...
                    gen_block_with(init, options.n_total - 1, options.gen_ilp),
                );
                (buf, Some(endpoints), None)
            }
        } else if buf.len() == options.n_total {
...
fn prepare_speculative_publish_target(
    ...
    let rename = prepare_rename(temporary, proof_path);
...
fn prepare_rename(
    ...
    let parent = proof_path.parent()?;
    if temporary.parent()? != parent {
        return None;
    }
    let directory = std::fs::File::open(parent).ok()?;
    let temporary_name = std::ffi::CString::new(temporary.file_name()?.as_bytes()).ok()?;
    let proof_name = std::ffi::CString::new(proof_path.file_name()?.as_bytes()).ok()?;
    Some(PreparedRename {
        directory,
        temporary_name,
        proof_name,
...
fn publish_speculative_result(
    ...
    let (temporary, result) = match prepared {
        ...
        None => {
            ...
            let result = publish_to_writer(out, &mut writer, &temporary, &proof_path, None);
            (temporary, result)
        }
    };
    if result.is_err() {
...
fn publish_to_writer<W: Write>(
    ...
    writer.flush()?;
    publish_rename(rename, temporary, proof_path)
}
...
fn publish_rename(
    ...
    if let Some(rename) = rename {
        // SAFETY: the held directory descriptor is live and both C strings
        // are NUL-terminated leaf names within that same directory.
        let result = unsafe {
            renameat(
                rename.directory.as_raw_fd(),
                rename.temporary_name.as_ptr(),
                rename.directory.as_raw_fd(),
                rename.proof_name.as_ptr(),
            )
        };
        if result == 0 {
            return Ok(());
...
mod tests {
    ...
    #[test]
    fn prepared_rename_publishes_with_cached_directory() {
        ...
        let temporary = directory.join("proof.spec.tmp");
        let proof = directory.join("proof");
        std::fs::write(&temporary, b"verified-bytes").unwrap();
        let rename = prepare_rename(&temporary, &proof).unwrap();
        publish_rename(Some(&rename), &temporary, &proof).unwrap();
        assert_eq!(std::fs::read(&proof).unwrap(), b"verified-bytes");
