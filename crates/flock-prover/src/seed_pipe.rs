...
fn read_line_fd(fd: i32) -> Option<Vec<u8>> {
    ...
    let spin = std::env::var_os("FLOCK_NO_SEED_SPIN").is_none();
...
fn publish_prewarm_enabled() -> bool {
    ...
    *ON.get_or_init(|| std::env::var_os("FLOCK_NO_PUBLISH_PREWARM").is_none())
}
...
fn spawn_publish_prewarm() {
    if !publish_prewarm_enabled() || std::env::var_os("FLOCK_NO_EARLY_PUBLISH").is_some() {
        return;
    }
...
fn early_publish(out: ProveOut) -> ProveOut {
    if std::env::var_os("FLOCK_NO_EARLY_PUBLISH").is_some()
