...
fn speculative_main(
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let lazy = buf.len() == n_total
            ...
            && !lazy_blocks_killed();
        let (blocks, mut endpoints) = if lazy {
        ...
        let blocks_at = std::time::Instant::now();
        if !defer_forward {
            let mut state = shared().state.lock().unwrap_or_else(|e| e.into_inner());
            state.seed_at = Some(seed_at);
            state.blocks_at = Some(blocks_at);
            state.endpoints = endpoints.take();
            state.blocks = Some(Arc::clone(&blocks));
            shared().signal.notify_all();
        }
        let out = run(setup_addr, &blocks);
        let deferred_adoption = if defer_forward {
            Some((blocks, endpoints, blocks_at))
        } else {
            None
        };
        (out, deferred_adoption)
    }));
