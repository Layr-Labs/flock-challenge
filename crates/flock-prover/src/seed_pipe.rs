...
fn speculative_main(
    ...
    match outcome {
        Ok(out) => {
            ...
            let _ = publish_speculative_result(&out);
            let mut state = shared().state.lock().unwrap_or_else(|e| e.into_inner());
...
fn publish_speculative_result(out: &ProveOut) -> std::io::Result<()> {
    if std::env::var("FLOCK_NO_SPEC_PUBLISH").is_ok_and(|v| v == "1") {
        return Ok(());
    }
    let mut args = std::env::args_os();
    let _exe = args.next();
    ...
    let Some(proof_path) = args.next().map(std::path::PathBuf::from) else {
        return Ok(());
    };
