...
/// Single-write proof buffer whose backing pages are dirtied during warm-up.
///
/// `BufWriter::with_capacity` reserves address space early but its pages can
/// still fault on the first scored writes. This writer makes that residency
/// explicit, then implements the same accumulate-and-flush behavior. Proofs
/// larger than the reviewed capacity grow safely; the verifier's independent
/// 500,000-byte cap remains authoritative.
struct PrefaultedProofWriter {
    file: std::fs::File,
    buffer: Vec<u8>,
    len: usize,
    flushed: usize,
...
impl PrefaultedProofWriter {
    fn new(file: std::fs::File) -> Self {
        let buffer = vec![0xA5; SPEC_PUBLISH_CAPACITY];
...
impl Write for PrefaultedProofWriter {
    ...
    fn flush(&mut self) -> io::Result<()> {
        self.file.write_all(&self.buffer[self.flushed..self.len])?;
...
pub(crate) fn arm(log2_size: u32, setup_addr: usize, run: fn(usize, &[Compression]) -> ProveOut) {
    ...
    // worker publishes ready, and failure simply leaves the existing
    // create-at-publish fallback armed.
...
fn prepare_speculative_publish_target() -> Option<SpecPublishTarget> {
    ...
    let writer = PrefaultedProofWriter::new(file);
...
mod tests {
    ...
    fn publish_writer_path(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!(
            "flock-prefault-writer-{label}-{}",
            std::process::id()
        ))
    }

    #[test]
    fn prefaulted_proof_writer_flushes_incrementally_without_duplication() {
        let path = publish_writer_path("incremental");
        let _ = std::fs::remove_file(&path);
        let file = std::fs::File::create(&path).unwrap();
        let mut writer = PrefaultedProofWriter::new(file);
        assert_eq!(writer.buffer.len(), SPEC_PUBLISH_CAPACITY);
        assert!(writer.buffer.iter().step_by(4096).all(|&byte| byte == 0xA5));
    ...
    #[test]
    fn prefaulted_proof_writer_grows_safely_past_reviewed_capacity() {
        ...
        assert!(writer.buffer.len() >= bytes.len());
        writer.flush().unwrap();
        drop(writer);
